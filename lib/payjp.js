...
const PAYJP_SUPPORTED_LOCALES = ['ja', 'en'];
...
export function normalizePayjpLocale(value = '', fallback = 'ja') {
  const raw = normalizeString(value || fallback, fallback).toLowerCase();
  if (raw.startsWith('ja')) return 'ja';
  if (raw.startsWith('en')) return 'en';
  return PAYJP_SUPPORTED_LOCALES.includes(fallback) ? fallback : 'ja';
}
...
export function payjpConfigFromEnv(source = {}) {
  return {
    ...
    defaultLocale: normalizePayjpLocale(source.PAYJP_DEFAULT_LOCALE || 'ja')
  };
}
...
export function payjpPublicConfig(config = null) {
  const locale = normalizePayjpLocale(config?.defaultLocale || 'ja');
  return {
    ...
    supportedLocales: [...PAYJP_SUPPORTED_LOCALES]
  };
}
