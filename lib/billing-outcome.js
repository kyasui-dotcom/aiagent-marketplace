export function createBillingOutcomeHelpers(deps = {}) {
  ...
  function mutateBillingForJob(storage, job, mutator) {
    if (typeof storage?.mutateBillingForJob === 'function' && job?.id) {
      return storage.mutateBillingForJob(job.id, mutator);
    }
  ...
  async function recordBillingOutcome(storage, job, billing, source) {
    ...
    const paymentProcessingRemoved = reservation?.paymentProcessingRemoved === true || reservation?.mode === 'donation_only';
    if (paymentProcessingRemoved) {
      let openAiSettlement = null;
      const settlement = {
        mode: 'donation_only',
        period: reservation?.period || billingPeriodId(job.createdAt),
        estimatedTotal: 0,
        total: 0,
        openAiCostSettled: Number(billing.apiCost ?? billing.totalCostBasis ?? 0) || 0,
        welcomeCreditsApplied: 0,
        creditsApplied: 0,
        depositApplied: 0,
        invoiceApplied: 0,
        autoTopupAdded: 0,
      ...
      await mutateBillingForJob(storage, job, async (draft) => {
        const draftJob = draft.jobs.find((item) => item.id === job.id);
        if (!draftJob) return;
        if (typeof settleOpenAiCostForJobInState === 'function') {
          openAiSettlement = settleOpenAiCostForJobInState(draft, draftJob, billing);
        }
        if (openAiSettlement) {
          settlement.openAiCostSettled = openAiSettlement.openAiCostSettled || settlement.openAiCostSettled;
          settlement.openAiMonthlyCostLimit = openAiSettlement.openAiMonthlyCostLimit || null;
        }
        draftJob.billingSettlement = settlement;
        draftJob.actualBilling = {
          ...(draftJob.actualBilling && typeof draftJob.actualBilling === 'object' ? draftJob.actualBilling : {}),
      ...
      await touchEvent(storage, 'BILLED_DONATION_ONLY', `payment processing removed total=${billing.total}`, {
        jobId: job.id,
        source
      });
