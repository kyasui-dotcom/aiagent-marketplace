...
async function fetchBraveWebSourcesWithinBudget(kind = '', body = {}, source = {}, timeoutMs = 5000) {
  const query = braveSearchQuery(kind, body);
  let timer = null;
  try {
    return await Promise.race([
      fetchBraveWebSources(kind, body, source),
      new Promise((resolve) => {
        timer = setTimeout(() => {
          resolve({
            enabled: true,
            ...
            sources: [],
            timedOut: true,
            error: new Error(`Brave source collection exceeded workflow budget after ${timeoutMs}ms`)
          });
...
function workflowCanCompleteFromLeaderPacket(kind = '', body = {}) {
  const normalizedKind = normalizedBuiltInKind(kind);
  if (!workflowContext(body)) return false;
  if (!isLeaderTaskType(normalizedKind)) return false;
  if (workflowRequiresSearchSources(body)) return false;
  return ['initial', 'checkpoint', 'final_summary', 'summary', ''].includes(workflowSequencePhase(body));
}
...
function workflowSearchSourcePacketPayload(kind = '', body = {}, braveSearch = {}, stages = []) {
  ...
  const sourceError = String(braveSearch.error?.message || braveSearch.error || '').trim();
  const sourceLimited = !collectedSources.length;
  const sources = collectedSources.length
    ? collectedSources
    : [{
        title: isJapanese ? '検索ソース取得は時間内に完了しませんでした' : 'Search source collection did not finish within the workflow budget',
        url: '',
        query,
        action: braveSearch.timedOut ? 'brave_search_timeout' : 'source_collection_attempt',
        provider: String(braveSearch.provider || 'brave').trim() || 'brave',
        snippet: sourceError || (isJapanese
          ? '後続工程はこのソース不足を明示し、公開前にURL検証を行ってください。'
          : 'Downstream work must state this source gap and verify URLs before publication.')
      }];
  ...
  const summary = isJapanese
    ? (sourceLimited ? '検索ソース制限付きパケットを作成しました' : `検索ソースパケットを作成しました: ${sources.length}件のBrave Searchソース`)
    : (sourceLimited ? 'Research source-limited packet ready.' : `Research source packet ready: ${sources.length} Brave Search source(s) collected.`);
  const nextAction = isJapanese
    ? (sourceLimited
      ? 'Planning/Preparation層へ進みますが、公開前に競合URLとSERPを再確認してください。'
      : 'この検索ソースをPlanning/Preparation層へ渡し、媒体選定、SEOギャップ、実行前承認パケットに反映してください。')
    : (sourceLimited
      ? 'Continue to planning/preparation with the source gap stated explicitly; verify competitor/SERP URLs before publication.'
  ...
  const process = [
    ...stages,
    stageRecord('source_packet', 0, sourceLimited
      ? 'Completed as a source-limited research packet to avoid blocking the workflow.'
      : `Completed from ${sources.length} Brave Search source(s).`, { status: 'completed' })
  ];
  const markdown = [
  ...
  return {
    ...
    report: {
      ...
      assumptions: [
        sourceLimited
          ? 'Live source collection did not return URLs inside the workflow budget; downstream work must treat external claims as unverified until URLs are checked.'
          : 'This workflow research layer used collected search snippets and URLs directly to stay within the Cloudflare background execution budget.',
...
async function callOpenAi(kind, body, source = {}) {
  ...
  try {
    braveSearch = sourcePacketEligible
      ? await fetchBraveWebSourcesWithinBudget(kind, body, source, 5000)
  ...
  if (sourcePacketEligible) {
    stages.push(stageRecord('source_preflight', 0, braveSearch.sources?.length
      ? `${braveSearch.sources.length} Brave Search source(s) available before generation.`
      : `No Brave Search URL returned inside the workflow budget; completing a source-limited packet.`, { status: braveSearch.sources?.length ? 'completed' : 'source_limited' }));
  ...
  if (fastWorkflow && workflowCanCompleteFromLeaderPacket(kind, body)) {
...
export async function runBuiltInAgent(kind, body = {}, source = {}) {
  ...
  const config = openAiConfig(source);
  const sampleFallbackEnabled = ['1', 'true', 'yes', 'on'].includes(
    envValue(source, 'BUILTIN_AGENT_SAMPLE_FALLBACK').toLowerCase()
  );
