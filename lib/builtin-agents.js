...
async function workflowSearchSourcePacketPayload(kind = '', body = {}, braveSearch = {}, stages = [], source = {}) {
  ...
  const bullets = synthesis.bullets;
  const summary = isJapanese
  ...
  return {
    ...
    usage: {
      ...
      total_tokens: Math.max(160, (JSON.stringify({ query, objective, taskFrame, sources, research_findings: synthesis.structured }).length + markdown.length) / 4),
      total_cost_basis: 0,
      cost_currency: 'USD',
      model: 'brave-search-research-synthesis',
      model_tier: 'source_collection_plus_synthesis',
      model_source: 'brave_search'
    },
    return_targets: ['chat', 'api'],
    runtime: {
      ...
      provider: 'brave',
      model: 'brave-search-research-synthesis',
      workflow: 'workflow_research_synthesis_packet',
