...
function workflowSearchSourcePacketPayload(kind = '', body = {}, braveSearch = {}, stages = []) {
  ...
  const workflow = workflowContext(body) || {};
  const request = payloadInput(kind, body);
  const sources = (Array.isArray(braveSearch.sources) ? braveSearch.sources : []).slice(0, 8);
  const query = String(braveSearch.query || sources[0]?.query || '').trim();
...
async function callOpenAi(kind, body, source = {}) {
  ...
  if (fastWorkflow && workflowCanCompleteFromSearchSourcePacket(kind, body) && braveSearch.sources.length) {
