...
function pushUniqueTerm(list = [], value = '') {
  ...
  if (!term) return;
  if (/(?:^|[-_\s])(?:delivery|handoff|packet)\.(?:md|markdown|txt|json|html)$/i.test(term)) return;
  if (/\b(?:leader|agent|data|research|analysis|cmo|seo)?[-_\s]*(?:delivery|handoff|packet)\.(?:md|markdown|txt|json|html)\b/i.test(term)) return;
  const key = term.toLowerCase();
  if (list.some((item) => String(item || '').toLowerCase() === key)) return;
  list.push(term);
}

function sanitizeSearchQuery(value = '', max = 260) {
  const normalized = String(value || '')
    .replace(/\b[\w-]*(?:delivery|handoff|packet)\.(?:md|markdown|txt|json|html)\b/ig, ' ')
    .replace(/[…]/g, ' ')
    .replace(/[^\p{L}\p{N}._+#:/ -]+/gu, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  if (!normalized || normalized.length <= max) return normalized;
  const clipped = normalized.slice(0, Math.max(1, max)).replace(/\s+\S*$/, '').trim();
  return clipped || normalized.slice(0, Math.max(1, max)).trim();
}
...
function meaningfulSearchTokens(text = '', limit = 8) {
  ...
  for (const token of cleaned.split(' ')) {
    ...
    if (/\b(?:delivery|handoff|packet)\.(?:md|markdown|txt|json|html)$/i.test(normalized)) continue;
...
function genericSearchQueryForKind(kind = '', body = {}, sourceText = '') {
  ...
  return sanitizeSearchQuery(terms.join(' '), 260);
}
...
async function fetchBraveWebSources(kind = '', body = {}, source = {}) {
  ...
  const query = braveSearchQuery(kind, body)
    || sanitizeSearchQuery([kind, promptText(body)].map((item) => String(item || '').trim()).filter(Boolean).join(' '), 260);
  if (!query) {
    return { enabled: true, provider: 'brave', query: '', sources: [] };
  }
  const queryLang = /[\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han}]/u.test(query) ? 'ja' : 'en';
  const lang = String(envValue(source, 'BRAVE_SEARCH_LANG') || queryLang).trim().toLowerCase();
  ...
  const requestVariants = [
    { country, search_lang: lang, ui_lang: uiLang, extra_snippets: 'true' },
    { country, search_lang: lang, ui_lang: uiLang },
    { country: 'ALL', search_lang: lang, ui_lang: uiLang }
  ];
  ...
  try {
    ...
    for (const variant of requestVariants) {
      const params = new URLSearchParams({
        ...
        search_lang: variant.search_lang,
        ui_lang: variant.ui_lang,
        safesearch: 'moderate',
        ...(variant.extra_snippets ? { extra_snippets: variant.extra_snippets } : {})
      });
    ...
    throw lastError || new Error('Brave search failed before returning sources.');
  } finally {
    clearTimeout(timer);
    try {
      controller.abort();
...
function workflowSourceCollectionTimeoutMs(source = {}) {
  const configured = parseNumber(
    envValue(source, 'WORKFLOW_SOURCE_COLLECTION_TIMEOUT_MS')
      || envValue(source, 'WORKFLOW_RESEARCH_SOURCE_TIMEOUT_MS'),
    60000
  );
  return Math.max(5000, Math.min(120000, configured));
}

async function fetchBraveWebSourcesWithinBudget(kind = '', body = {}, source = {}, timeoutMs = 60000) {
  const query = braveSearchQuery(kind, body);
  const budgetMs = Math.max(5000, Number(timeoutMs) || 60000);
  const searchSource = {
    ...source,
    BRAVE_SEARCH_TIMEOUT_MS: String(budgetMs)
  };
  let timer = null;
  try {
    return await Promise.race([
      fetchBraveWebSources(kind, body, searchSource),
      new Promise((resolve) => {
        timer = setTimeout(() => {
          resolve({
            enabled: true,
            ...
            sources: [],
            timedOut: true,
            error: new Error(`Brave source collection exceeded workflow budget after ${budgetMs}ms`)
          });
        }, budgetMs);
      })
    ]);
...
function stripHtmlForResearchText(html = '') {
...
function extractHtmlTagTexts(html = '', tagName = 'h1', limit = 8) {
  const texts = [];
  const pattern = new RegExp(`<${tagName}\\b[^>]*>([\\s\\S]*?)<\\/${tagName}>`, 'gi');
  let match = pattern.exec(String(html || ''));
  while (match && texts.length < limit) {
    const text = stripHtmlForResearchText(match[1]);
    if (text) texts.push(clipText(text, 180));
...
function extractHtmlTitleText(html = '') {
  ...
  return match ? clipText(stripHtmlForResearchText(match[1]), 180) : '';
}

function estimateResearchTextMetrics(text = '') {
  const clean = String(text || '').replace(/\s+/g, ' ').trim();
  if (!clean) return { wordCount: 0, cjkCharacterCount: 0 };
  const latinWords = clean.match(/[A-Za-z0-9]+(?:[-'][A-Za-z0-9]+)*/g) || [];
  const cjkChars = clean.match(/[\u3040-\u30ff\u3400-\u9fff]/g) || [];
  const estimatedCjkWords = Math.round(cjkChars.length / 2);
  return {
    wordCount: latinWords.length + estimatedCjkWords,
    cjkCharacterCount: cjkChars.length
  };
}

function researchPageFetchConfig(source = {}) {
  const enabledText = envValue(source, 'WORKFLOW_RESEARCH_PAGE_FETCH_DISABLED').toLowerCase();
  const disabled = ['1', 'true', 'yes', 'on'].includes(enabledText);
  const maxPages = Math.max(0, Math.min(5, parseNumber(envValue(source, 'WORKFLOW_RESEARCH_PAGE_FETCH_MAX'), 3)));
  const timeoutMs = Math.max(2000, Math.min(12000, parseNumber(envValue(source, 'WORKFLOW_RESEARCH_PAGE_FETCH_TIMEOUT_MS'), 3500)));
  return { enabled: !disabled && maxPages > 0, maxPages, timeoutMs };
}

async function fetchResearchPageSignal(source = {}, index = 0, config = {}) {
  const url = normalizeHttpUrl(source?.url);
  if (!url) return { rank: index + 1, url: '', fetched: false, reason: 'missing_url' };
  const controller = new AbortController();
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'CAIt-ResearchAgent/1.0 (+https://aiagent-marketplace.net)'
      }
    });
    const contentType = String(response.headers.get('content-type') || '').toLowerCase();
    if (!response.ok) {
      return { rank: index + 1, url, fetched: false, status: response.status, reason: `http_${response.status}` };
    }
    if (contentType && !/(text\/html|application\/xhtml|text\/plain)/i.test(contentType)) {
      return { rank: index + 1, url, fetched: false, status: response.status, contentType, reason: 'unsupported_content_type' };
    }
    const html = await response.text();
    const text = stripHtmlForResearchText(html);
    const metrics = estimateResearchTextMetrics(text);
    const h1 = extractHtmlTagTexts(html, 'h1', 3);
    const headings = [
      ...extractHtmlTagTexts(html, 'h2', 8),
      ...extractHtmlTagTexts(html, 'h3', 6)
    ].slice(0, 10);
    return {
      rank: index + 1,
      url,
      ...
      title: extractHtmlTitleText(html) || source.title || '',
      wordCount: metrics.wordCount,
      cjkCharacterCount: metrics.cjkCharacterCount,
      h1,
      headings,
      textSample: clipText(text, 900)
    };
  } catch (error) {
    return {
      rank: index + 1,
      url,
      fetched: false,
      reason: String(error?.name || error?.message || error || 'fetch_failed').slice(0, 120)
    };
  } finally {
    clearTimeout(timer);
    try {
...
async function enrichResearchSourcesWithPageSignals(sources = [], source = {}) {
  const config = researchPageFetchConfig(source);
  if (!config.enabled || !Array.isArray(sources) || !sources.length) return [];
  const targets = sources
    .slice(0, config.maxPages)
    .filter((item) => normalizeHttpUrl(item?.url));
  return Promise.all(targets.map((item, index) => fetchResearchPageSignal(item, index, config)));
}
...
function researchStopWords() {
  return new Set([
    'the', 'and', 'for', 'with', 'that', 'this', 'from', 'are', 'you', 'your', 'how', 'what', 'why', 'best', 'top',
    'in', 'on', 'to', 'of', 'a', 'an', 'is', 'or', 'as', 'by', 'it', 'be', 'can', 'use', 'using', 'guide',
    'する', 'できる', 'こと', 'ため', 'とは', 'ます', 'です', 'から', 'よう', 'この', 'その'
  ]);
}

function researchTopTermsFromSources(sources = [], pageSignals = [], limit = 8) {
  const counts = new Map();
  const stop = researchStopWords();
  const text = [
    ...sources.flatMap((source) => [source.title, source.snippet, source.query]),
    ...pageSignals.flatMap((signal) => [signal.title, ...(signal.h1 || []), ...(signal.headings || []), signal.textSample])
  ].join(' ');
  const terms = String(text || '').toLowerCase().match(/[a-z0-9][a-z0-9-]{2,}|[\u3040-\u30ff\u3400-\u9fff]{2,}/gi) || [];
  for (const term of terms) {
    const normalized = String(term || '').toLowerCase();
    if (stop.has(normalized) || normalized.length > 32) continue;
...
function researchIntentFromSource(source = {}, objective = '') {
  const text = [source.title, source.snippet, objective].join(' ').toLowerCase();
  if (/pricing|price|cost|料金|価格/i.test(text)) return 'pricing/commercial evaluation';
  if (/alternative|compare|vs|competitor|比較|代替|競合/i.test(text)) return 'comparison / alternative evaluation';
  if (/how to|guide|tutorial|手順|方法|始め方/i.test(text)) return 'how-to / education';
  if (/review|ranking|best|おすすめ|ランキング/i.test(text)) return 'review / best-of research';
  if (/signup|trial|demo|lead|問い合わせ|登録|トライアル/i.test(text)) return 'conversion proof';
  return 'topic evidence';
}

function researchImplicationForSource(source = {}, signal = {}, objective = '', isJapanese = false) {
  const intent = researchIntentFromSource(source, objective);
  const headings = Array.isArray(signal?.headings) ? signal.headings.slice(0, 3).join(' / ') : '';
  const snippet = source?.snippet || '';
  const basis = headings || snippet || source?.title || '';
  if (isJapanese) {
    if (/comparison|alternative/.test(intent)) return `比較・代替検討の文脈が強い。後続SEO/LPでは「何と違うか」「誰向けか」「移行/試用の負担」を明示する。根拠: ${clipText(basis, 120)}`;
    if (/pricing/.test(intent)) return `価格/導入判断の文脈。費用対効果、無料試用、導入前提をページ内で早めに説明する。根拠: ${clipText(basis, 120)}`;
    if (/how-to/.test(intent)) return `学習・実装意図。手順、利用例、失敗回避をコンテンツ化すると流入後の理解が進む。根拠: ${clipText(basis, 120)}`;
    return `検索結果は ${intent} の証拠として使える。後続はこの語彙を訴求、見出し、CTAの制約にする。根拠: ${clipText(basis, 120)}`;
  }
  if (/comparison|alternative/.test(intent)) return `Comparison/alternative intent is visible; downstream SEO and LP work should explain who this is for, what it replaces, and trial friction. Basis: ${clipText(basis, 120)}`;
  if (/pricing/.test(intent)) return `Commercial/pricing intent is visible; explain cost, ROI, trial path, and adoption assumptions early. Basis: ${clipText(basis, 120)}`;
  if (/how-to/.test(intent)) return `Educational intent is visible; turn steps, examples, and failure avoidance into content. Basis: ${clipText(basis, 120)}`;
  return `Use this as ${intent}; downstream work should reuse its vocabulary in claims, headings, and CTA constraints. Basis: ${clipText(basis, 120)}`;
}

function buildResearchSourceRows(sources = [], pageSignals = [], objective = '', isJapanese = false) {
  return sources.map((source, index) => {
    const url = normalizeHttpUrl(source?.url);
    const signal = pageSignals.find((item) => item.url && item.url === url) || null;
    const wordCount = signal?.fetched && signal.wordCount
      ? `${Math.round(signal.wordCount).toLocaleString('en-US')} words${signal.cjkCharacterCount ? ` / ${signal.cjkCharacterCount.toLocaleString('en-US')} CJK chars` : ''}`
      : (signal?.fetched === false ? `not fetched: ${signal.reason || 'unavailable'}` : 'snippet only');
    const headingSignal = [
      ...(Array.isArray(signal?.h1) ? signal.h1 : []),
      ...(Array.isArray(signal?.headings) ? signal.headings.slice(0, 3) : [])
    ].filter(Boolean).join(' / ') || source?.snippet || source?.title || '';
    return {
      rank: index + 1,
      title: clipText(source?.title || url || source?.query || `Source ${index + 1}`, 160),
      domain: sourceDomainLabel(url),
      url,
      intent: researchIntentFromSource(source, objective),
      wordCount,
      signal: clipText(headingSignal, 260),
      implication: researchImplicationForSource(source, signal, objective, isJapanese)
    };
  });
}

function researchWordCountRange(pageSignals = [], isJapanese = false) {
  const stats = researchWordCountStats(pageSignals);
  if (!stats.counts.length) {
    return isJapanese
      ? '本文取得できたページがないため、文字数/語数は未確認です。'
      : 'No page body was fetched, so word-count range is unverified.';
  }
  return isJapanese
    ? `取得できた上位ページの推定語数は ${stats.min.toLocaleString('en-US')} - ${stats.max.toLocaleString('en-US')} words、平均約 ${stats.avg.toLocaleString('en-US')} words です。`
    : `Fetched top-page estimated word-count range is ${stats.min.toLocaleString('en-US')} - ${stats.max.toLocaleString('en-US')} words, average about ${stats.avg.toLocaleString('en-US')} words.`;
}

function researchWordCountStats(pageSignals = []) {
  const counts = pageSignals
    .filter((item) => item?.fetched && Number(item.wordCount) > 0)
    .map((item) => Number(item.wordCount))
    .sort((left, right) => left - right);
  if (!counts.length) {
    return {
      counts: [],
      fetchedPageCount: pageSignals.filter((item) => item?.fetched).length,
      attemptedPageCount: pageSignals.length,
      min: 0,
      max: 0,
      avg: 0,
      median: 0
  ...
  const avg = Math.round(counts.reduce((sum, item) => sum + item, 0) / counts.length);
...
function researchTaskLensesFromText(text = '') {
  const raw = String(text || '').toLowerCase();
  const lenses = [];
  const add = (lens) => {
    if (!lenses.includes(lens)) lenses.push(lens);
  };
  if (/(seo|serp|organic|keyword|search intent|検索意図|自然検索|検索順位|検索流入|キーワード|記事|コンテンツSEO|h1|h2|h3|landing page|lp\b|ランディング)/i.test(raw)) add('seo_serp');
  if (/(competitor|competition|alternative|compare|vs\.?|teardown|競合|代替|比較|ベンチマーク|他社)/i.test(raw)) add('competitive');
  if (/(sns|social|twitter|x\.com|\bx\b|linkedin|reddit|instagram|threads|community|ソーシャル|投稿|コミュニティ|インスタ|リディット)/i.test(raw)) add('social_channel');
  if (/(ad\b|ads|paid|ppc|campaign|広告|有料|配信|キャンペーン)/i.test(raw)) add('paid_acquisition');
  if (/(lead|signup|sign-up|trial|demo|conversion|contact|inquir|cv\b|登録|トライアル|問い合わせ|リード|コンバージョン|申込)/i.test(raw)) add('conversion');
  return lenses.length ? lenses : ['general_research'];
}

function researchLensLabel(lens = '', isJapanese = false) {
  const labels = {
    seo_serp: isJapanese ? 'SEO/SERP分析' : 'SEO/SERP analysis',
    competitive: isJapanese ? '競合/代替調査' : 'competitive/alternative research',
    social_channel: isJapanese ? 'SNS/コミュニティ調査' : 'social/community research',
    paid_acquisition: isJapanese ? '広告/有料獲得調査' : 'paid acquisition research',
    conversion: isJapanese ? 'CV/リード獲得調査' : 'conversion/lead research',
    general_research: isJapanese ? '一般リサーチ' : 'general research'
  };
  return labels[lens] || lens || labels.general_research;
}

function researchHeadingThemes(pageSignals = [], limit = 8) {
  const counts = new Map();
  for (const heading of pageSignals.flatMap((signal) => [
    ...(Array.isArray(signal?.h1) ? signal.h1 : []),
    ...(Array.isArray(signal?.headings) ? signal.headings : [])
  ])) {
    const normalized = String(heading || '').replace(/\s+/g, ' ').trim();
    if (!normalized || normalized.length > 120) continue;
    const key = normalized.toLowerCase();
    const current = counts.get(key) || { label: normalized, count: 0 };
    current.count += 1;
...
function researchContentPatternDefinitions(isJapanese = false) {
  return [
    {
      id: 'how_to',
      label: isJapanese ? '手順/使い方' : 'how-to / steps',
      pattern: /(how to|getting started|tutorial|guide|setup|install|step|手順|方法|始め方|使い方|導入|設定)/i
    },
    {
      id: 'comparison',
      label: isJapanese ? '比較/代替' : 'comparison / alternatives',
      pattern: /(alternative|compare|comparison|vs\.?|competitor|best|top|ranking|比較|代替|競合|おすすめ|ランキング)/i
    },
    {
      id: 'pricing',
      label: isJapanese ? '価格/導入判断' : 'pricing / buying decision',
      pattern: /(pricing|price|cost|plan|roi|無料|料金|価格|費用|プラン|投資対効果)/i
    },
    {
      id: 'use_case',
      label: isJapanese ? 'ユースケース/事例' : 'use cases / examples',
      pattern: /(use case|example|template|case study|workflow|事例|ユースケース|例|テンプレート|業務)/i
    },
    {
      id: 'trust',
      label: isJapanese ? '信頼/証拠' : 'trust / proof',
      pattern: /(review|testimonial|customer|security|privacy|case study|trusted|導入事例|レビュー|評判|信頼|セキュリティ|実績|証拠)/i
    },
    {
      id: 'integration',
      label: isJapanese ? '連携/技術適合' : 'integration / technical fit',
      pattern: /(integration|api|github|slack|google|analytics|developer|docs|連携|api|開発者|技術|ドキュメント|ga4|search console)/i
    },
    {
      id: 'faq',
      label: isJapanese ? 'FAQ/不安解消' : 'FAQ / objections',
      pattern: /(faq|question|troubleshoot|problem|issue|limit|risk|よくある質問|質問|課題|問題|制約|不安|リスク)/i
    }
  ];
}

function researchContentPatterns(sources = [], pageSignals = [], isJapanese = false) {
  const definitions = researchContentPatternDefinitions(isJapanese);
  const items = [
    ...sources.map((source, index) => ({
      rank: index + 1,
      title: source?.title || source?.url || '',
      text: [source?.title, source?.snippet, source?.query].join(' ')
    })),
  ...
  return definitions
    .map((definition) => {
      const matches = items.filter((item) => definition.pattern.test(String(item.text || '')));
      const examples = matches
        .map((item) => clipText(item.title || String(item.text || ''), 90))
        .filter(Boolean)
        .slice(0, 3);
      return {
        id: definition.id,
...
function researchIntentBreakdown(rows = []) {
  const counts = new Map();
  for (const row of rows) {
    const intent = String(row?.intent || 'topic evidence').trim();
...
function buildTaskAlignedResearchFindings({ lenses = [], rows = [], pageSignals = [], terms = [], wordStats = {}, contentPatterns = [], headingThemes = [], objective = '', isJapanese = false } = {}) {
  const intentBreakdown = researchIntentBreakdown(rows);
  const intentText = intentBreakdown.slice(0, 3).map((item) => `${item.intent} (${item.count})`).join(' / ') || (isJapanese ? '未分類' : 'unclassified');
  const headingText = headingThemes.slice(0, 5).map((item) => item.label).join(' / ') || (isJapanese ? '見出し取得なし' : 'no fetched heading signal');
  const patternText = contentPatterns.slice(0, 4).map((item) => `${item.label} (${item.count})`).join(' / ') || (isJapanese ? 'パターン抽出なし' : 'no dominant pattern detected');
  const termText = terms.slice(0, 6).join(', ') || (isJapanese ? '語彙不足' : 'limited vocabulary signal');
  const wordText = wordStats?.counts?.length
    ? `${wordStats.min.toLocaleString('en-US')}-${wordStats.max.toLocaleString('en-US')} words / avg ${wordStats.avg.toLocaleString('en-US')}`
    : (isJapanese ? '未取得' : 'unverified');
  const findings = [];
  const add = (lens, text) => findings.push({
    lens,
    label: researchLensLabel(lens, isJapanese),
    finding: text
  });
  if (lenses.includes('seo_serp')) {
    add('seo_serp', isJapanese
      ? `SEO上位の読まれ方は「${intentText}」が中心です。取得できた上位ページの深さは ${wordText}、共通見出しは ${headingText}、内容パターンは ${patternText} です。後続SEO/Writerはキーワード案ではなく、勝たせる1ページ、必要セクション、推定本文量、差別化見出し、CTA/FAQまで落としてください。`
      : `The SERP reads primarily as ${intentText}. Fetched top pages show ${wordText}; recurring headings include ${headingText}; content patterns are ${patternText}. Downstream SEO/Writer work should convert this into one page to win, required sections, target depth, differentiated headings, CTA, and FAQ, not just keyword ideas.`);
  }
  if (lenses.includes('competitive')) {
    add('competitive', isJapanese
      ? `競合/代替の論点は「${intentText}」と「${termText}」に寄っています。後続Planningは“どの競合に勝つか”を、機能差ではなく対象ユーザー、導入負担、証拠、試用導線で説明してください。`
      : `Competitive/alternative evidence clusters around ${intentText} and terms such as ${termText}. Planning should explain which alternatives to beat using audience, adoption friction, proof, and trial path, not only feature comparison.`);
  }
  if (lenses.includes('social_channel')) {
    add('social_channel', isJapanese
      ? `SNS/コミュニティでは、検索で見えた語彙「${termText}」と内容パターン「${patternText}」を投稿角度に使うべきです。後続Actionは媒体別に、問題提起、具体例、証拠、CTAを分けた承認可能な投稿案にしてください。`
      : `For social/community work, reuse search vocabulary (${termText}) and content patterns (${patternText}) as the post angle. Action agents should produce approval-ready posts by channel with problem, example, proof, and CTA separated.`);
  }
  if (lenses.includes('paid_acquisition')) {
    add('paid_acquisition', isJapanese
      ? `広告では「${intentText}」の意図に合う訴求が必要です。後続Actionは広告文だけでなく、LP上の受け皿、計測イベント、停止条件を同じ仮説に揃えてください。`
      : `Paid acquisition should match the ${intentText} intent. Action work must align ad copy, landing-page receiving message, measurement event, and stop rule to the same hypothesis.`);
  }
  if (lenses.includes('conversion')) {
    add('conversion', isJapanese
      ? `CV/リード獲得では、上位ページの共通パターン「${patternText}」を安心材料とCTA前の証拠に変換してください。後続Preparationは登録/問い合わせ直前の不安、期待成果、次に起こることを明文化する必要があります。`
      : `For conversion/lead work, turn common top-page patterns (${patternText}) into proof before the CTA. Preparation must spell out objections, expected outcome, and what happens after signup/contact.`);
  }
  if (!findings.length || lenses.includes('general_research')) {
    add('general_research', isJapanese
      ? `リーダーから渡された調査目的「${clipText(objective, 120)}」に対し、検索結果は ${intentText} を示しています。後続はソース一覧ではなく、この意図、語彙、証拠不足を作業制約として使ってください。`
      : `For the leader task "${clipText(objective, 120)}", sources point to ${intentText}. Downstream work should use the intent, vocabulary, and proof gaps as constraints instead of treating this as a source list.`);
  }
  return findings;
}

function researchThreeCText(value = '', fallback = '') {
  return clipText(String(value || fallback || '').replace(/\s+/g, ' ').trim(), 420);
}

function buildResearchThreeCAnalysis({ sources = [], rows = [], terms = [], taskAlignedFindings = [], contentPatterns = [], headingThemes = [], wordStats = {}, objective = '', lenses = [], isJapanese = false } = {}) {
  const topDomains = [...new Set(rows.map((row) => row.domain).filter(Boolean))].slice(0, 5);
  const topSourceTitles = sources
    .map((source) => String(source?.title || source?.url || '').replace(/\s+/g, ' ').trim())
    .filter(Boolean)
    .slice(0, 4);
  const intentText = researchIntentBreakdown(rows).slice(0, 4).map((item) => `${item.intent} (${item.count})`).join(' / ') || (isJapanese ? '未分類' : 'unclassified');
  const termText = terms.slice(0, 8).join(', ') || (isJapanese ? '語彙不足' : 'limited vocabulary signal');
  const headingText = headingThemes.slice(0, 5).map((item) => item.label).join(' / ') || (isJapanese ? '本文見出し未取得' : 'no fetched heading signal');
  const patternText = contentPatterns.slice(0, 5).map((item) => `${item.label} (${item.count})`).join(' / ') || (isJapanese ? '明確な共通パターンなし' : 'no dominant pattern detected');
  const wordText = wordStats?.counts?.length
    ? `${wordStats.min.toLocaleString('en-US')}-${wordStats.max.toLocaleString('en-US')} words / avg ${wordStats.avg.toLocaleString('en-US')}`
    : (isJapanese ? '語数未取得' : 'word count unverified');
  const proofPatternCount = contentPatterns
    .filter((item) => ['trust', 'use_case', 'integration', 'faq'].includes(item.id))
    .reduce((sum, item) => sum + Number(item.count || 0), 0);
  const proofQuality = proofPatternCount >= 6 || (wordStats?.avg || 0) >= 1500
    ? (isJapanese ? '高め' : 'high')
    : proofPatternCount >= 3 || (wordStats?.avg || 0) >= 800
      ? (isJapanese ? '中程度' : 'medium')
      : (isJapanese ? '限定的' : 'limited');
  const socialSourceRows = rows.filter((row) => /(twitter|x\.com|linkedin|reddit|instagram|threads|social|community)/i.test([row.title, row.url, row.signal].join(' ')));
  const socialFinding = socialSourceRows.length
    ? (isJapanese
        ? `SNS/コミュニティの直接ソースは ${socialSourceRows.length} 件あり、投稿角度は ${termText} に寄っています。いいね数などの engagement はソース本文に出た場合だけ使い、未取得なら推測しません。`
        : `${socialSourceRows.length} direct social/community source(s) appeared; post angles cluster around ${termText}. Likes/engagement should only be used when present in fetched source text, not inferred.`)
    : (isJapanese
        ? `SNSの投稿本文やいいね数は今回のBrave結果だけでは直接検証できません。SNS施策へ渡す場合は、競合アカウントURL、投稿URL、反応数を次の収集対象にしてください。`
        : `Direct competitor post text and like counts were not verified from this Brave result set. For social execution, collect competitor account URLs, post URLs, and engagement counts next.`);
  const seoFinding = isJapanese
    ? `SEOでは ${intentText} が主な競争意図です。上位/参考ページは ${wordText}、共通見出しは ${headingText}、E-E-A-T/証拠品質は ${proofQuality} と見ます。後続SEOは「狙うキーワード」だけでなく、必要語数、証拠ブロック、比較表、FAQ、CTA前の不安解消を指定してください。`
    : `For SEO, ${intentText} is the main competitive intent. Reference/top pages show ${wordText}; recurring headings include ${headingText}; E-E-A-T/proof quality appears ${proofQuality}. Downstream SEO should specify target depth, proof blocks, comparison table, FAQ, and pre-CTA objection handling, not just keywords.`;
  const companyText = isJapanese
    ? `Company: リーダーからの目的は「${researchThreeCText(objective, '未指定')}」。自社側の勝ち筋は、検索意図 ${intentText} に対して、競合より具体的な実行手順・証拠・承認安全性・CV導線を示すことです。`
    : `Company: The leader objective is "${researchThreeCText(objective, 'unspecified')}". The company-side path to win is to answer ${intentText} with clearer execution steps, proof, approval safety, and conversion path than alternatives.`;
  const customerText = isJapanese
    ? `Customer: 顧客/検索者は ${termText} 周辺の語彙で、比較・証拠・導入判断を求めています。コンテンツやSNSでは、課題→比較基準→実例/証拠→次アクションの順で不安を潰す必要があります。`
    : `Customer: Searchers use vocabulary around ${termText} and are looking for comparison, proof, and buying/adoption confidence. Content and social should move from pain, to comparison criteria, to proof/example, to next action.`;
  const competitorText = isJapanese
    ? `Competitor: 観測ソースは ${topDomains.join(', ') || topSourceTitles.join(', ') || '限定的'}。競合/上位は ${patternText} を出しており、SEOでは ${seoFinding} SNSでは ${socialFinding}`
    : `Competitor: Observed sources include ${topDomains.join(', ') || topSourceTitles.join(', ') || 'limited source signal'}. Competitors/top pages emphasize ${patternText}. SEO view: ${seoFinding} Social view: ${socialFinding}`;
  const marketText = isJapanese
    ? `Market: 市場/チャネル状況は ${intentText} が中心で、比較・代替・信頼形成の情報密度が重要です。すぐに広告や外部送信へ広げるより、検索意図に合う受け皿と証拠を先に固めるべきです。`
    : `Market: Market/channel evidence centers on ${intentText}; information density around comparison, alternatives, and proof matters. Build the search-intent landing proof before widening into ads or outbound.`;
  return {
    company: companyText,
    ...
    sourceTitles: topSourceTitles,
    taskFindings: taskAlignedFindings
  };
}

function buildResearchDownstreamHandoff({ sources = [], rows = [], terms = [], taskLenses = [], taskAlignedFindings = [], contentPatterns = [], headingThemes = [], wordStats = {}, threeCAnalysis = {}, objective = '', isJapanese = false } = {}) {
  const sourceUrls = sources.map((source) => normalizeHttpUrl(source?.url)).filter(Boolean);
  const sourceDomains = [...new Set(sourceUrls.map(sourceDomainLabel).filter(Boolean))].slice(0, 8);
  const fetchedRows = rows.filter((row) => Number(row.wordCount || 0) > 0);
  const termText = terms.slice(0, 8).join(', ') || (isJapanese ? '語彙シグナル不足' : 'limited vocabulary signal');
  const patternText = contentPatterns.slice(0, 5).map((item) => `${item.label} (${item.count})`).join(' / ') || (isJapanese ? '共通パターン未検出' : 'no common pattern detected');
  const headingText = headingThemes.slice(0, 6).map((item) => item.label).join(' / ') || (isJapanese ? '見出し取得不足' : 'headings not sufficiently fetched');
  const sourceStatus = {
    provider: 'brave',
    source_count: sources.length,
    fetched_page_count: fetchedRows.length,
    domains: sourceDomains,
    observation_date: nowIso().slice(0, 10),
    limitations: [
      ...(sources.length ? [] : [isJapanese ? 'Brave検索ソースがありません。' : 'No Brave search sources were available.']),
      ...(fetchedRows.length ? [] : [isJapanese ? '本文取得に成功したページがないため、語数/見出しはsnippet中心です。' : 'No fetched page bodies succeeded, so word-count/headings are snippet-led.']),
      ...(rows.some((row) => /social|twitter|x\.com|reddit|linkedin|instagram/i.test([row.title, row.url, row.signal].join(' '))) ? [] : [isJapanese ? 'SNS投稿本文・反応数は未検証です。' : 'Social post text and engagement counts are unverified.'])
    ]
  };
  const evidenceGaps = [
    ...(fetchedRows.length ? [] : [{
      id: 'page_body_depth',
      severity: 'medium',
      gap: isJapanese ? '上位ページ本文の取得が不足' : 'Top-page body depth is incomplete',
      next_check: isJapanese ? '上位3ページの本文、H1/H2/H3、語数、FAQ、証拠ブロックを再取得する' : 'Fetch the top 3 page bodies, H1/H2/H3, word count, FAQ, and proof blocks'
    }]),
    ...(taskLenses.includes('social_channel') ? [{
      id: 'social_engagement',
      severity: 'medium',
      gap: isJapanese ? '競合SNS投稿の本文・いいね/反応数が未取得' : 'Competitor social post text and likes/engagement are missing',
      next_check: isJapanese ? '競合アカウントURL、投稿URL、反応数、投稿日時を収集する' : 'Collect competitor account URLs, post URLs, engagement counts, and post dates'
    }] : []),
    ...(taskLenses.includes('paid_acquisition') ? [{
      id: 'ad_benchmark',
      severity: 'low',
      gap: isJapanese ? '広告訴求と停止条件の実測根拠が不足' : 'Ad-message benchmark and stop-rule evidence are missing',
      next_check: isJapanese ? 'LP計測イベント、CPC上限、停止条件、1仮説だけの広告文を確定する' : 'Confirm LP measurement events, CPC cap, stop rule, and one-hypothesis ad copy'
    }] : [])
  ];
  const channelRequirements = {
    planning: {
      use: isJapanese
        ? `意図/語彙: ${termText}。媒体選定は一般論ではなく、この語彙と${patternText}に合わせる。`
        : `Intent/vocabulary: ${termText}. Choose channels from this vocabulary and ${patternText}, not a generic channel list.`,
      avoid: isJapanese ? '未検証の媒体反応や市場規模を断定しない。' : 'Do not claim unverified channel response or market size.'
    },
    seo: {
      use: isJapanese
        ? `見出し: ${headingText}。語数/構成/証拠品質を基準に、勝つ1ページと補助ページを指定する。`
        : `Headings: ${headingText}. Use word count, structure, and proof quality to specify one winning page plus one support page.`,
      required_artifacts: ['keyword/search-intent cluster', 'H1/H2/H3 pattern', 'word-count range', 'E-E-A-T/proof block', 'CTA and internal-link path']
    },
    social: {
      use: threeCAnalysis.social || '',
      required_artifacts: ['channel-native angle', 'exact draft copy', 'source-backed claim list', 'approval checklist'],
      evidence_rule: isJapanese
        ? 'いいね数/反応数はソースにある場合だけ使う。未取得なら収集対象にする。'
        : 'Use likes/engagement only when source-verified; otherwise make them a collection target.'
    },
    paid: {
      use: isJapanese
        ? '広告はLP/CTA/計測/停止条件が同じ仮説に揃った後だけ小額検証にする。'
        : 'Paid should be a small validation only after LP, CTA, measurement, and stop rule share one hypothesis.',
      required_artifacts: ['one hypothesis', 'ad copy', 'landing message', 'measurement event', 'stop rule']
    },
    writing: {
      use: isJapanese
        ? `Customer/Competitorの差分を、課題 -> 比較基準 -> 証拠 -> CTA の順で本文化する。`
        : 'Turn the Customer/Competitor gap into copy in the order: problem -> comparison criteria -> proof -> CTA.',
      avoid: isJapanese ? '検索ソースにない実績、導入数、顧客名を作らない。' : 'Do not invent customer names, adoption counts, or proof absent from sources.'
    },
    action: {
      use: isJapanese
        ? '外部投稿/送信/PR/repository writeは、文面・アカウント・URL・停止条件を承認packetに分離する。'
        : 'Separate copy, account, URL, and stop rule into an approval packet before any external post/send/PR/repository write.',
  ...
  const agentHandoff = [
    {
      agent: 'Media Planner',
      must_use: [channelRequirements.planning.use, threeCAnalysis.market || ''].filter(Boolean),
      must_return: ['priority lane', 'why this channel now', 'metric', 'stop rule']
    },
    {
      agent: 'SEO / Writer',
      must_use: [channelRequirements.seo.use, channelRequirements.writing.use].filter(Boolean),
      must_return: ['page target', 'outline or copy', 'proof requirements', 'CTA path']
    },
    {
      agent: 'Social / Action',
      must_use: [channelRequirements.social.use, channelRequirements.action.use].filter(Boolean),
      must_return: ['exact copy', 'source-backed claims', 'approval checklist', 'connector/manual handoff']
    }
  ];
  return {
    objective: clipText(objective, 800),
    source_status: sourceStatus,
    channel_requirements: channelRequirements,
    evidence_gaps: evidenceGaps,
    agent_handoff: agentHandoff,
    approval_boundary: channelRequirements.action
  };
}

function researchDownstreamHandoffMarkdown(handoff = {}, isJapanese = false) {
  const requirements = handoff.channel_requirements || {};
  const evidenceGaps = Array.isArray(handoff.evidence_gaps) ? handoff.evidence_gaps : [];
  const agents = Array.isArray(handoff.agent_handoff) ? handoff.agent_handoff : [];
  return [
    isJapanese ? '## 下流エージェント用handoff packet' : '## Downstream agent handoff packet',
    `- ${isJapanese ? 'Source status' : 'Source status'}: ${handoff.source_status?.source_count || 0} Brave source(s), ${handoff.source_status?.fetched_page_count || 0} fetched page(s), ${handoff.source_status?.domains?.join(', ') || 'no domains'}`,
    `- ${isJapanese ? 'Planning' : 'Planning'}: ${requirements.planning?.use || ''}`,
    `- SEO: ${requirements.seo?.use || ''}`,
    `- SNS/Social: ${requirements.social?.evidence_rule || requirements.social?.use || ''}`,
    `- Paid: ${requirements.paid?.use || ''}`,
    `- ${isJapanese ? 'Writing' : 'Writing'}: ${requirements.writing?.use || ''}`,
    `- ${isJapanese ? 'Approval boundary' : 'Approval boundary'}: ${requirements.action?.use || ''}`,
    '',
    isJapanese ? '### 証拠不足 / 次に集めるもの' : '### Evidence gaps / next collection',
    ...(evidenceGaps.length
      ? evidenceGaps.map((gap) => `- **${gap.id} (${gap.severity})**: ${gap.gap} -> ${gap.next_check}`)
      : ['- No high-priority evidence gap beyond normal source verification.']),
    '',
    isJapanese ? '### Agent handoff' : '### Agent handoff',
    ...(agents.length
      ? agents.map((item) => `- **${item.agent}**: must use ${markdownTableCell((item.must_use || []).join(' / '), 260)}; must return ${(item.must_return || []).join(', ')}`)
      : ['- No downstream agent handoff generated.'])
  ].join('\n');
}

function buildResearchSynthesis({ sources = [], pageSignals = [], objective = '', query = '', taskFrame = '', isJapanese = false } = {}) {
  const rows = buildResearchSourceRows(sources, pageSignals, objective, isJapanese);
  const terms = researchTopTermsFromSources(sources, pageSignals, 8);
  const fetchedCount = pageSignals.filter((item) => item?.fetched).length;
  const attemptedFetchCount = pageSignals.length;
  const wordRange = researchWordCountRange(pageSignals, isJapanese);
  const wordStats = researchWordCountStats(pageSignals);
  const taskLenses = researchTaskLensesFromText([taskFrame, objective, query].join(' '));
  const headingThemes = researchHeadingThemes(pageSignals, 8);
  const contentPatterns = researchContentPatterns(sources, pageSignals, isJapanese);
  const taskAlignedFindings = buildTaskAlignedResearchFindings({
    lenses: taskLenses,
    rows,
    pageSignals,
    terms,
  ...
  const threeCAnalysis = buildResearchThreeCAnalysis({
    sources,
    rows,
    terms,
    taskAlignedFindings,
  ...
  const topIntents = [...new Set(rows.map((row) => row.intent).filter(Boolean))].slice(0, 4);
  const downstreamHandoff = buildResearchDownstreamHandoff({
    sources,
    rows,
    terms,
    taskLenses,
  ...
  const bullets = isJapanese
    ? [
        `リーダーから渡された調査目的に合わせ、Brave Search ${sources.length} 件を「検索結果」ではなくリサーチ結論に再整理しました。上位 ${attemptedFetchCount} 件の本文取得を試行して ${fetchedCount} 件成功しました。`,
        `3C分析: ${threeCAnalysis.customer.slice(0, 180)} / ${threeCAnalysis.competitor.slice(0, 180)}`,
        wordRange,
        `タスク別の主な結論: ${taskAlignedFindings.map((item) => `${item.label}: ${item.finding}`).join(' / ').slice(0, 520)}`,
        `多く出るテーマ/語彙: ${terms.join(', ') || 'snippet不足のため限定的'}`,
        `検索意図は ${topIntents.join(' / ') || '未分類'} が中心。後続のPlanning/SEO/Preparationはこの意図を見出し、CTA、証拠条件に変換してください。`
      ]
    : [
        `For the leader task, ${sources.length} Brave Search source(s) were converted into research conclusions rather than returned as raw search results; page-body fetch succeeded for ${fetchedCount} of ${attemptedFetchCount} attempted top results.`,
        `3C analysis: ${threeCAnalysis.customer.slice(0, 180)} / ${threeCAnalysis.competitor.slice(0, 180)}`,
        wordRange,
        `Task-aligned findings: ${taskAlignedFindings.map((item) => `${item.label}: ${item.finding}`).join(' / ').slice(0, 520)}`,
        `Recurring terms/themes: ${terms.join(', ') || 'limited because snippets were sparse'}.`,
        `Visible search intent clusters: ${topIntents.join(' / ') || 'unclassified'}. Planning, SEO, and preparation should convert these into headings, CTA, and proof requirements.`
      ];
  const table = rows.map((row) => `| ${row.rank} | ${markdownTableCell(row.title || row.domain, 120)} | ${markdownTableCell(row.intent, 90)} | ${markdownTableCell(row.wordCount, 70)} | ${markdownTableCell(row.signal, 180)} | ${markdownTableCell(row.implication, 220)} |`);
  return {
    rows,
    terms,
    fetchedCount,
    ...
    bullets,
    markdown: [
      isJapanese ? '## リサーチ要約' : '## Research synthesis',
      ...bullets.map((item) => `- ${item}`),
      '',
      isJapanese ? '## 3C分析' : '## 3C analysis',
      `- **Company**: ${threeCAnalysis.company}`,
      `- **Customer**: ${threeCAnalysis.customer}`,
      `- **Competitor**: ${threeCAnalysis.competitor}`,
      `- **Market**: ${threeCAnalysis.market}`,
      '',
      isJapanese ? '## 指示別の見解' : '## Channel-specific interpretation',
      `- **SEO**: ${threeCAnalysis.seo}`,
      `- **SNS/Social**: ${threeCAnalysis.social}`,
      '',
      isJapanese ? '## タスク別リサーチ結論' : '## Task-aligned research interpretation',
      ...(taskAlignedFindings.length
        ? taskAlignedFindings.map((item) => `- **${item.label}**: ${item.finding}`)
        : ['- No task-specific finding could be derived from the available sources.']),
      '',
      researchDownstreamHandoffMarkdown(downstreamHandoff, isJapanese),
      '',
      isJapanese ? '## 上位コンテンツの共通パターン' : '## Top-content patterns',
      `- ${isJapanese ? '主要レンズ' : 'Primary lens'}: ${taskLenses.map((lens) => researchLensLabel(lens, isJapanese)).join(' / ')}`,
      `- ${isJapanese ? '共通見出し' : 'Common headings'}: ${headingThemes.map((item) => item.label).join(' / ') || (isJapanese ? '本文取得不足' : 'not enough fetched headings')}`,
      `- ${isJapanese ? '共通内容パターン' : 'Common content patterns'}: ${contentPatterns.map((item) => `${item.label} (${item.count})`).join(' / ') || (isJapanese ? '抽出なし' : 'none detected')}`,
      '',
      isJapanese ? '## SERP / source readout' : '## SERP / source readout',
      '| Rank | Source | Intent | Word count | Observed content signal | Research implication |',
      '| --- | --- | --- | --- | --- | --- |',
      ...(table.length ? table : ['| - | No source | - | - | - | - |']),
      '',
      isJapanese ? '## 後続エージェントへの制約' : '## Constraints for downstream agents',
      ...(isJapanese
        ? [
            '- Planning: 媒体選定は検索意図と実際の見出し/語彙に合わせる。',
            '- SEO/Writer: 上位ページの語数、見出し、証拠の出し方を基準に、勝てる差分を本文に入れる。',
            '- Action: 外部投稿/掲載/送信は、この調査で見えた訴求とCTAを使った承認可能packetにする。'
          ]
        : [
            '- Planning: choose channels from the observed intent and vocabulary, not from a generic channel list.',
            '- SEO/Writer: use top-page word count, headings, and proof style as constraints, then add a differentiated response.',
            '- Action: approval-ready packets should reuse the claims, CTA, and proof gaps found here.'
          ])
    ].join('\n'),
    structured: {
      query,
      ...
      wordCountStats: wordStats,
      commonThemes: terms,
      taskLenses,
      three_c_analysis: threeCAnalysis,
      downstream_handoff: downstreamHandoff,
      channel_requirements: downstreamHandoff.channel_requirements,
      evidence_gaps: downstreamHandoff.evidence_gaps,
      task_aligned_findings: taskAlignedFindings,
      headingThemes,
      contentPatterns,
      intentBreakdown: researchIntentBreakdown(rows),
      rows
    }
  };
}

function workflowResearchOpenAiSynthesisTimeoutMs(source = {}) {
  const configured = parseNumber(
    envValue(source, 'BUILTIN_OPENAI_RESEARCH_SYNTHESIS_TIMEOUT_MS')
      || envValue(source, 'WORKFLOW_RESEARCH_OPENAI_TIMEOUT_MS')
      || envValue(source, 'BUILTIN_OPENAI_WORKFLOW_TIMEOUT_MS'),
    60000
  );
  return Math.max(10000, Math.min(120000, configured));
}

function research3COpenAiSystemPrompt(kind = '', body = {}, isJapanese = false) {
  return [
    builtInAgentDefinitionForKind(kind).systemPrompt,
    currentDateInstruction(),
    `Write every user-facing field in ${isJapanese ? 'Japanese' : deliveryLanguageInstruction(body)} unless source names or exact terms require English.`,
    'You are the research specialist in a leader-owned workflow. The leader owns handoff and final synthesis; your job is to turn Brave source evidence into research judgment that downstream planning, SEO, SNS, writing, and action agents can use.',
    'Process: use the leader handoff and objective to understand the product, customer, goal, channels, constraints, and missing proof; use the supplied Brave sources and fetched page signals as the only current-source evidence; then synthesize a 3C analysis.',
    'Required output: include clear 3C analysis sections for Company, Customer, Competitor, plus Market/channel situation. Do not return a raw search result list as the main answer.',
    'For SEO instructions: identify the competitive keyword/search-intent cluster, what top pages/articles are doing, approximate word-count range, headings/content patterns, E-E-A-T/proof quality, and the gap downstream SEO/writing should exploit.',
    'For SNS/social instructions: identify competitor/social/community posting angles from supplied sources. Only mention likes, engagement, or high-performing posts when the source evidence contains that data; otherwise state that engagement data still needs social-platform collection.',
    'For generic market/competitive requests: research competitors, alternatives, market situation, customer pain, positioning gap, and recommended next evidence to collect.',
    'Cite source titles or URLs in file_markdown. Separate verified source evidence from inference. Keep external posting, sending, PR creation, and repository writes approval-gated.',
    'Return only the JSON schema.'
  ].join(' ');
}

function compactResearchSourceForOpenAi(source = {}, index = 0) {
  const signal = source?.page_signal && typeof source.page_signal === 'object' ? source.page_signal : {};
  return {
    rank: index + 1,
    title: clipText(source?.title || source?.url || `Source ${index + 1}`, 180),
    url: normalizeHttpUrl(source?.url) || '',
    snippet: clipText(source?.snippet || '', 520),
    query: clipText(source?.query || '', 180),
    page_signal: {
      fetched: Boolean(signal.fetched),
      word_count: Number(signal.word_count || signal.wordCount || 0) || 0,
      cjk_character_count: Number(signal.cjk_character_count || signal.cjkCharacterCount || 0) || 0,
      h1: Array.isArray(signal.h1) ? signal.h1.slice(0, 3).map((item) => clipText(item, 140)) : [],
      headings: Array.isArray(signal.headings) ? signal.headings.slice(0, 10).map((item) => clipText(item, 140)) : [],
      reason: clipText(signal.reason || '', 140)
    }
  };
}

function research3COpenAiPayload(kind = '', body = {}, { sources = [], query = '', objective = '', taskFrame = '', synthesis = {} } = {}) {
  const workflow = workflowContext(body) || {};
  const leaderHandoff = workflow.leaderHandoff && typeof workflow.leaderHandoff === 'object'
    ? workflow.leaderHandoff
    : {};
  return {
    request: withWebSourcesInRequest(kind, body, sources),
    leader_handoff: {
      owner: leaderHandoff.handoffOwner || leaderHandoff.owner || 'leader',
      summary: clipText(leaderHandoff.summary || leaderHandoff.leaderSummary || workflow.objective || '', 900),
      prior_runs: Array.isArray(leaderHandoff.priorRuns)
        ? leaderHandoff.priorRuns.slice(0, 8).map((run) => ({
            task_type: run.taskType || run.task_type || '',
            status: run.status || '',
            summary: clipText(run.summary || run.outputSummary || '', 260)
          }))
        : []
    },
    research_brief: {
      kind,
      objective,
      task_frame: clipText(taskFrame, 1000),
      brave_query: query,
      observation_date: nowIso().slice(0, 10)
    },
    brave_sources: sources.map(compactResearchSourceForOpenAi),
    deterministic_source_synthesis: {
      terms: synthesis.terms || synthesis.commonThemes || [],
      task_lenses: synthesis.taskLenses || [],
      three_c_analysis: synthesis.structured?.three_c_analysis || synthesis.threeCAnalysis || null,
      task_aligned_findings: synthesis.taskAlignedFindings || synthesis.structured?.task_aligned_findings || [],
      downstream_handoff: synthesis.structured?.downstream_handoff || null,
      evidence_gaps: synthesis.structured?.evidence_gaps || [],
      content_patterns: synthesis.contentPatterns || synthesis.structured?.contentPatterns || [],
      heading_themes: synthesis.headingThemes || synthesis.structured?.headingThemes || [],
      intent_breakdown: synthesis.structured?.intentBreakdown || []
    },
    required_analysis: [
      '3C analysis: Company, Customer, Competitor, plus market/channel situation.',
      'SEO-specific view when SEO/search intent is present: keyword/search-intent competition, top article structure, word count, E-E-A-T/proof quality, gap.',
      'SNS-specific view when social is present: competitor/community posting angles and engagement only if source-verified.',
      'Actionable handoff constraints for downstream planning, SEO, SNS, writing, and action agents.'
    ]
  };
}

function ensureResearch3CMarkdown(markdown = '', synthesis = {}, sources = [], isJapanese = false) {
  const text = String(markdown || '').trim();
  const has3C = /3C分析|3C analysis|Company|Customer|Competitor/i.test(text);
  const sourceTail = [
    '',
    isJapanese ? '## Braveソース由来の補助分析' : '## Brave-source baseline',
    synthesis.markdown || '',
    '',
    isJapanese ? '## Source URLs' : '## Source URLs',
    ...(sources.length ? sources.map((source, index) => `- ${index + 1}. ${source.title || source.url || 'Source'}: ${source.url || source.query || ''}`) : ['- No source URL available.'])
  ].join('\n').trim();
  if (has3C) return `${text}\n\n${sourceTail}`.trim();
  const threeC = synthesis.structured?.three_c_analysis || {};
  const prefix = [
    isJapanese ? '# 3Cリサーチ分析' : '# 3C research analysis',
    '',
    isJapanese ? '## 3C分析' : '## 3C analysis',
    `- **Company**: ${threeC.company || ''}`,
    `- **Customer**: ${threeC.customer || ''}`,
    `- **Competitor**: ${threeC.competitor || ''}`,
    `- **Market**: ${threeC.market || ''}`,
    '',
    isJapanese ? '## 指示別の見解' : '## Channel-specific interpretation',
    `- **SEO**: ${threeC.seo || ''}`,
    `- **SNS/Social**: ${threeC.social || ''}`
  ].join('\n');
  return [prefix, text, sourceTail].filter(Boolean).join('\n\n').trim();
}

async function openAiResearch3CSynthesis(kind = '', body = {}, source = {}, options = {}) {
  const config = openAiConfig(source);
  if (!config.apiKey) return { ok: false, skipped: 'missing_openai_api_key' };
  const isJapanese = builtInDeliveryLanguage(body) === 'ja';
  const routing = builtInModelRoutingForKind(config, 'research', body);
  const startedAt = Date.now();
  try {
    const result = await callOpenAiStructured(config, {
      model: routing.model,
      systemPrompt: research3COpenAiSystemPrompt(kind, body, isJapanese),
      schemaName: 'aiagent2_research_3c_synthesis',
      schema: BUILT_IN_RESULT_SCHEMA,
      payload: research3COpenAiPayload(kind, body, options),
      timeoutMs: workflowResearchOpenAiSynthesisTimeoutMs(source)
    });
    ...
    return {
      ...
      apiCost: estimateApiCost(config, usage, routing),
      pricing: priceForRouting(config, routing)
    };
  } catch (error) {
    return {
      ok: false,
...
function leaderActionProtocolInstruction(body = {}, kind = '') {
  ...
  if (isLeader && phase === 'final_summary') {
    return `Leader final summary phase: synthesize the completed research and supporting work products into one accountable delivery. Do not reopen planning or create new lanes.${priorInstruction} Follow input._broker.workflow.leaderActionProtocol version ${String(protocol.version || 'v1')}. The final report must summarize the actual content of each prior deliverable, not merely list agent names, statuses, or file names. Separate evidence, recommendations, unresolved risks, immediate next actions, approval-to-execution requirements, and a Delivered content summaries table.`;
  }
  if (isLeader) {
...
function leaderControlContractInstruction(kind = '', body = {}) {
  ...
  return [
    ...
    `Handoff owner=${contract.handoffOwner || 'leader'}; orchestration only preserves sequence and quality gates, while the leader receives, passes, reviews, and synthesizes the information.`,
...
function agentSpecificDeliveryStyleInstruction(kind = '', body = {}) {
  const defaults = builtInAgentDefinitionForKind(kind);
  const role = String(defaults.modelRole || kind || 'specialist').replace(/_/g, ' ');
  const workflowPhase = String(body?.input?._broker?.workflow?.sequencePhase || '').trim().toLowerCase();
  const phaseText = workflowPhase ? ` for the ${workflowPhase} phase` : '';
  return `Agent-specific delivery style: the primary file_markdown must read like a ${role}${phaseText} produced a real deliverable for this exact user, not like a platform QA template. Make the agent's domain artifact, decision, packet, draft, table, or analysis the visible center of the delivery. Do not use common policy headings such as Output contract, Professional preflight, Confidence rubric, Minimum blocker questions, Trust and quality assurance, Agent QA appendix, or Quality checks. The only quality check is whether the proposal information has originality grounded in this user's context and this agent's expertise.`;
}
...
function sensitiveDataPolicyInstruction(kind = '') {
  return `Sensitive data policy: ${kindSensitiveDataPolicy(kind)} Redact secrets, credentials, personal data, and unrelated private context from user-facing output unless the user explicitly needs a safe redaction label or aggregated summary.`;
}
...
function trustAssuranceInstruction(kind = '') {
  return 'Originality check only: do not add a visible trust/QA section, score, rubric, or policy structure to file_markdown. Before finalizing, check whether the proposed information is specific to this user, product, source context, and agent role. If a recommendation could be reused unchanged by another agent for another customer, replace it with a more original, context-grounded proposal or mark the missing source/input that prevents originality.';
}
...
function deliveryQualityInstruction(kind = '', body = {}) {
  ...
  const workflowPhase = String(body?.input?._broker?.workflow?.sequencePhase || '').trim().toLowerCase();
  const lines = [
    'Quality means originality of proposal information, not a structured QA checklist.',
    'The visible deliverable should be the agent-specific proposal/artifact itself. Do not add generic quality sections, trust sections, acceptance-check sections, or rubric appendices.',
    'Self-check privately: each main recommendation must depend on at least one concrete user input, source fact, product detail, constraint, prior handoff, or role-specific judgment. Replace generic advice with specific proposal information.',
    'When context is missing, write the smallest useful original proposal with explicit assumptions instead of broad boilerplate.'
  ];
  if (workflowPhase === 'research') {
    lines.push('Research-stage originality: conclusions must be tied to searched or supplied source content, not generic market commentary.');
  }
  if (workflowPhase === 'planning') {
    lines.push('Planning-stage originality: use the facts handed off from research or earlier work to make a distinct decision, not another generic plan.');
  }
  if (['preparation', 'action', 'implementation'].includes(workflowPhase)) {
    lines.push('Execution-stage originality: create concrete creative, copy, code, submission, send, publish, or connector-ready artifacts that reflect prior inputs. Do not finish with strategy-only notes.');
  }
  ...
  if (normalizedKind === 'prompt_brushup') {
    lines.push('Because this is prompt brush-up, originality means a better order brief shaped from the actual request, not completion of the underlying task.');
  }
  if (normalizedKind === 'code' || CODE_MODEL_KINDS.has(normalizedKind)) {
    lines.push('For engineering work, the proposal must name the concrete subsystem, risk, file/module boundary, test, or rollout concern implied by the request; avoid generic engineering process advice.');
  }
...
function shouldUseWebSearchForKind(kind, body = {}) {
  ...
  if (
    ...
    const phase = String(workflow.sequencePhase || '').trim().toLowerCase();
    const forced = workflowSearchRequiredForKind(normalizedKind, body);
    if (forced) return true;
    if (phase === 'research') {
      return ['research', 'teardown', 'competitor_teardown', 'validation', 'diligence'].includes(normalizedKind)
        || promptNeedsCurrentSources(body);
    }
    return false;
  }
  const mode = researchPromptMode(normalizedKind, body);
...
function webSearchSourceInstruction(kind = '', body = {}) {
  ...
  const workflow = body?.input?._broker?.workflow;
  const forced = workflowSearchRequiredForKind(kind, body);
...
- Scope: Do not execute the original task; turn it into a better dispatchable order brief
...
- Missing proof: [label missing proof instead of inventing it]
...
- 不明な相手情報、空き時間、約束事項は未確認として分け、勝手に補完しません。
...
- Unknown recipient context, availability, commitments, owners, and deadlines stay explicitly unresolved instead of being invented.
  ...
  return {
    ...
    bullets: isJapanese
      ? [
        ...
        '未接続データは未確認として表示し、未確定欄や空欄を納品しない',
      ...
      : [
        ...
        'Missing data is labeled as unconnected or unverified; unresolved fields are not accepted as delivery.',
...
function cmoWorkflowPhaseDelivery(fallbackPrompt = '', body = {}, isJapanese = false, context = cmoContextFromBody(body, fallbackPrompt)) {
  ...
  const snippetMarkdown = snippets.length
    ...
    : '_No specialist file snippets were attached to the leader handoff. Use the visible delivery summary and action files first._';
  const executionLabel = chosenTask || 'growth';
  const actionFileNames = snippets.map((item) => item.name).filter(Boolean).join(', ') || 'visible delivery action packet';
  ...
  const nextAction = isJapanese
    ? `DELIVERYで表示される要約と ${actionFileNames} を確認し、${executionLabel} の承認済みpacketを確認して、必要なconnectorを接続して実行してください。`
    : `Review the visible DELIVERY summary and ${actionFileNames}, confirm the ${executionLabel} packet, connect any required connector, then execute the approved action.`;
...
- 最終納品で確認すべき本文は、表示されているdelivery要約と各 action file です。
...
## 次に実行すること
1. DELIVERYで表示される要約と \`${actionFileNames}\` を確認する。
...
- The concrete work products to inspect are the visible delivery summary and the action files listed above.
...
## Execute next
1. Review the visible DELIVERY summary and \`${actionFileNames}\`.
...
## Media-fit analysis
| Medium | Decision | Readiness score basis | Missing asset | Next specialist |
| --- | --- | --- | --- | --- |
| SEO / search landing page | execute_now or prepare_first | intent, conversion proximity, measurement clarityが高い | H1, proof, CTA, FAQ | seo_gap / landing |
| Industry/product directories | prepare_first | カテゴリ検索・比較用途がある商材なら有効 | listing copy, screenshots, category fields | directory_submission |
| X / LinkedIn / community | ask_user_before_action | 反応語彙と初期流入向きだがaccount文脈と規約確認が必要 | short proof, exact copy, rule check | x_post / writer |
| Reddit / niche forums | ask_user_before_action | 宣伝ではなく議論になる時だけ有効 | useful question, subreddit rule check | reddit / writer |
| Email / CRM | avoid_now unless consented | consent済み接点がある時だけ使う | segment, sender, suppression | email_ops / cold_email |
| Local citation / GBP | prepare_first | 地域商圏なら重要 | canonical business facts | citation_ops |

## Readiness scorecard
| Criterion | Check | Decision impact |
| --- | --- | --- |
| audience intent | 今すぐ比較/検索/相談している場所か | 低意図チャネルは後回し |
| proof and assets | 画面、価格、手順、実績、FAQがあるか | 不足時はwriting/landingへ |
| account and connector | 投稿/送信/掲載に使うアカウントがあるか | 実行前にleader approvalへ |
| policy risk | 規約、宣伝色、spam riskがあるか | rule checkとstop ruleを必須化 |
| measurement clarity | UTMやCV eventで判定できるか | GA4なしなら手動statusで代替 |
...
- 上位媒体をそのまま承認するか、1媒体だけ先に実行するか
- 投稿/掲載/送信に使うアカウント、URL、exact copy、UTM、停止条件
- データ未接続なら簡易UTMと手動statusで進めるか
...
| Medium | Decision | Readiness score basis | Missing asset | Next specialist |
| --- | --- | --- | --- | --- |
| SEO / search landing page | execute_now or prepare_first | high intent, close to conversion, clear measurement | H1, proof, CTA, FAQ | seo_gap / landing |
| Industry/product directories | prepare_first | useful when the category has active comparison or discovery behavior | listing copy, screenshots, category fields | directory_submission |
| X / LinkedIn / community | ask_user_before_action | good for response language, but account context and rules matter | short proof, exact copy, rule check | x_post / writer |
| Reddit / niche forums | ask_user_before_action | only works when the angle is useful and discussion-first | question, subreddit rule check | reddit / writer |
| Email / CRM | avoid_now unless consented | only when a consented segment already exists | segment, sender, suppression | email_ops / cold_email |
| Local citation / GBP | prepare_first | important when discovery is location-driven | canonical business facts | citation_ops |

## Readiness scorecard
| Criterion | Check | Decision impact |
| --- | --- | --- |
| audience intent | Are buyers already comparing, searching, or deciding there? | Low-intent channels wait. |
| proof and assets | Are screenshots, pricing, workflow, examples, or FAQs ready? | Missing proof routes to writing or landing first. |
| account and connector | Is the right account or connector ready for posting, sending, or submission? | External writes wait for leader approval. |
| policy risk | Does the surface have promotion, spam, or disclosure constraints? | Require rule checks and stop rules. |
| measurement clarity | Can UTM, source, status, or conversion events prove signal quality? | Without analytics, use manual status and UTM fallback. |
...
## Ask user before action
- Approve the top media queue as a group or pick one first lane.
- Confirm the account, destination URL, exact copy, UTM, and stop rule for any post, send, or submission.
- If analytics are not connected, approve simple UTM plus manual status tracking.
  ...
  const map = {
    ...
    writer: {
      ...
      report: {
        ...
        bullets: [
          ...
          isJapanese ? '未確認の数字や testimonial は未確認項目として分けて誇張しない' : 'Label missing proof clearly instead of inventing numbers or testimonials.'
        ],
    ...
    media_planner: {
      ...
      report: {
        ...
        bullets: [
          isJapanese ? 'URL、業種、ICP、地域、証拠、アカウント状態から媒体taxonomyを作る' : 'Use the site URL, business type, ICP, geography, proof, and account readiness to build the media taxonomy.',
          isJapanese ? '媒体ごとに audience intent / CV距離 / 素材 / connector / 規約risk / 計測性でscoreする' : 'Score each medium by audience intent, conversion proximity, assets, connector readiness, policy risk, and measurement clarity.',
          isJapanese ? 'execute_now / prepare_first / avoid_now / ask_user_before_action に分けて次のagentへ渡す' : 'Split the queue into execute_now, prepare_first, avoid_now, and ask_user_before_action before specialist handoff.'
        ],
        nextAction: isJapanese ? '上位媒体と実行前確認事項をleader承認に回し、承認後に準備agentまたはaction agentへ渡してください。' : 'Route the top media and pre-action questions to leader approval, then hand approved lanes to preparation or action specialists.'
      },
...
List import, sending, scheduling, and replying only happen after explicit connector approval. In a leader workflow, always return the Leader handoff packet for mediation first.`
    ...
    reply_draft: {
      ...
      report: {
        ...
        bullets: [
          isJapanese ? '本文、目的、トーン、未確認項目、送信ガードを分離' : 'Separated message body, outcome, tone, unresolved facts, and send guardrail.',
          isJapanese ? '未確認の約束や日時は補完しない' : 'Unknown commitments or times remain explicitly unresolved.',
    ...
    meeting_notes: {
      ...
      report: {
        ...
        bullets: [
          ...
          isJapanese ? '不明な担当/期限は未確認として保持' : 'Unknown owners or deadlines stay explicitly unresolved.',
...
function buildPlanSystemPrompt(kind, body = {}) {
  ...
  return [
    ...
    agentSpecificDeliveryStyleInstruction(kind, body),
...
function workflowAppContexts(body = {}) {
  ...
  const collect = (value) => {
    ...
    if (typeof value === 'object') contexts.push(value);
  };
  collect(broker.appContexts);
  collect(broker.connectorContexts);
  collect(broker.appContext);
  collect(broker.connectorContext);
  collect(input.appContexts);
  collect(input.connectorContexts);
  collect(body.appContexts);
  collect(body.connectorContexts);
  collect(body.app_context);
  collect(body.appContext);
  return contexts;
}

function workflowDataAppContexts(body = {}) {
  return workflowAppContexts(body).filter((context) => {
    ...
    const artifactTypes = Array.isArray(context?.artifacts)
      ? context.artifacts.map((artifact) => artifact?.type || artifact?.name || artifact?.label)
      : [];
    const serviceSignals = Array.isArray(raw?.connector_services)
    ...
    return /(analytics|google analytics|ga4|search console|\bgsc\b|conversion|funnel|cohort|acquisition|traffic|landing page|query|event|billing|orders?|stripe|dataset|spreadsheet|sheet|csv|metric)/i.test(text);
  });
}
...
function workflowOpenAiTimeoutMs(config = {}, source = {}) {
  const configured = parseNumber(envValue(source, 'BUILTIN_OPENAI_WORKFLOW_TIMEOUT_MS'), 12000);
  return Math.max(3000, Math.min(Number(config.timeoutMs || 45000), configured));
}

function fastWorkflowRequestOptions(kind = '', body = {}) {
  const normalizedKind = String(kind || '').trim().toLowerCase();
  if (normalizedKind.endsWith('_leader')) {
    return {};
  }
...
function workflowSearchRequiredForKind(kind = '', body = {}) {
  const workflow = workflowContext(body) || {};
  ...
  return phase === 'research'
    && ['research', 'teardown', 'competitor_teardown', 'validation', 'diligence'].includes(normalizedKind);
}

function workflowRequiresSearchSources(kind = '', body = {}) {
  return workflowSearchRequiredForKind(kind, body);
}
...
function searchSourcesRequiredForCompletion(kind = '', body = {}, source = {}) {
  ...
  if (workflowRequiresSearchSources(kind, body)) return true;
  if (workflowContext(body)) return false;
  const normalizedKind = normalizedBuiltInKind(kind);
  return ['research', 'teardown', 'competitor_teardown', 'validation', 'diligence'].includes(normalizedKind)
    && shouldUseWebSearchForKind(kind, body);
}
...
function workflowCanCompleteFromSearchSourcePacket(kind = '', body = {}, source = {}) {
  const workflow = workflowContext(body) || {};
  const phase = String(workflow.sequencePhase || '').trim().toLowerCase();
  const normalizedKind = normalizedBuiltInKind(kind);
  return (workflowRequiresSearchSources(kind, body) || searchSourcesRequiredForCompletion(kind, body, source))
    && phase === 'research'
    && ['research', 'teardown', 'competitor_teardown', 'validation'].includes(normalizedKind);
}

function workflowCanCompleteFromLeaderPacket(kind = '', body = {}, source = {}) {
  const normalizedKind = normalizedBuiltInKind(kind);
  if (!workflowContext(body)) return false;
  if (!isLeaderTaskType(normalizedKind)) return false;
  const phase = workflowSequencePhase(body);
  if (phase === 'initial') {
    const disabled = ['1', 'true', 'yes', 'on'].includes(envValue(source, 'WORKFLOW_INITIAL_LEADER_PACKET_DISABLED').toLowerCase());
    return !disabled;
  }
  if (phase === 'checkpoint') {
    const disabled = ['1', 'true', 'yes', 'on'].includes(envValue(source, 'WORKFLOW_LEADER_CHECKPOINT_PACKET_DISABLED').toLowerCase());
    return !disabled;
  }
  if (['final_summary', 'summary'].includes(phase) && workflowPriorRunsFromBody(body).length) {
    const disabled = ['1', 'true', 'yes', 'on'].includes(envValue(source, 'WORKFLOW_FINAL_SUMMARY_PACKET_DISABLED').toLowerCase());
    return !disabled;
  }
  const enabled = ['1', 'true', 'yes', 'on'].includes(envValue(source, 'WORKFLOW_LEADER_PACKET_FALLBACK').toLowerCase());
  if (!enabled) return false;
  return ['final_summary', 'summary', ''].includes(phase);
}

function workflowCanCompleteFromMissingDataPacket(kind = '', body = {}) {
  const normalizedKind = normalizedBuiltInKind(kind);
  const workflow = workflowContext(body);
  if (!workflow) return false;
  if (normalizedKind !== 'data_analysis') return false;
  if (workflowRequiresSearchSources(kind, body)) return false;
  const phase = workflowSequencePhase(body);
  if (phase && phase !== 'data') return false;
  return workflowDataAppContexts(body).length === 0;
}

function workflowCanCompleteFromAttachedDataPacket(kind = '', body = {}) {
  const normalizedKind = normalizedBuiltInKind(kind);
  const workflow = workflowContext(body);
  if (!workflow) return false;
  if (normalizedKind !== 'data_analysis') return false;
...
function workflowLeaderPacketPayload(kind = '', body = {}) {
  const payload = sampleAgentPayload(kind, body);
  return {
    ...payload,
    runtime: {
      ...
      mode: 'leader_packet',
      provider: 'built_in',
      model: 'deterministic-leader-packet',
      workflow: 'workflow_leader_packet'
    },
    usage: {
      ...(payload.usage && typeof payload.usage === 'object' ? payload.usage : {}),
      api_cost: 0,
      total_cost_basis: Number(payload.usage?.total_cost_basis || 0),
      model: 'deterministic-leader-packet',
...
function workflowPriorRunsFromBody(body = {}) {
  const workflow = workflowContext(body) || {};
  ...
  ].filter((run) => run && typeof run === 'object').slice(0, 10);
}

function workflowSourceBundleFromBody(body = {}, webSources = []) {
  const seen = new Set();
  const sources = [];
  const push = (source = {}, fallbackTitle = '') => {
    if (!source) return;
    const raw = typeof source === 'string' ? source : (source.url || source.link || source.source || '');
    const url = normalizeHttpUrl(raw);
    const title = clipText(typeof source === 'string' ? fallbackTitle : (source.title || source.name || fallbackTitle || url), 140);
    const snippet = clipText(typeof source === 'string' ? '' : (source.snippet || source.description || source.summary || ''), 220);
    const key = [url, title, snippet].join('|').toLowerCase();
    if (!url && !title && !snippet) return;
    if (seen.has(key)) return;
    seen.add(key);
    sources.push({ title: title || 'Prior source', url, snippet });
  };
  for (const source of Array.isArray(webSources) ? webSources : []) push(source, 'Generated source');
  for (const run of workflowPriorRunsFromBody(body)) {
    const label = clipText(run.taskType || run.workflowTask || 'prior work', 80);
    for (const source of Array.isArray(run.webSources) ? run.webSources : []) push(source, label);
    const digest = run.structuredDigest && typeof run.structuredDigest === 'object' ? run.structuredDigest : {};
    for (const source of Array.isArray(digest.sources) ? digest.sources : []) push(source, label);
    for (const url of extractHttpUrlsFromText([run.summary, run.reportSummary, ...(Array.isArray(run.bullets) ? run.bullets : [])].join('\n'), 4)) {
      push(url, label);
    }
  }
  const request = payloadInput('', body);
  for (const url of extractHttpUrlsFromText([request.prompt, request.input?.original_prompt, request.input?.originalPrompt].join('\n'), 4)) {
    push(url, 'Requested target');
  }
  return sources.slice(0, 10);
}

function workflowSpecialistRecoveryEligible(kind = '', body = {}) {
  const normalizedKind = normalizedBuiltInKind(kind);
  if (!workflowContext(body)) return false;
  if (workflowRequiresSearchSources(kind, body)) return false;
  const phase = workflowSequencePhase(body);
  if (!['planning', 'preparation', 'action', 'implementation'].includes(phase)) return false;
  return [
    'media_planner',
    'growth',
    'seo_gap',
...
function workflowSpecialistRecoveryMarkdown(kind = '', body = {}, sources = []) {
  const task = normalizedBuiltInKind(kind);
  const request = payloadInput(kind, body);
  const workflow = workflowContext(body) || {};
  const isJapanese = builtInDeliveryLanguage(body) === 'ja';
  const sourceUrls = sources.map((source) => source.url).filter(Boolean);
  const target = extractHttpUrlsFromText([request.prompt, workflow.objective, workflow.originalPrompt, sourceUrls.join('\n')].join('\n'), 1)[0]
    || 'https://aiagent-marketplace.net/chat';
  const priorRuns = workflowPriorRunsFromBody(body);
  const priorLabels = [...new Set(priorRuns.map((run) => String(run.taskType || run.workflowTask || '').trim()).filter(Boolean))].slice(0, 8);
  const sourceLines = sources.length
    ? sources.slice(0, 6).map((source) => `- ${[source.title, source.url, source.snippet].filter(Boolean).join(' | ')}`)
    : ['- No URL source was included; use only the leader brief and mark external facts unverified.'];
  const upstream = priorLabels.length ? priorLabels.join(' / ') : 'leader brief';
  if (task === 'seo_gap') {
    return [
      '# SEO page artifact',
      '',
      ...
      '',
      '## Keyword and intent',
      '- Primary cluster: AI agent marketplace for developer and technical-user workflows.',
      '- Search intent: compare tools, verify execution capability, understand approval safety, and decide whether to start a trial.',
      '- Page to improve first: /chat as the order/start surface.',
      '',
      '## H1 and metadata',
      '- H1: Order AI agents that carry research into finished technical work.',
      '- Meta title: CAIt - AI agent marketplace for developer growth and workflow orders',
      '- Meta description: Start one chat order, keep GA4/Search Console context attached, route research to specialists, and review approval-gated deliverables before any external action.',
      '',
      '## Page structure draft',
      '1. Hero: explain the order outcome, not only agent discovery.',
      '2. Proof: show source status, agent chain, approval boundary, and reusable delivery packet.',
      '3. Comparison: CAIt vs agent catalog vs single chatbot vs agency handoff.',
      '4. How it works: brief -> leader -> research/data -> planning -> preparation -> approval/action.',
      '5. FAQ: OAuth, external writes, GA4/Search Console context, delivery reuse, retry behavior.',
      '',
      '## Internal links and CTA',
      `- Primary CTA URL: ${target}`,
      '- Internal links: /agents.html, /delivery-manager.html, /apps.html, /help.html.',
      '- CTA copy: Start a growth order with attached analytics context.',
      '',
      '## Measurement',
      '- Track signup_or_trial_start, order_created, delivery_opened, approval_clicked, oauth_connected.',
      '- Stop rule: if qualified signup/order signal does not improve after 7 days, revise proof section before changing channels.'
    ].join('\n');
  }
  if (['writing', 'writer', 'landing'].includes(task)) {
    return [
      ...
      '',
      '## Hero copy',
      '- Headline: Turn one growth request into coordinated AI-agent work.',
      '- Subhead: CAIt keeps your brief, GA4/Search Console context, research, specialist handoffs, approval gates, and final delivery in one chat.',
      '- Primary CTA: Start an order',
      '- Secondary CTA: View delivery manager',
      '',
      '## Body draft',
      'Technical teams do not need another agent catalog that resets context at every step. CAIt starts from the order, lets a leader pass source-backed research and data context to specialists, and returns reviewable artifacts before any external post, send, ad launch, or repository write.',
      '',
      '## Proof and objection handling',
      '- Proof block: show agent map, source status, connector status, retry state, and delivery files.',
      '- Objection: OAuth or external writes are not automatic; exact account, copy, URL, and stop rule require approval.',
      '- Objection: research quality depends on real sources; research layers must carry Brave/search sources forward.',
      '',
      '## Social/post hook',
      `For developer teams trying to turn AI-agent research into actual work: CAIt keeps the order, sources, handoffs, and approval boundary together. Start here: ${target}`,
      '',
      '## Revision test',
      '- A/B test proof-first hero vs speed-first hero for 7 days using signup_or_trial_start and order_created.'
    ].join('\n');
  }
  if (task === 'list_creator') {
    const rows = sources.slice(0, 5).map((source, index) => `| ${index + 1} | ${source.title || `Source ${index + 1}`} | ${source.url || target} | Review as directory, comparison, community, or outreach surface before contact. |`).join('\n');
    return [
      '# Reviewable target list artifact',
      '',
      '## Upstream work used',
      `- ${upstream}`,
      '',
      '| # | Source / company | Public URL | Review action |',
      '| --- | --- | --- | --- |',
      rows || `| 1 | CAIt target surface | ${target} | Use as owned conversion destination; source more third-party rows before outreach. |`,
      '',
      '## Exclusions',
      '- Do not import, enrich, scrape, or send to contacts yet.',
      '- Rows without a public URL stay out of the send queue.',
      '',
      '## Next step',
      '- Leader should approve the reviewed rows and pass only approved targets to cold_email or manual outreach.'
    ].join('\n');
  }
  if (['cold_email', 'email_ops'].includes(task)) {
    return [
      '# Cold email approval draft',
      '',
      '## Upstream work used',
      `- ${upstream}`,
      ...sourceLines,
      '',
      '## Audience and gate',
      '- Audience: developer-tool operators, AI workflow teams, and technical growth owners with public fit evidence.',
      '- Send gate: no send/import until target rows, sender account, exact text, destination URL, and stop rule are approved.',
      '',
      '## Subject options',
      '- Subject A: AI-agent orders that keep research attached',
      '- Subject B: Quick idea for your AI workflow users',
      '',
      '## Email body draft',
      `Hi [Name],\n\nI noticed [company/source signal] and thought this might be relevant if your team is evaluating AI-agent workflows for technical or growth work.\n\nCAIt is an AI agent marketplace built around the order rather than just discovery: a leader keeps the brief, source research, GA4/Search Console context, specialist handoffs, and approval boundary together before anything posts or sends externally.\n\nA useful first check is here: ${target}\n\nWorth a quick look, or should I send a more specific example for your use case?\n\nBest,\n[Sender]`,
      '',
      '## Stop rule',
      '- Stop after one unanswered follow-up or any refusal/unsubscribe signal.'
    ].join('\n');
  }
  return [
    '# Approval-ready action artifact',
    '',
    '## Upstream work used',
    `- ${upstream}`,
    ...sourceLines,
    '',
    '## Exact draft',
    `Draft: CAIt turns one AI-agent order into leader-routed research, planning, preparation, and approval-gated action. Technical teams can start from ${target} and inspect the delivery chain before external execution.`,
    '',
    '## Approval boundary',
    '- Confirm account, exact copy, URL, UTM, timing, and stop rule before posting, sending, publishing, ads, or repository writes.',
    '',
    '## Metric',
    '- signup_or_trial_start and order_created within 7 days.'
  ].join('\n');
}

function workflowSpecialistRecoveryPayload(kind = '', body = {}, webSources = [], stages = [], reason = '') {
  if (!workflowSpecialistRecoveryEligible(kind, body)) return null;
  const task = normalizedBuiltInKind(kind);
  const defaults = builtInAgentDefinitionForKind(task);
  const isJapanese = builtInDeliveryLanguage(body) === 'ja';
  const sources = workflowSourceBundleFromBody(body, webSources);
  const markdown = workflowSpecialistRecoveryMarkdown(task, body, sources);
  const summary = isJapanese
    ? `${defaults.label || task} は leader-owned prior work から具体成果物を返しました。`
    : `${defaults.label || task} returned a concrete artifact from leader-owned prior work.`;
  const nextAction = isJapanese
    ? 'Leader checkpointでこの成果物を確認し、外部実行が必要な場合だけ承認待ちにしてください。'
    : 'Have the leader checkpoint review this artifact and request approval only for external execution.';
  return {
    accepted: true,
    status: 'completed',
    summary,
    report: {
      summary,
      bullets: [
        `Upstream work used: ${workflowPriorRunsFromBody(body).map((run) => run.taskType || run.workflowTask).filter(Boolean).slice(0, 6).join(' / ') || 'leader brief'}`,
        `Concrete artifact file: ${defaults.fileName || `${task}-delivery.md`}`,
        'External posting, sending, ad launch, and repository writes remain approval-gated.'
      ],
      nextAction,
      confidence: 'medium',
      web_sources: sources,
      assumptions: [
        reason ? `Generation provider issue observed: ${clipText(reason, 180)}` : 'Generated from available leader-owned prior work.',
        'Do not treat unverified external claims as facts.'
      ],
      process: [
        ...stages.map((stage) => `${String(stage.stage || 'stage').toUpperCase()} (${stage.durationMs || 0}ms, ${stage.status || 'recorded'}): ${stage.summary || ''}`),
        'SPECIALIST_ARTIFACT_RECOVERY (completed): Endpoint returned a concrete artifact from leader-owned prior work instead of a generic plan.'
      ],
      runtime: {
        workflow: 'workflow_specialist_artifact_recovery',
        mode: 'specialist_artifact_recovery',
        task,
        source_count: sources.length
      }
    },
    files: [
      {
        name: defaults.fileName || `${task}-delivery.md`,
        type: 'text/markdown',
        content: markdown
      }
    ],
    usage: {
      api_cost: 0,
      api_cost_currency: 'USD',
      input_tokens: Math.max(80, JSON.stringify({ task, sources, prior: workflowPriorRunsFromBody(body) }).length / 4),
      output_tokens: Math.max(120, markdown.length / 4),
      total_tokens: Math.max(200, (JSON.stringify({ task, sources }).length + markdown.length) / 4),
      total_cost_basis: 0,
      cost_currency: 'USD',
      model: 'leader-owned-prior-work-artifact',
      model_tier: 'workflow_recovery',
      model_source: 'agent_endpoint'
    },
    return_targets: ['chat', 'api'],
    runtime: {
      mode: 'specialist_artifact_recovery',
...
function workflowAttachedDataPacketPayload(kind = '', body = {}) {
  const defaults = builtInAgentDefinitionForKind(kind);
  const isJapanese = builtInDeliveryLanguage(body) === 'ja';
  const workflow = workflowContext(body) || {};
  const request = payloadInput(kind, body);
  const contexts = workflowDataAppContexts(body);
  const objective = clipText(
    [
      ...
      workflow.objective,
      workflow.originalPrompt
    ].map((item) => String(item || '').trim()).find(Boolean) || String(kind || 'data_analysis'),
    360
  );
  const contextRows = contexts.map((context, index) => {
    const raw = context?.raw_context && typeof context.raw_context === 'object'
      ? context.raw_context
      : (context?.rawContext && typeof context.rawContext === 'object' ? context.rawContext : {});
    const metrics = Array.isArray(context?.metrics) ? context.metrics : [];
    const metricText = metrics.map((metric) => `${metric?.label || metric?.name || 'metric'}: ${metric?.value || metric?.summary || ''}`.trim()).filter(Boolean).join('; ');
    const facts = Array.isArray(context?.facts) ? context.facts.join('; ') : '';
    const services = Array.isArray(raw?.connector_services)
      ? raw.connector_services.join(', ')
      : (Array.isArray(raw?.connectorServices) ? raw.connectorServices.join(', ') : '');
    return {
      index: index + 1,
      title: clipText(context?.title || context?.source_app_label || context?.source_app || `data context ${index + 1}`, 120),
      summary: clipText(context?.summary || facts || metricText || 'Attached data context is available.', 420),
      facts: clipText(facts, 420),
      metrics: clipText(metricText, 420),
      services: clipText(services || raw?.connector_type || raw?.connectorType || '', 160),
      ga4: clipText(raw?.googleGa4Property || '', 160),
      gsc: clipText(raw?.googleSearchConsoleSite || '', 160)
    };
  });
  const bullets = isJapanese
    ? [
        `${contextRows.length}件のサーバー側 app context / connector context を確認しました。`,
        'GA4/Search Console 等の実測コンテキストは「添付データ」として扱い、数値を捏造しません。',
        'Research、Planning、Preparation はこのデータ状態、指標、未確認点を必須の前提として使ってください。'
      ]
    : [
        `${contextRows.length} server-side app/connector data context(s) were attached.`,
        'GA4/Search Console or analytics context is treated as attached evidence; no metric is invented.',
        'Downstream research, planning, and preparation must use this data status, metrics, and gaps as mandatory context.'
      ];
  const nextAction = isJapanese
    ? 'このデータパケットを Research/Planning への必須入力として渡し、未確認の数値は追加接続または再取得時だけ更新してください。'
    : 'Pass this data packet as mandatory context to research/planning; refresh only when new connector data or exports are attached.';
  const markdown = [
    isJapanese ? '# 添付データコンテキストパケット' : '# Attached data context packet',
    '',
    '## Objective',
    objective,
    '',
    isJapanese ? '## Data contexts' : '## Data contexts',
    ...contextRows.flatMap((row) => [
      `### ${row.index}. ${row.title}`,
      `- Summary: ${row.summary || 'available'}`,
      row.services ? `- Services: ${row.services}` : '',
      row.ga4 ? `- GA4 property: ${row.ga4}` : '',
      row.gsc ? `- Search Console site: ${row.gsc}` : '',
      row.metrics ? `- Metrics: ${row.metrics}` : '',
      row.facts ? `- Facts: ${row.facts}` : ''
    ].filter(Boolean)),
    '',
    isJapanese ? '## Downstream handoff' : '## Downstream handoff',
    ...bullets.map((item) => `- ${item}`),
    '',
  ...
  return {
    accepted: true,
    status: 'completed',
    summary: isJapanese ? '添付データコンテキストをデータ層パケットとして整理しました。' : 'Attached data context was converted into a durable data-layer packet.',
    report: {
      summary: isJapanese ? '添付データコンテキストパケット' : 'Attached data context packet',
      bullets,
      nextAction,
      confidence: 'medium',
      web_sources: [],
      assumptions: [
        isJapanese ? '添付コンテキストはサーバー側の注文文脈として信頼します。' : 'Attached contexts are trusted as server-side order context.',
        isJapanese ? '未記載の指標は推測しません。' : 'Metrics not present in the context are not inferred.'
      ],
      workstreams: [
        'Data context inventory',
        'Metric/fact handoff',
        'Downstream planning guardrails'
      ],
      process: [
        `DATA_CONTEXT_CHECK (0ms, completed): ${contextRows.length} analytics/data context(s) attached.`,
        'HANDOFF (0ms, completed): Attached data packet prepared for downstream specialists.'
      ],
      data_contexts: contextRows
    },
    files: [
      {
        name: defaults.fileName,
    ...
    return_targets: ['chat', 'api'],
    runtime: {
      mode: 'attached_data_context_packet',
      provider: 'built_in',
      model: 'deterministic-attached-data-packet',
      workflow: 'workflow_attached_data_context_packet',
      phase: workflowSequencePhase(body) || 'data',
      data_contexts: contextRows.length,
      search_provider: 'none',
      web_sources: []
    }
  };
...
function workflowMissingDataPacketPayload(kind = '', body = {}) {
  const defaults = builtInAgentDefinitionForKind(kind);
  const isJapanese = builtInDeliveryLanguage(body) === 'ja';
  const workflow = workflowContext(body) || {};
  const request = payloadInput(kind, body);
  ...
  const summary = isJapanese
    ? '分析コンテキストが未接続のため、データ層をスキップして仮定を明示しました。'
    : 'Data layer skipped because no analytics/data context is attached; assumptions are explicit.';
  const bullets = isJapanese
    ? [
        'GA4、Search Console、内部イベント、課金/注文データのコンテキストはこの実行に添付されていません。',
        'この段階では実測値を作らず、Research/Planning へ進めるための前提として扱います。',
        '接続またはエクスポートが追加された場合だけ、データ層を再実行してください。'
      ]
    : [
        'No GA4, Search Console, internal event, billing, or order-data context is attached to this run.',
        'No measured metrics were invented; downstream phases should treat this as an explicit assumption.',
        'Rerun the data layer only after analytics context or exported data is attached.'
      ];
  const nextAction = isJapanese
    ? 'データなし前提で Research/Planning/Preparation へ進め、GA4/Search Console 等が接続されたら同じ注文文脈でデータ層だけ再実行してください。'
    : 'Proceed to research, planning, and preparation with the no-data assumption; rerun only this data layer after GA4/Search Console or exported data is attached.';
  const markdown = [
    isJapanese ? '# データ層スキップパケット' : '# Data layer skip packet',
    '',
    isJapanese ? '## Objective' : '## Objective',
    objective,
    '',
    isJapanese ? '## Source status' : '## Source status',
    ...bullets.map((item) => `- ${item}`),
    '',
    isJapanese ? '## Assumptions for downstream agents' : '## Assumptions for downstream agents',
    ...(isJapanese
      ? [
          '- 実測ファネル、検索クエリ、流入チャネル、CVR は未確認です。',
          '- 施策優先度は公開情報、競合調査、ユーザー指定の目標を優先して決めます。',
          '- 外部投稿、送信、PR 作成などの write は別途 approval gate が必要です。'
        ]
      : [
          '- Funnel, query, acquisition channel, and conversion-rate metrics are unverified.',
          '- Prioritization should use public research, competitor/channel evidence, and the user-stated goal.',
          '- External posting, sending, PR creation, or repository write actions still require an approval gate.'
        ]),
    '',
    isJapanese ? '## Next action' : '## Next action',
    nextAction
  ...
  return {
    ...
    report: {
      ...
      workstreams: [
        ...
        'No-data assumption handoff',
        'Measurement rerun trigger'
      ],
      process: [
    ...
    usage: {
      ...
      total_cost_basis: 0,
      cost_currency: 'USD',
      model: 'deterministic-data-availability-packet',
      model_tier: 'orchestration',
      model_source: 'built_in'
    },
    return_targets: ['chat', 'api'],
    runtime: {
      mode: 'data_unavailable_packet',
      provider: 'built_in',
      model: 'deterministic-data-availability-packet',
      workflow: 'workflow_data_unavailable_packet',
      phase: workflowSequencePhase(body) || 'data',
      data_contexts: 0,
      search_provider: 'none',
      web_sources: []
...
async function workflowSearchSourcePacketPayload(kind = '', body = {}, braveSearch = {}, stages = [], source = {}) {
  const defaults = builtInAgentDefinitionForKind(kind);
  const isJapanese = builtInDeliveryLanguage(body) === 'ja';
  const workflow = workflowContext(body) || {};
  const request = payloadInput(kind, body);
  const collectedSources = (Array.isArray(braveSearch.sources) ? braveSearch.sources : []).slice(0, 8);
  const query = String(braveSearch.query || collectedSources[0]?.query || braveSearchQuery(kind, body)).trim();
  const objective = clipText(
    [
      request.prompt,
      request.goal,
      workflow.objective,
      workflow.originalPrompt
    ].map((item) => String(item || '').trim()).find(Boolean) || String(kind || 'research'),
    320
  );
  const taskFrame = orderUserRequestText(body, objective);
  const pageSignals = await enrichResearchSourcesWithPageSignals(collectedSources, source);
  const pageSignalByUrl = new Map(pageSignals.filter((signal) => signal?.url).map((signal) => [signal.url, signal]));
  const sources = collectedSources.map((item) => {
    const url = normalizeHttpUrl(item?.url);
    const signal = pageSignalByUrl.get(url) || null;
    return signal
      ? {
          ...item,
          page_signal: {
            fetched: signal.fetched,
            word_count: signal.wordCount || 0,
            cjk_character_count: signal.cjkCharacterCount || 0,
            h1: signal.h1 || [],
  ...
  const synthesis = buildResearchSynthesis({ sources, pageSignals, objective, query, taskFrame, isJapanese });
  const openAi3C = await openAiResearch3CSynthesis(kind, body, source, {
    sources,
    query,
    objective,
    taskFrame,
  ...
  const sourceLines = sources.map((item, index) => {
    const title = String(item.title || item.url || item.query || `Source ${index + 1}`).trim();
    const url = String(item.url || '').trim();
    const snippet = String(item.snippet || '').trim();
    return [
      `${index + 1}. ${title}${url ? ` - ${url}` : ''}`,
      snippet ? `   - ${clipText(snippet, 220)}` : ''
    ].filter(Boolean).join('\n');
  });
  const openAiResult = openAi3C.ok ? openAi3C.parsed : null;
  const bullets = openAiResult?.bullets?.length ? openAiResult.bullets : synthesis.bullets;
  const summary = openAiResult?.report_summary || (isJapanese
    ? `検索結果をタスク別リサーチメモに整理しました: ${sources.length}件のBrave Searchソース`
    : `Task-aligned research memo ready: ${sources.length} Brave Search source(s) synthesized.`);
  const nextAction = isJapanese
    ? 'このリサーチメモをPlanning/Preparation層へ渡し、タスク別結論、上位コンテンツの共通パターン、語数レンジ、証拠不足を媒体選定、SEOギャップ、ライティング、実行前承認パケットに反映してください。'
    : 'Pass this research memo to planning/preparation; use its task-aligned findings, top-content patterns, word-count range, source signals, and proof gaps in channel, SEO, writing, and approval packets.';
  const process = [
    ...stages,
    stageRecord('research_synthesis', 0, `Synthesized ${sources.length} Brave Search source(s); ${synthesis.fetchedCount} page body fetch(es) succeeded.`, { status: 'completed' }),
    openAi3C.ok
      ? stageRecord('openai_3c_synthesis', openAi3C.durationMs, `OpenAI converted Brave sources into 3C analysis using ${openAi3C.model}.`, { status: 'completed' })
      : stageRecord('openai_3c_synthesis', openAi3C.durationMs || 0, openAi3C.skipped ? `OpenAI 3C synthesis skipped: ${openAi3C.skipped}.` : `OpenAI 3C synthesis failed: ${clipText(openAi3C.error?.message || openAi3C.error || 'unknown error', 120)}.`, { status: openAi3C.skipped ? 'skipped' : 'warning' })
  ];
  const deterministicMarkdown = [
    isJapanese ? '# リサーチメモ（検索証跡付き）' : '# Research memo with search evidence',
    '',
    `${isJapanese ? 'Observation date' : 'Observation date'}: ${nowIso().slice(0, 10)}`,
    query ? `${isJapanese ? 'Search query' : 'Search query'}: ${query}` : '',
    '',
    isJapanese ? '## Objective' : '## Objective',
    objective,
    '',
    synthesis.markdown,
    '',
    isJapanese ? '## Sources' : '## Sources',
    ...(sourceLines.length ? sourceLines : ['- No source URL available.']),
    '',
    isJapanese ? '## Handoff' : '## Handoff',
    nextAction
  ].filter((line) => line !== '').join('\n');
  const markdown = openAiResult?.file_markdown
    ? ensureResearch3CMarkdown(openAiResult.file_markdown, synthesis, sources, isJapanese)
    : deterministicMarkdown;
  const totalUsage = openAi3C.ok
    ? {
        input_tokens: parseNumber(openAi3C.usage?.input_tokens, Math.max(80, JSON.stringify({ query, objective, taskFrame, sources, research_findings: synthesis.structured }).length / 4)),
        output_tokens: parseNumber(openAi3C.usage?.output_tokens, Math.max(80, markdown.length / 4)),
        total_tokens: parseNumber(openAi3C.usage?.total_tokens, 0)
      }
    : {
        input_tokens: Math.max(80, JSON.stringify({ query, objective, taskFrame, sources, research_findings: synthesis.structured }).length / 4),
        output_tokens: Math.max(80, markdown.length / 4),
        total_tokens: 0
      };
  if (!totalUsage.total_tokens) totalUsage.total_tokens = totalUsage.input_tokens + totalUsage.output_tokens;
  const model = openAi3C.ok ? openAi3C.model : 'brave-search-research-synthesis';
  const modelTier = openAi3C.ok ? openAi3C.routing?.tier : 'source_collection_plus_synthesis';
  const modelSource = openAi3C.ok ? `openai_${openAi3C.routing?.source || 'research_3c'}` : 'brave_search';
  const apiCost = openAi3C.ok ? openAi3C.apiCost : 0;
  return {
    accepted: true,
    status: 'completed',
    summary,
    report: {
      summary,
      bullets,
      nextAction: openAiResult?.next_action || nextAction,
      confidence: openAiResult?.confidence || 'medium',
      authority_request: openAiResult?.authority_request || undefined,
      web_sources: sources,
      research_findings: synthesis.structured,
      three_c_analysis: synthesis.structured.three_c_analysis,
      downstream_handoff: synthesis.structured.downstream_handoff,
      evidence_gaps: synthesis.structured.evidence_gaps,
      task_aligned_findings: synthesis.taskAlignedFindings,
      content_patterns: synthesis.contentPatterns,
      heading_themes: synthesis.headingThemes,
      openai_3c_synthesis: openAi3C.ok
        ? {
            status: 'completed',
            model: openAi3C.model,
            duration_ms: openAi3C.durationMs
          }
        : {
            status: openAi3C.skipped ? 'skipped' : 'warning',
            reason: openAi3C.skipped || clipText(openAi3C.error?.message || openAi3C.error || 'unknown error', 180)
          },
      assumptions: [
        'This workflow research layer used Brave search snippets, URLs, and fetched page signals where available.',
        openAi3C.ok
          ? 'OpenAI converted the Brave evidence bundle into 3C analysis; source-specific claims still require the cited URLs.'
          : 'OpenAI 3C synthesis was unavailable, so the deterministic Brave-source 3C analysis is marked as synthesis from source snippets/page signals.',
        'Word counts are estimates from fetched HTML text; rows marked snippet-only were not treated as full-page reads.',
        'This is research synthesis, not a raw search-result handoff; downstream agents should use the task-aligned conclusions and verify exact page claims before publishing or spending.'
      ],
      workstreams: [
        'Source collection',
        'Research synthesis',
        'SEO/channel preparation'
      ],
      process: process.map((stage) => `${stage.stage.toUpperCase()} (${stage.durationMs}ms, ${stage.status}): ${stage.summary}`)
    },
    files: [
      {
        name: defaults.fileName,
    ...
    usage: {
      ...
      ...(openAi3C.ok ? {
        pricing: {
          provider: 'openai',
          model,
          input_price_per_mtok: openAi3C.pricing?.inputPricePerMTok || 0,
          output_price_per_mtok: openAi3C.pricing?.outputPricePerMTok || 0,
          source: openAi3C.pricing?.source || 'none'
        }
      } : {})
    },
    return_targets: ['chat', 'api'],
    runtime: {
      mode: 'research_synthesis_packet',
      provider: openAi3C.ok ? 'openai' : 'brave',
      model,
      model_tier: modelTier,
      model_source: modelSource,
      workflow: openAi3C.ok ? 'workflow_research_3c_synthesis_packet' : 'workflow_research_synthesis_packet',
      search_provider: 'brave',
      steps: process,
      openai_3c_synthesis: openAi3C.ok ? 'completed' : (openAi3C.skipped || 'warning'),
      research_findings: synthesis.structured,
      three_c_analysis: synthesis.structured.three_c_analysis,
      downstream_handoff: synthesis.structured.downstream_handoff,
      evidence_gaps: synthesis.structured.evidence_gaps,
...
function builtInGenerationFailedPayload(kind = '', body = {}, reason = 'Built-in agent generation failed.', extra = {}) {
  const message = String(reason || 'Built-in agent generation failed.').trim();
  return {
    accepted: false,
    status: 'failed',
    error: message,
    ...
    report: {
      summary: message,
      bullets: [],
      nextAction: 'Retry after the missing input, source, or generation provider is available.',
      web_sources: Array.isArray(extra.webSources) ? extra.webSources : []
    },
    files: [],
    ...
    runtime: {
      mode: 'failed',
      provider: 'none',
      kind: normalizedBuiltInKind(kind),
      workflow: extra.workflow || 'generation_failed',
      failure_reason: message,
      search_provider: extra.searchProvider || 'none',
      web_sources: Array.isArray(extra.webSources) ? extra.webSources : [],
      steps: Array.isArray(extra.stages) ? extra.stages : []
    }
  };
}
...
async function callOpenAi(kind, body, source = {}) {
  ...
  const sourcePacketEligible = fastWorkflow && workflowCanCompleteFromSearchSourcePacket(kind, body, source);
  if (fastWorkflow && workflowCanCompleteFromAttachedDataPacket(kind, body)) {
    return workflowAttachedDataPacketPayload(kind, body);
  }
  if (fastWorkflow && workflowCanCompleteFromMissingDataPacket(kind, body)) {
    return workflowMissingDataPacketPayload(kind, body);
  }
  if (fastWorkflow && workflowCanCompleteFromLeaderPacket(kind, body, source)) {
    return workflowLeaderPacketPayload(kind, body);
  }
  let braveSearch = { enabled: false, provider: 'none', query: '', sources: [], error: null };
  const sourceCollectionTimeoutMs = workflowSourceCollectionTimeoutMs(source);
  const sourceRequiredBeforeGeneration = workflowRequiresSearchSources(kind, body)
    || searchSourcesRequiredForCompletion(kind, body, source);
  try {
    braveSearch = sourcePacketEligible || sourceRequiredBeforeGeneration
      ? await fetchBraveWebSourcesWithinBudget(kind, body, source, sourceCollectionTimeoutMs)
      : await fetchBraveWebSources(kind, body, source);
  } catch (error) {
    braveSearch = { enabled: true, provider: 'brave', query: braveSearchQuery(kind, body), sources: [], error };
  }
  const stages = [];
  if (sourcePacketEligible) {
    if (braveSearch.sources?.length) {
      stages.push(stageRecord('source_preflight', 0, `${braveSearch.sources.length} Brave Search source(s) available before generation.`, { status: 'completed' }));
      return await workflowSearchSourcePacketPayload(kind, body, braveSearch, stages, source);
    }
    const reason = braveSearch.error
      ? `Source collection failed before generation. Brave search error: ${String(braveSearch.error?.message || braveSearch.error).slice(0, 260)}`
      : `No source URLs were available before generation after waiting ${sourceCollectionTimeoutMs}ms. OpenAI generation was not started for this source-required run.`;
    stages.push(stageRecord('source_preflight', 0, reason, { status: 'failed' }));
    return builtInGenerationFailedPayload(
      kind,
      body,
      reason,
      {
        searchProvider: braveSearch.enabled ? 'brave' : 'none',
        webSources: [],
        stages,
  ...
  if (sourceRequiredBeforeGeneration && !braveSearch.sources.length) {
    const reason = braveSearch.error
      ? `Source collection failed before generation. Brave search error: ${String(braveSearch.error?.message || braveSearch.error).slice(0, 260)}`
      : 'No source URLs were available before generation. OpenAI generation was not started for this source-required run.';
    stages.push(stageRecord('source_preflight', 0, reason, { status: 'failed' }));
    return builtInGenerationFailedPayload(
      kind,
      body,
  ...
  if (!config.apiKey) {
    return builtInGenerationFailedPayload(
      kind,
      body,
      'OpenAI generation is not configured, so this built-in agent cannot create a fresh delivery from the current input and sources.',
      {
        searchProvider: braveSearch.sources.length ? 'brave' : 'none',
        webSources: braveSearch.sources,
        workflow: 'missing_generation_provider'
      }
    );
  }
  ...
  if (fastWorkflow) {
    ...
    try {
      ...
      return builtInGenerationFailedPayload(
        kind,
        body,
        `OpenAI plan generation failed: ${String(error?.message || error).slice(0, 300)}`,
        {
          searchProvider: braveSearch.sources.length ? 'brave' : (webSources.length ? 'openai_web_search' : 'none'),
          webSources,
          stages,
          workflow: 'plan_generation_failed'
        }
      );
    }
  }
  ...
  try {
    ...
    const recoveryPayload = fastWorkflow
      ? workflowSpecialistRecoveryPayload(kind, body, webSources, stages, String(error?.message || error))
      : null;
    if (recoveryPayload) return recoveryPayload;
    return builtInGenerationFailedPayload(
      kind,
      body,
      `OpenAI draft generation failed: ${String(error?.message || error).slice(0, 300)}`,
      {
        searchProvider: braveSearch.sources.length ? 'brave' : (webSources.length ? 'openai_web_search' : 'none'),
        webSources,
        stages,
        workflow: fastWorkflow ? 'workflow_fast_generation_failed' : 'draft_generation_failed'
      }
    );
  }
  ...
  if (workflowRequiresSearchSources(kind, body) && !webSources.length) {
    return builtInGenerationFailedPayload(
      kind,
      body,
      braveSearch.error
        ? `Brave search request failed. ${String(braveSearch.error?.message || braveSearch.error).slice(0, 280)}`
        : 'No verifiable source URLs were available. This workflow requires source-backed generation before completion.',
      { searchProvider: 'none', webSources, stages, workflow: 'missing_required_search_sources' }
    );
  }
  if (searchSourcesRequiredForCompletion(kind, body, source) && !webSources.length) {
    return builtInGenerationFailedPayload(
      ...
      braveSearch.error
        ? `Brave search request failed. ${String(braveSearch.error?.message || braveSearch.error).slice(0, 280)}`
        : 'Search was required, but Brave/OpenAI returned no verifiable source URLs.',
      { searchProvider: 'none', webSources, stages, workflow: 'missing_required_search_sources' }
    );
  }
  const qualityFailure = cmoWorkflowDeliveryQualityFailure(kind, body, finalMarkdown, isJapanese, promptText(body));
  const qualityWarnings = qualityFailure ? [qualityFailure] : [];
  if (qualityFailure) {
    stages.push(stageRecord('quality_gate', 0, qualityFailure, { status: 'warning', completionBlocking: false }));
  }
  ...
  return {
    ...
    report: {
      ...
      ...(executionCandidateReport ? { execution_candidate: executionCandidateReport } : {}),
      ...(qualityWarnings.length ? { quality_warnings: qualityWarnings } : {}),
      web_sources: webSources,
...
export async function runBuiltInAgent(kind, body = {}, source = {}) {
  const preparedBody = bodyWithEffectivePrompt(body);
  const config = openAiConfig(source);
  const sampleFallbackRequested = ['1', 'true', 'yes', 'on'].includes(envValue(source, 'BUILTIN_AGENT_SAMPLE_FALLBACK').toLowerCase());
  const explicitTestRuntime = envValue(source, 'NODE_ENV').toLowerCase() === 'test'
    || envValue(source, 'APP_VERSION').toLowerCase().includes('test');
  const sampleFallbackEnabled = sampleFallbackRequested && explicitTestRuntime;
  if (!config.apiKey && sampleFallbackEnabled) {
    const sourceRequiredBeforeGeneration = workflowRequiresSearchSources(kind, preparedBody)
      || searchSourcesRequiredForCompletion(kind, preparedBody, source);
    let braveSearch = { enabled: false, provider: 'none', query: '', sources: [], error: null };
    if (sourceRequiredBeforeGeneration) {
      const stages = [];
      const sourceCollectionTimeoutMs = workflowSourceCollectionTimeoutMs(source);
      try {
        braveSearch = await fetchBraveWebSourcesWithinBudget(kind, preparedBody, source, sourceCollectionTimeoutMs);
      } catch (error) {
        braveSearch = { enabled: true, provider: 'brave', query: braveSearchQuery(kind, preparedBody), sources: [], error };
      }
      if (!braveSearch.sources.length) {
        const reason = braveSearch.error
          ? `Source collection failed before generation. Brave search error: ${String(braveSearch.error?.message || braveSearch.error).slice(0, 260)}`
    ...
    const payload = sampleAgentPayload(kind, preparedBody);
    return braveSearch.sources.length
      ? withGroundedWebSources(payload, braveSearch.sources, preparedBody, braveSearch.provider)
      : payload;
  }
  return callOpenAi(kind, preparedBody, source);
}
