...
function workflowSearchSourcePacketPayload(kind = '', body = {}, braveSearch = {}, stages = []) {
  const defaults = builtInAgentDefinitionForKind(kind);
  const isJapanese = builtInDeliveryLanguage(body) === 'ja';
  const workflow = workflowContext(body) || {};
  const request = payloadInput(kind, body);
  ...
  const sourceLines = sources.map((source, index) => {
    const title = String(source.title || source.url || source.query || `Source ${index + 1}`).trim();
    const url = String(source.url || '').trim();
  ...
  const bullets = sources.slice(0, 6).map((source) => {
    const title = String(source.title || source.url || source.query || 'Search source').trim();
    const snippet = String(source.snippet || '').trim();
    return clipText([title, snippet].filter(Boolean).join(': '), 240);
  });
  const summary = isJapanese
    ? `検索ソースパケットを作成しました: ${sources.length}件のBrave Searchソース`
    : `Research source packet ready: ${sources.length} Brave Search source(s) collected.`;
  const nextAction = isJapanese
    ? 'この検索ソースをPlanning/Preparation層へ渡し、媒体選定、SEOギャップ、実行前承認パケットに反映してください。'
    : 'Use this search source packet as the research layer, then continue to planning/preparation and cite these URLs in channel, SEO, and approval packets.';
  const process = [
    ...stages,
    stageRecord('source_packet', 0, `Completed from ${sources.length} Brave Search source(s).`, { status: 'completed' })
  ];
  const markdown = [
    isJapanese ? '# リサーチソースパケット' : '# Research source packet',
    '',
    ...
    objective,
    '',
    isJapanese ? '## Source-backed signals' : '## Source-backed signals',
    ...(bullets.length ? bullets.map((item) => `- ${item}`) : ['- Search returned URLs but no snippets. Treat detailed claims as source-limited.']),
    '',
  ...
  return {
    ...
    report: {
      ...
      web_sources: sources,
      assumptions: [
        'This workflow research layer used collected search snippets and URLs directly to preserve source quality before planning.',
    ...
    usage: {
      api_cost: 0,
      api_cost_currency: 'USD',
      input_tokens: Math.max(80, JSON.stringify({ query, objective, sources }).length / 4),
      output_tokens: Math.max(80, markdown.length / 4),
      total_tokens: Math.max(160, (JSON.stringify({ query, objective, sources }).length + markdown.length) / 4),
      total_cost_basis: 0,
      cost_currency: 'USD',
      model: 'brave-search-source-packet',
      model_tier: 'source_collection',
    ...
    runtime: {
      ...
      provider: 'brave',
      model: 'brave-search-source-packet',
      workflow: 'workflow_search_source_packet',
...
async function callOpenAi(kind, body, source = {}) {
  ...
  if (sourcePacketEligible) {
    if (braveSearch.sources?.length) {
      ...
      return workflowSearchSourcePacketPayload(kind, body, braveSearch, stages);
    }
