...
function workflowSearchRequiredForKind(kind = '', body = {}) {
  ...
  return workflow.forceWebSearch === true
    || (phase === 'research' && ['research', 'teardown', 'competitor_teardown', 'validation', 'diligence'].includes(normalizedKind));
}
...
async function callOpenAi(kind, body, source = {}) {
  ...
  if (qualityFailure) {
    stages.push(stageRecord('quality_gate', 0, qualityFailure, { status: 'failed' }));
    return builtInGenerationFailedPayload(
      kind,
      body,
      `OpenAI output failed the originality/source quality gate: ${qualityFailure}`,
      {
        searchProvider: braveSearch.sources.length ? 'brave' : (webSources.length ? 'openai_web_search' : 'none'),
        webSources,
        stages,
        workflow: 'quality_gate_failed'
      }
    );
  }
