...
function workflowCanCompleteFromSearchSourcePacket(kind = '', body = {}) {
  const workflow = workflowContext(body) || {};
  const phase = String(workflow.sequencePhase || '').trim().toLowerCase();
  const normalizedKind = normalizedBuiltInKind(kind);
  return workflowRequiresSearchSources(body)
    && phase === 'research'
...
async function callOpenAi(kind, body, source = {}) {
  ...
  const sourcePacketEligible = fastWorkflow && workflowCanCompleteFromSearchSourcePacket(kind, body);
