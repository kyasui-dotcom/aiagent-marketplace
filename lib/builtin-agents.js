...
function workflowCanCompleteFromSearchSourcePacket(kind = '', body = {}, source = {}) {
  ...
  const phase = String(workflow.sequencePhase || '').trim().toLowerCase();
  const normalizedKind = normalizedBuiltInKind(kind);
  return (workflowRequiresSearchSources(body) || searchSourcesRequiredForCompletion(kind, body, source))
    && phase === 'research'
...
function workflowCanCompleteFromLeaderPacket(kind = '', body = {}, source = {}) {
  ...
  if (workflowRequiresSearchSources(body)) return false;
  const phase = workflowSequencePhase(body);
  if (phase === 'initial') {
...
async function callOpenAi(kind, body, source = {}) {
  ...
  const sourceRequiredBeforeGeneration = workflowRequiresSearchSources(body)
...
export async function runBuiltInAgent(kind, body = {}, source = {}) {
  ...
  if (!config.apiKey && sampleFallbackEnabled) {
    const sourceRequiredBeforeGeneration = workflowRequiresSearchSources(preparedBody)
