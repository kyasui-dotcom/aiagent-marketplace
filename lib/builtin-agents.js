...
function researchPageFetchConfig(source = {}) {
  ...
  const maxPages = Math.max(0, Math.min(5, parseNumber(envValue(source, 'WORKFLOW_RESEARCH_PAGE_FETCH_MAX'), 5)));
  const timeoutMs = Math.max(3000, Math.min(20000, parseNumber(envValue(source, 'WORKFLOW_RESEARCH_PAGE_FETCH_TIMEOUT_MS'), 8000)));
...
function researchWordCountRange(pageSignals = [], isJapanese = false) {
  ...
  return isJapanese
    ? `取得できた上位ページの推定語数は ${min.toLocaleString('en-US')} - ${max.toLocaleString('en-US')} words、平均約 ${avg.toLocaleString('en-US')} words です。`
    : `Fetched top-page estimated word-count range is ${min.toLocaleString('en-US')} - ${max.toLocaleString('en-US')} words, average about ${avg.toLocaleString('en-US')} words.`;
}

function buildResearchSynthesis({ sources = [], pageSignals = [], objective = '', query = '', isJapanese = false } = {}) {
  ...
  const bullets = isJapanese
    ? [
        `Brave Searchで ${sources.length} 件を確認し、上位 ${attemptedFetchCount} 件の本文取得を試行して ${fetchedCount} 件成功しました。`,
        wordRange,
    ...
    : [
        `Brave Search returned ${sources.length} source(s); page-body fetch was attempted for the top results and succeeded for ${fetchedCount}.`,
        wordRange,
...
async function workflowSearchSourcePacketPayload(kind = '', body = {}, braveSearch = {}, stages = [], source = {}) {
  ...
  const synthesis = buildResearchSynthesis({ sources, pageSignals, objective, query, isJapanese });
  ...
  const summary = isJapanese
    ? `検索結果をリサーチメモに整理しました: ${sources.length}件のBrave Searchソース`
    : `Research memo ready: ${sources.length} Brave Search source(s) synthesized.`;
  const nextAction = isJapanese
    ? 'このリサーチメモをPlanning/Preparation層へ渡し、媒体選定、SEOギャップ、ライティング、実行前承認パケットに反映してください。'
    : 'Pass this research memo to planning/preparation; use its intent, source signals, word-count range, and proof gaps in channel, SEO, writing, and approval packets.';
  const process = [
    ...stages,
  ...
  return {
    ...
    report: {
      ...
      assumptions: [
        ...
        'Downstream agents should verify exact page claims before publishing or spending.'
      ],
      workstreams: [
        'Source collection',
        'Research synthesis',
    ...
    usage: {
      api_cost: 0,
      api_cost_currency: 'USD',
      input_tokens: Math.max(80, JSON.stringify({ query, objective, sources, research_findings: synthesis.structured }).length / 4),
      output_tokens: Math.max(80, markdown.length / 4),
      total_tokens: Math.max(160, (JSON.stringify({ query, objective, sources, research_findings: synthesis.structured }).length + markdown.length) / 4),
      total_cost_basis: 0,
      cost_currency: 'USD',
