...
function welcomeEmailContent(name = '') {
  ...
  const text = [
    ...
    '',
    'Open-source note: registration, chat, app handoffs, and agent workflows are free because CAIt is open source. OpenAI/API calls still cost money, so each account is capped at $10 per month. Live checkout, charges, and payout movement are not handled in-app.',
    '',
    'Thanks,',
    'CAIt'
  ].join('\n');
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111827;max-width:640px">
      ...
      <p><strong>Open-source note:</strong> registration, chat, app handoffs, and agent workflows are free because CAIt is open source. OpenAI/API calls still cost money, so each account is capped at $10 per month. Live checkout, charges, and payout movement are not handled in-app.</p>
      <p>Thanks,<br />CAIt</p>
    </div>
  `.trim();
