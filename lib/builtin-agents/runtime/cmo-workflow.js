...
| --- | --- | --- | --- | --- |
| 1 | ${channel} | execute_now | audience intent と ${conversion} への近さが最も高い | 入口URL、主CTA、証拠block、計測event |
| 2 | owned/search page | prepare_first | 検索意図と比較検討を受け止めやすい | H1、FAQ、内部リンク、UTM |
| 3 | directory/comparison | prepare_first | カテゴリ検索がある商材なら高効率 | 掲載文、スクリーンショット、価格/カテゴリfields |
| 4 | X/Reddit/community | ask_user_before_action | 反応語彙収集に向くが、規約/宣伝感/アカウント文脈の確認が必要 | exact copy、投稿先、アカウント、stop rule |
...
| Proof/assets | 実績、画面、価格、手順が不足しやすい | writer/landing/seo_gap で先に補う |
| Account/connector | 投稿・送信・掲載は承認と接続状態が必要 | action agentの前にleader checkpointへ戻す |
| Policy risk | コミュニティ投稿、メール、掲載は規約差が大きい | rule check と exact copy approval を必須にする |

## Specialist handoff
- landing/seo_gap: H1、導線、FAQ、内部リンク、proof blockを作る。
- writing/x_post/reddit/indie_hackers: 媒体ごとの exact copy、UTM、投稿先rule checkを作る。
- directory_submission/citation_ops: カテゴリ/地域/掲載条件に合う候補だけを実行packet化する。
- data_analysis: データがある場合だけ referral、CV開始、CV完了を媒体別に見る。

## Ask user before action
- どの媒体を最初に実行するか、またはMedia Plannerの上位候補をまとめて承認するか。
- 投稿/掲載/送信に使うアカウント、URL、exact copy、停止条件。
...
| --- | --- | --- | --- | --- |
| 1 | ${channel} | execute_now | strongest audience intent and closest path to ${conversion}. | Destination URL, primary CTA, proof block, and one measurable intent event |
| 2 | Owned/search destination | prepare_first | captures comparison and use-case intent before broad promotion. | H1/meta, FAQ, internal links, proof block, UTM |
...
| 4 | X/Reddit/community | ask_user_before_action | useful for response language, but surface rules and account context matter. | Exact copy, destination, UTM, account, stop rule |
...
| Conversion proximity | Favor surfaces that can point directly to ${conversion}. | Destination and CTA preparation come before external posting. |
| Proof and assets | Claims need real screenshots, examples, pricing, process, or approved proof. | Missing proof routes to writing, landing, or seo_gap before action. |
| Account and connector readiness | Posting, sending, and submission require the right account and connector state. | Action agents wait for leader approval and connector confirmation. |
...
- Before any external write, ask the user to approve the selected media, account, exact copy, destination URL, UTM, and stop rule.`;
  }
