...
function cmoLeaderPriorRuns(body = {}) {
  ...
  const runs = [
    ...
    ...(Array.isArray(workflow.priorRuns) ? workflow.priorRuns : []),
    ...(Array.isArray(workflow.priorDeliverables) ? workflow.priorDeliverables : []),
...
function cmoLeaderRunTask(run = {}) {
  return normalizedCmoTask(run.taskType || run.task_type || run.workflowTask || run.kind || '');
}
...
function cmoLeaderRunSummary(run = {}) {
  ...
  return agentProviderText(run.summary || run.reportSummary || report.summary || digest.summary || '');
}
...
function cmoLeaderSelectedFields(selectedRun = {}, body = {}) {
  ...
  const firstSectionLine = (titles = [], max = 240) => cmoLeaderExtractSection(selectedText, titles, max)
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)[0] || '';
  const title = cmoLeaderLabeledValue(selectedText, ['Meta title', 'Title', 'タイトル'])
    || firstSectionLine(['Meta title', 'Title', 'タイトル'])
...
function cmoLeaderEvaluationContext(kind = '', definition = {}, body = {}, options = {}) {
  ...
  return {
    ...
    service_name: agentProviderText(body.input?.service_name || body.input?.serviceName || body.input?.business || body.input?.product_name || body.input?.productName),
    growth_goal: agentProviderText(body.input?.growth_goal || body.input?.growthGoal || body.input?.goal || body.goal),
...
function cmoLeaderNormalizeHeadingTitle(value = '') {
  return String(value || '')
    .trim()
    .replace(/^[0-9０-９]+[\).）．、:\s-]*/, '')
    .replace(/[（(][^）)]*[）)]/g, '')
...
function cmoLeaderHasSection(markdown = '', titles = []) {
  const normalizedTitles = new Set(titles.map(cmoLeaderNormalizeHeadingTitle).filter(Boolean));
  return String(markdown || '').split(/\r?\n/).some((line) => {
    const match = String(line || '').trim().match(/^##\s+(.+)$/);
    if (!match?.[1]) return false;
    return normalizedTitles.has(cmoLeaderNormalizeHeadingTitle(match[1]));
  });
}
...
function cmoLeaderInsertSectionAfter(markdown = '', sectionMarkdown = '', afterTitles = [], titles = []) {
  ...
  const normalizedAfterTitles = new Set(afterTitles.map(cmoLeaderNormalizeHeadingTitle).filter(Boolean));
  const headingMatches = (line = '') => {
    const match = String(line || '').trim().match(/^#{1,6}\s+(.+)$/);
    if (!match?.[1]) return false;
    return normalizedAfterTitles.has(cmoLeaderNormalizeHeadingTitle(match[1]));
  };
...
function cmoLeaderConfirmedFactsMarkdown(leaderSynthesis = null, japanese = false) {
  ...
  const perspectiveRows = Array.isArray(leaderSynthesis?.perspective_review_rows)
    ...
    : [];
  const product = leaderSynthesis?.service_name
    || cmoLeaderSynthesisRequestField(leaderSynthesis, ['Product/service', 'Product', 'Service', 'Business', 'サービス', '商品']);
  const goal = leaderSynthesis?.growth_goal
    || cmoLeaderSynthesisRequestField(leaderSynthesis, ['Goal', 'Main goal', 'Objective', '目的']);
...
function cmoLeaderReviewPayloadMarkdown(leaderSynthesis = null, japanese = false) {
  const packet = leaderSynthesis?.selected_review_packet || {};
  const fields = packet.fields && typeof packet.fields === 'object' ? packet.fields : {};
  const perspective = packet.perspective || (japanese ? 'レビュー用成果物' : 'review item');
  const targetUrl = fields.targetUrl || leaderSynthesis?.target_url || '';
  const title = fields.title || fields.h1 || `${perspective}${targetUrl ? ` / ${targetUrl}` : ''}`;
  const h1 = fields.h1 || '';
  const cta = fields.primaryCta || '';
  const bodyDraft = cmoLeaderCleanMarkdown(fields.bodyDraft || '', 520);
  return japanese
    ? [
        ...
        '| 項目 | 値 |',
        '| --- | --- |',
        `| 目的 | ${cmoLeaderTableCell(leaderSynthesis?.growth_goal || cmoLeaderSynthesisRequestField(leaderSynthesis, ['Goal', 'Objective', '目的']) || '一般認知の拡大', 160)} |`,
        `| 対象URL | ${cmoLeaderTableCell(targetUrl || '-', 180)} |`,
        `| 最初に承認する成果物 | ${cmoLeaderTableCell(perspective, 120)} |`,
        `| タイトル | ${cmoLeaderTableCell(title, 180)} |`,
        h1 ? `| H1/見出し | ${cmoLeaderTableCell(h1, 180)} |` : '',
        cta ? `| 主CTA | ${cmoLeaderTableCell(cta, 120)} |` : '',
        '| 実行状態 | 外部公開、投稿、送信、登録、計測アクセスは未実行 |',
        '| 承認条件 | 主張、CTA、公開面、計測イベント、停止条件をオーナーが確認 |',
        bodyDraft ? `| 本文/ドラフト抜粋 | ${cmoLeaderTableCell(bodyDraft, 260)} |` : ''
      ].filter(Boolean).join('\n')
    : [
        '## Review-surface fields',
        '| Field | Value |',
        '| --- | --- |',
        `| Goal | ${cmoLeaderTableCell(leaderSynthesis?.growth_goal || cmoLeaderSynthesisRequestField(leaderSynthesis, ['Goal', 'Objective']) || 'Increase awareness', 160)} |`,
        `| Target URL | ${cmoLeaderTableCell(targetUrl || '-', 180)} |`,
        `| First approval item | ${cmoLeaderTableCell(perspective, 120)} |`,
        `| Title | ${cmoLeaderTableCell(title, 180)} |`,
        h1 ? `| H1/headline | ${cmoLeaderTableCell(h1, 180)} |` : '',
        cta ? `| Primary CTA | ${cmoLeaderTableCell(cta, 120)} |` : '',
        '| Execution status | No external publishing, posting, sending, submission, or analytics access is claimed |',
        '| Approval condition | Owner approves claims, CTA, surface, measurement events, and stop rule |',
        bodyDraft ? `| Draft excerpt | ${cmoLeaderTableCell(bodyDraft, 260)} |` : ''
      ].filter(Boolean).join('\n');
}

function cmoLeaderFallbackLeaderMarkdown(leaderSynthesis = null, japanese = false) {
  const product = leaderSynthesis?.service_name
    || cmoLeaderSynthesisRequestField(leaderSynthesis, ['Product/service', 'Product', 'Service', 'Business', 'サービス', '商品'])
    || agentProviderHost(leaderSynthesis?.target_url)
    || (japanese ? '対象サービス' : 'the service');
  const goal = leaderSynthesis?.growth_goal
    || cmoLeaderSynthesisRequestField(leaderSynthesis, ['Goal', 'Main goal', 'Objective', '目的'])
    || (japanese ? '一般認知の拡大' : 'increase awareness');
  const packet = leaderSynthesis?.selected_review_packet || {};
  const selected = packet.perspective || (japanese ? '最初のレビュー用成果物' : 'the first review item');
  const targetUrl = leaderSynthesis?.target_url || '';
  const summary = japanese
    ? [
        '## 先に結論',
        `${product}${targetUrl ? `（${targetUrl}）` : ''} は、まず「何を自動化するサービスか」を自社ページで明確にし、${selected} を最初の承認対象にします。目的は ${goal} です。`,
        '複数の専門成果物は、計測・市場理解・チャネル選定・ページ/コピー作成の根拠として統合し、外部公開や投稿を実行済みとは書きません。'
      ].join('\n')
    : [
        '## Executive summary',
        `${product}${targetUrl ? ` (${targetUrl})` : ''} should first clarify what it automates on the owned surface, with ${selected} as the first approval item. Goal: ${goal}.`,
        'Prior specialist work is consolidated as evidence for measurement, market understanding, channel priority, and page/copy preparation; no external execution is claimed.'
      ].join('\n');
  return [
    japanese ? '# 認知拡大プラン' : '# Growth improvement plan',
    '',
    summary,
    '',
    cmoLeaderConfirmedFactsMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderSourceCoverageMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderOpenQuestionsMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderPriorityDiagnosisMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderPerspectiveReviewMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderChannelPriorityMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderPrimaryReviewMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderReviewPayloadMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderTwoWeekPlanMarkdown(leaderSynthesis, japanese),
    '',
    cmoLeaderMeasurementMarkdown(japanese),
    '',
    cmoLeaderNextInputsMarkdown(japanese)
  ].filter((item) => agentProviderText(item)).join('\n\n').replace(/\n{3,}/g, '\n\n').trim();
}

function cmoLeaderFallbackLeaderDelivery(kind = '', leaderSynthesis = null) {
  if (leaderSynthesis?.mode !== 'llm_leader_evaluation_required') return null;
  const japanese = cmoLeaderSynthesisIsJapanese(leaderSynthesis, '');
  const markdown = cmoLeaderFallbackLeaderMarkdown(leaderSynthesis, japanese);
  if (!markdown) return null;
  return {
    summary: japanese ? 'CMO Leader が認知拡大プランを統合しました。' : 'CMO Leader consolidated the growth plan.',
    reportSummary: japanese ? 'CMO final fallback synthesis' : 'CMO final fallback synthesis',
    bullets: japanese
      ? [
          '専門成果物を統合して1つの認知拡大プランにしました。',
          '最初の承認対象とレビュー画面に入れる項目を明示しました。',
          '外部公開、投稿、送信、登録、計測アクセスは未実行として扱いました。'
        ]
      : [
          'Integrated specialist outputs into one awareness plan.',
          'Named the first approval item and review-surface fields.',
          'Kept external execution unclaimed.'
        ],
    nextAction: japanese ? '最初の承認対象を確認し、計測イベントと公開面を決めてください。' : 'Review the first approval item, then confirm measurement events and publication surface.',
    fileMarkdown: markdown,
    contentType: 'cmo_leader_delivery',
    artifacts: cmoLeaderNormalizeLeaderArtifacts([], leaderSynthesis),
    approvalRequests: []
  };
}
...
function cmoLeaderNormalizeLeaderMarkdown(markdown = '', leaderSynthesis = null) {
  ...
  const reviewPayload = cmoLeaderReviewPayloadMarkdown(leaderSynthesis, japanese);
  let normalized = text
  ...
  normalized = cmoLeaderAppendSectionIfMissing(normalized, reviewPayload, ['Review-surface fields', 'レビュー画面に入れる項目']);
...
export function cmoLeaderNormalizeLeaderDelivery(kind = '', delivery = null, leaderSynthesis = null) {
  ...
  if (delivery.error) {
    return cmoLeaderFallbackLeaderDelivery(kind, leaderSynthesis) || delivery;
  }
