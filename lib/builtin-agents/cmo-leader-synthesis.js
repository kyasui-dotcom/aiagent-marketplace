...
function cmoLeaderNormalizeLeaderMarkdown(markdown = '', leaderSynthesis = null) {
  ...
  normalized = cmoLeaderAppendSectionIfMissing(normalized, coverage, ['Source coverage ledger', '情報ソースの充足状況']);
...
function cmoLeaderNormalizeLeaderArtifacts(artifacts = [], leaderSynthesis = null) {
  ...
  return sourceArtifacts.length
    ? sourceArtifacts.map((artifact) => {
        ...
        return {
          ...artifact,
          title: cmoLeaderNormalizeArtifactText(artifact.title || ''),
          summary: cmoLeaderNormalizeArtifactText(artifact.summary || ''),
          reason: cmoLeaderNormalizeArtifactText(artifact.reason || ''),
          body: cmoLeaderNormalizeArtifactText(artifact.body || ''),
          metadata: {
            ...metadata,
            prepared_in_chat: true,
