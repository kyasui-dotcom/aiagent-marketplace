...
const SEO_SPECIALIST_AGENT_PURPOSE = 'Own SEO analysis, rewrite, Search Console row-level intent mapping, article planning, article batch drafting after approval, and Publisher-ready packets inside this specialist.';
...
const SEO_SPECIALIST_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    prepares: Object.freeze(['row_coverage_ledger', 'query_to_landing_page_map', 'search_intent_cluster', 'intent_to_cta_mapping', 'seo_landing_priority_queue', 'missing_export_request_queue']),
    produces: Object.freeze(['search_console_landing_intent_map', 'search_console_row_coverage_ledger']),
    cannotClaim: Object.freeze(['Search Console data verified without export or connector proof', 'intent mapped from Search Console without query rows', 'landing priority proven without impressions/clicks context', 'query performance inferred for missing Search Console rows']),
    authorityBoundary: 'Search Console intent mapping is verified only for supplied query/page rows with date range, metrics, and property/source status; missing rows must stay unverified with exact export fields requested.'
  })
]);

const SEO_SPECIALIST_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['SEO mode', 'SERP/source status', 'Search Console source status', 'Search Console row coverage ledger', 'Keyword/page target', 'Search Console landing intent map', 'Rewrite or article spec', 'CTA/trust plan', 'Source ledger', 'Publisher/implementation handoff']),
  requiredEvidence: Object.freeze(['SERP status or unavailable label', 'target page/source status', 'Search Console query/page rows or missing export request', 'row-level query/page metric status for each mapped intent', 'Search Console date range and property status']),
  mustLabel: Object.freeze(['search unavailable', 'Search Console missing', 'Search Console row supplied', 'Search Console row missing', 'intent map unverified', 'batch approval needed', 'implementation gap']),
  forbiddenClaims: Object.freeze(['ranking improved', 'page updated', 'published', 'Publisher item created without proof', 'Search Console data verified without export or connector proof', 'intent mapped from Search Console without query rows', 'query performance inferred for missing Search Console rows', 'CTA priority proven without row-level metrics']),
  validDeliveryCheck: 'A valid SEO delivery is mode-specific, names source status, separates supplied-row intent from missing-row requests, maps Search Console query/page intent only when row evidence exists, and names the next handoff.'
});
...
const AGENT_DEFINITION = {
  ...
  "specialistMethod": [
    ...
    "When Search Console rows are missing or partial, keep those opportunities in a missing export queue with exact requested fields instead of creating a verified intent map.",
