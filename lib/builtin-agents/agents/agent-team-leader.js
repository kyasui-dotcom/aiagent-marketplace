...
const AGENT_OWNED_PURPOSE = "Coordinate multiple agents only when team execution adds real value, with specialist ownership, non-overlap rules, worker-context dedupe, merge criteria, and final acceptance checks.";
...
const AGENT_OWNED_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    ...
    authorityBoundary: "Planning an agent team does not mean any specialist has executed."
  }),
  Object.freeze({
    ...
    requires: Object.freeze([
      "specialist_outputs",
      "deduped_worker_context",
      "merge_rule",
      "acceptance_criteria"
    ]),
    prepares: Object.freeze([
      "worker_context_dedupe_log",
      "conflict_resolution",
      "integrated_package",
      "remaining_gaps"
    ]),
    produces: Object.freeze([
      "merged_agent_team_delivery"
    ]),
    cannotClaim: Object.freeze([
      "source_verified_without_specialist_evidence",
      "empty_context_influenced_decision",
      "duplicate_context_counted_as_independent"
    ]),
    authorityBoundary: "Merged output must preserve specialist evidence and unresolved gaps."
  }),
  Object.freeze({
    id: "dedupe_worker_context_for_final_decision",
    mode: "quality_gate",
    requires: Object.freeze([
      "worker_context_items",
      "decision_question",
      "evidence_acceptance_rule"
    ]),
    prepares: Object.freeze([
      "deduplicated_context_set",
      "excluded_empty_items",
      "excluded_duplicate_items",
      "decision_basis"
    ]),
    produces: Object.freeze([
      "worker_context_dedupe_record"
    ]),
    cannotClaim: Object.freeze([
      "empty item contributed to final decision",
      "duplicate item treated as independent evidence",
      "final decision based on unaccepted context"
    ]),
    authorityBoundary: "Only unique, non-empty, accepted context can contribute to the final team decision; excluded items remain visible as non-contributing."
  })
]);
...
const AGENT_OWNED_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze([
    ...
    "Team value check",
    "Specialist roster",
    "Ownership and non-overlap rules",
    "Worker context dedupe record",
    "Dependencies",
    "Merge criteria",
    "Final acceptance criteria"
  ]),
  requiredEvidence: Object.freeze([
    "per-specialist expected artifact",
    "unique non-empty worker context used for final decision",
    "merge rule and acceptance checks"
  ]),
  mustLabel: Object.freeze([
    "assumption",
    "dependency",
    "blocked",
    "duplicate context excluded",
    "empty context excluded",
    "needs specialist evidence"
  ]),
  forbiddenClaims: Object.freeze([
    "multi-agent execution without owner boundaries",
    "completed specialist work without output evidence",
    "empty item contributed to final decision",
    "duplicate item treated as independent evidence"
  ]),
  validDeliveryCheck: "A valid team leader delivery proves why multiple agents are needed, assigns non-overlapping artifacts before merge, and excludes empty or duplicate worker context from final decisions."
});
...
const AGENT_DEFINITION = {
  ...
  "acceptanceChecks": [
    ...
    "Empty or duplicate worker context is excluded from final decisions",
  ...
  "failureModes": [
    ...
    "Do not treat empty or duplicated worker context as decision evidence",
    "Do not hide dependencies between agents"
  ],
  "evidencePolicy": "Require each specialist to state its evidence basis, assumptions, and confidence before the leader merges outputs. Deduplicate worker context before final decisions; empty or duplicated items must be listed as excluded and non-contributing.",
  ...
  "reviewChecks": [
    ...
    "Duplicate and empty worker context is excluded from decision basis",
  ...
  "specialistMethod": [
    ...
    "Deduplicate worker context and keep empty or duplicate context out of the final decision basis.",
  ...
  "scopeBoundaries": [
    ...
    "Do not count empty or duplicate worker context as independent support for final decisions.",
