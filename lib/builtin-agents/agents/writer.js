const AGENT_PROVIDER = Object.freeze({
  ...
  async runJob({ kind = '', definition = {}, body = {}, source = {} } = {}) {
    ...
    const localPublisherHandoffs = agentProviderPublisherHandoffArtifacts(kind, definition, body, markdown);
    const generatedArtifacts = Array.isArray(generatedDelivery?.artifacts) ? generatedDelivery.artifacts : [];
    const publisherHandoffs = agentProviderEnrichPublisherHandoffArtifacts(generatedArtifacts, localPublisherHandoffs);
    return {
      accepted: true,
      status: 'completed',
...
function agentProviderClaimUseLedger(sources = [], body = {}) {
  const baseFacts = [
    {
      claim: `Audience: ${agentProviderAudience(body)}`,
      status: /the target audience/i.test(agentProviderAudience(body)) ? 'assumption' : 'approved_or_supplied',
      source: 'request brief',
      handoff_rule: 'Use only as audience targeting context; ask before naming a narrower segment.'
    },
    {
      claim: `Conversion action: ${agentProviderConversion(body)}`,
      status: /primary conversion/i.test(agentProviderConversion(body)) ? 'assumption' : 'approved_or_supplied',
      source: 'request brief',
      handoff_rule: 'Use as CTA direction, not as evidence that conversion performance exists.'
    }
  ];
  const sourceFacts = sources.slice(0, 6).map((source) => ({
    claim: source.snippet
      ? source.snippet.slice(0, 180)
      : `${source.title}${source.url ? ` (${source.url})` : ''}`,
    status: 'source_supplied_needs_review',
    source: source.title || source.url || source.source_type,
    handoff_rule: 'May inform draft wording after source ownership, attribution, and claim accuracy are reviewed.'
  }));
  return [...baseFacts, ...sourceFacts].filter((item) => item.claim);
}

function agentProviderClaimUseLedgerLines(ledger = []) {
  if (!ledger.length) return 'No claim ledger prepared. Treat all proof as missing until the requester supplies sources.';
  return ledger.map((item, index) => [
    `${index + 1}. Claim: ${item.claim}`,
    `   Status: ${item.status}`,
    `   Source: ${item.source}`,
    `   Downstream rule: ${item.handoff_rule}`
  ].join('\n')).join('\n');
}
...
function agentProviderPublisherBody(markdown = '', sources = [], profile = {}, body = {}, variants = []) {
  ...
  const claimLedger = agentProviderClaimUseLedger(sources, body);
  return [
    markdown,
    '',
    '## Publisher handoff',
    ...
    '',
    '## Claim use ledger',
    agentProviderClaimUseLedgerLines(claimLedger),
    '',
    '## Downstream handoff packet',
    `Next owner: ${profile.destination} operator or downstream channel agent`,
    `Allowed material: approved/source-reviewed draft variants, CTA direction, source ledger, and claim use ledger`,
    `Blocked material: unsupported metrics, testimonials, pricing, legal/compliance claims, or source snippets copied as final copy`,
    `Execution status: handoff prepared only; not ingested, queued, posted, sent, or published`,
    '',
    '## E-E-A-T usage notes',
...
function agentProviderPublisherHandoffArtifact(kind = '', definition = {}, body = {}, markdown = '', profile = null, index = 0) {
  ...
  return {
    ...
    claim_use_ledger: claimLedger,
    downstream_handoff: {
      next_owner: `${safeProfile.destination} operator or downstream channel agent`,
      allowed_material: ['approved draft variants', 'source ledger', 'claim use ledger', 'CTA direction', 'placement notes'],
      blocked_material: ['unsupported metrics', 'unapproved testimonials', 'pricing claims without source', 'legal/compliance claims without approval'],
      execution_status: 'handoff_prepared_not_ingested_not_published'
    },
    ...
    metadata: {
      ...
      claim_ledger_count: claimLedger.length,
      handoff_status: 'prepared_not_ingested_not_published',
...
function agentProviderEnrichPublisherHandoffArtifacts(generatedArtifacts = [], localArtifacts = []) {
  if (!generatedArtifacts.length) return localArtifacts;
  const localByKey = new Map(localArtifacts.map((item) => [`${item.channel_key || item.channelKey || ''}:${item.item_type || item.itemType || item.type || ''}`, item]));
  return generatedArtifacts.map((artifact, index) => {
    if (!artifact || typeof artifact !== 'object') return artifact;
    const key = `${artifact.channel_key || artifact.channelKey || ''}:${artifact.item_type || artifact.itemType || artifact.type || ''}`;
    const fallback = localByKey.get(key) || localArtifacts[index] || localArtifacts[0] || null;
    if (!fallback) return artifact;
    const body = agentProviderText(artifact.body || artifact.content || artifact.markdown);
    const enrichedBody = /##\s*Claim use ledger/i.test(body) && /##\s*Downstream handoff packet/i.test(body)
      ? body
      : fallback.body;
    return {
      ...fallback,
      ...artifact,
      body: enrichedBody,
      source_evidence: Array.isArray(artifact.source_evidence) && artifact.source_evidence.length ? artifact.source_evidence : fallback.source_evidence,
      claim_use_ledger: Array.isArray(artifact.claim_use_ledger) && artifact.claim_use_ledger.length ? artifact.claim_use_ledger : fallback.claim_use_ledger,
      downstream_handoff: artifact.downstream_handoff && typeof artifact.downstream_handoff === 'object' ? artifact.downstream_handoff : fallback.downstream_handoff,
      publish_variants: Array.isArray(artifact.publish_variants) && artifact.publish_variants.length ? artifact.publish_variants : fallback.publish_variants,
      eeat_notes: artifact.eeat_notes && typeof artifact.eeat_notes === 'object' ? artifact.eeat_notes : fallback.eeat_notes,
      metadata: {
        ...(fallback.metadata || {}),
        ...(artifact.metadata || {})
      }
...
const WRITER_AGENT_PURPOSE = 'Produce publishable copy packets, not copywriting advice, with audience, proof status, proof placeholder map, objection handling, CTA, variants, placement, and test notes.';

const WRITER_AGENT_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    id: 'prepare_copy_packet',
    mode: 'prepare_only',
    requires: Object.freeze(['audience', 'offer_or_message_goal', 'proof_or_placeholder_policy']),
    prepares: Object.freeze(['primary_copy', 'two_distinct_variants', 'proof_placeholder_map', 'cta', 'placement_notes']),
    produces: Object.freeze(['copy_packet']),
    cannotClaim: Object.freeze(['published', 'sent', 'proof-backed copy when proof is missing']),
    authorityBoundary: 'Publishing/sending remains with the relevant channel/app.'
  }),
  Object.freeze({
    id: 'prepare_publisher_handoff_copy',
    mode: 'handoff_only',
    requires: Object.freeze(['approved_copy', 'destination_surface', 'review_owner']),
    prepares: Object.freeze(['publisher_metadata', 'source_ledger', 'approval_notes']),
    produces: Object.freeze(['copy_publisher_handoff_packet']),
    cannotClaim: Object.freeze(['Publisher ingested', 'published']),
    authorityBoundary: 'Publisher ingest requires proof from Publisher.'
  }),
  Object.freeze({
    id: 'prepare_claim_safe_downstream_handoff',
    mode: 'handoff_only',
    requires: Object.freeze(['copy_packet', 'source_ledger_or_missing_proof_labels', 'downstream_owner']),
    prepares: Object.freeze(['claim_use_ledger', 'proof_placeholder_map', 'allowed_material_scope', 'blocked_claims', 'execution_status_labels']),
    produces: Object.freeze(['claim_safe_downstream_handoff_packet']),
    cannotClaim: Object.freeze(['claims verified without source review', 'proof placeholders approved as facts', 'downstream action completed', 'copy approved by owner']),
    authorityBoundary: 'Downstream agents may reuse only approved or explicitly labeled material; owner approval and connector proof remain separate.'
  })
]);

const WRITER_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Copy mode', 'Audience/stage', 'Proof status', 'Proof placeholder map', 'Primary version', 'Two distinct alternatives', 'CTA and placement', 'Claim use ledger', 'Downstream handoff packet', 'Revision test']),
  requiredEvidence: Object.freeze(['approved facts or placeholders for unsupported claims', 'audience or stage basis', 'source ledger or explicit missing-source label', 'handoff owner and allowed material scope', 'proof replacement condition for each placeholder']),
  mustLabel: Object.freeze(['missing proof', 'placeholder claim', 'approval needed', 'source supplied needs review', 'handoff prepared only', 'proof replacement pending']),
  forbiddenClaims: Object.freeze(['published', 'sent', 'external draft created without proof', 'claims verified without source review', 'copy approved by owner without evidence', 'testimonial, metric, customer proof, or outcome claim invented without source evidence', 'placeholder treated as approved proof without owner review']),
  validDeliveryCheck: 'A valid writer delivery includes actual copy, a proof placeholder map, a claim-use ledger, and a downstream handoff packet that separates approved, source-supplied, assumed, placeholder, and blocked claims.'
});

const AGENT_DEFINITION = {
  provider: AGENT_PROVIDER,
  agentPurpose: WRITER_AGENT_PURPOSE,
  agentActionBoundaries: WRITER_AGENT_ACTION_BOUNDARIES,
  deliveryContract: WRITER_DELIVERY_CONTRACT,
  ...
  "inputNeeds": [
    ...
    "Source ledger or missing-source status",
    "Distribution channel or surface",
    "Primary CTA",
    "Downstream owner or publishing path",
  ...
  "acceptanceChecks": [
    ...
    "Proof placeholder map names replacement evidence and downstream reuse status",
    "Claim use ledger separates approved, source-supplied, assumed, placeholder, and blocked claims",
    "Downstream handoff names the next owner, allowed material, blocked claims, and execution status",
  ...
  "failureModes": [
    ...
    "Do not write placeholder proof as if it is approved evidence",
    "Do not omit the CTA, placement note, or revision test"
  ],
  "evidencePolicy": "Ground copy in the supplied audience, awareness stage, offer, proof, objection, current copy, source ledger, and channel. If examples are used, state whether they are supplied examples, comparable patterns, assumptions, placeholders, or blocked claims. Pass downstream agents a claim-use ledger instead of making them infer what is safe to reuse.",
  "nextAction": "End with the recommended final copy, where each line should be placed, the downstream owner/status, and the first revision test or metric to watch.",
  ...
  "assumptionPolicy": "Assume the user wants publishable copy for the named surface. If awareness stage, proof, source ownership, downstream owner, or objection is missing, use a conservative default and label it. Do not invent claims to make the copy stronger.",
  "escalationTriggers": [
    "The copy depends on proof, pricing, or legal/compliance claims the user did not provide",
    "A downstream channel agent would need to reuse claims whose source status is unclear",
  ...
  "reviewChecks": [
    ...
    "Claim use ledger and downstream handoff are explicit",
    "No invented proof appears"
  ],
  "depthPolicy": "Default to one recommended version plus two alternatives, a claim use ledger, and one downstream handoff packet. Go deeper when audience segmentation, proof architecture, placement notes, or rewrite context materially changes the copy.",
  "concisionRule": "Avoid copywriting theory or generic messaging frameworks; deliver the actual copy, a short why, claim-use status, downstream handoff, placement notes, and the first test.",
  "toolStrategy": {
  ...
  "specialistMethod": [
    ...
    "When proof is missing, create a proof placeholder map with replacement evidence, owner review condition, and downstream reuse status.",
    "Create a claim use ledger before handoff: approved claims, source-supplied claims needing review, assumptions, placeholders, and blocked claims.",
    "Deliver one recommended publishable version plus strategically different alternatives, then name the downstream owner/status, first revision test, and placement notes."
  ],
  "scopeBoundaries": [
