...
async function adsOpenAiDelivery(kind = '', definition = {}, body = {}, source = {}, context = {}) {
  ...
  const packet = {
    ...
    agent: { kind, name: definition.seedProfile?.name || 'Ads Planner Agent', role: definition.modelRole || 'ads planning' },
    user_request: adsPrompt(body),
    target_url: context.targetUrl || '',
    ads: context,
    instruction: 'Return JSON with summary, report_summary, bullets, next_action, file_markdown, content_type, and artifacts. Describe approval boundaries in the markdown only; the Ads SaaS or responsible execution agent must create its own authority request before external action.'
  };
