function adsText(value = '', fallback = '') {
  const safe = String(value ?? '').trim();
  return safe || fallback;
}

function adsValueText(value = '', fallback = '', depth = 0) {
  if (value == null || depth > 5) return fallback;
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') return adsText(value, fallback);
  if (Array.isArray(value)) {
    const text = value.map((item) => adsValueText(item, '', depth + 1)).filter(Boolean).join('\n\n').trim();
    return text || fallback;
  }
  if (typeof value === 'object') {
    const preferredText = [
      value.markdown,
      value.content,
      value.body,
      value.text,
      value.value,
      value.file_markdown,
      value.fileMarkdown,
      value.output_text
    ].map((item) => adsValueText(item, '', depth + 1)).filter(Boolean).join('\n\n').trim();
    return preferredText || fallback;
  }
  return fallback;
}

function adsList(value = []) {
  return Array.isArray(value) ? value.map((item) => String(item || '').trim()).filter(Boolean) : [];
}

function adsObject(value = {}) {
  return value && typeof value === 'object' && !Array.isArray(value) ? value : {};
}

function adsPrompt(body = {}) {
  return adsText(body.prompt || body.full_prompt || body.input?.prompt || body.input?.order_prompt, 'No prompt provided.');
}

function adsJapanese(text = '') {
  return /[\u3040-\u30ff\u3400-\u9fff]/.test(String(text || ''));
}

function adsContext(body = {}) {
  const input = adsObject(body.input);
  const campaign = adsObject(body.campaign || input.campaign || input._broker?.campaign);
  const ads = adsObject(campaign.ads || body.ads || input.ads);
  const prompt = adsPrompt(body);
  return {
    prompt,
    objective: adsText(campaign.objective || ads.objective || prompt),
    audience: adsText(campaign.audience || ads.audience || input.audience),
    provider: adsText(ads.provider || body.provider || input.provider, 'google_ads'),
    budgetCap: Number(ads.budgetCap ?? ads.budget_cap ?? body.budgetCap ?? 0) || 0,
    targetCpa: Number(ads.targetCpa ?? ads.target_cpa ?? body.targetCpa ?? 0) || 0,
    targetUrl: adsText(campaign.targetUrl || campaign.target_url || ads.targetUrl || ads.target_url || input.target_url),
    conversionEvent: adsText(ads.conversionEvent || ads.conversion_event || body.conversionEvent || body.conversion_event),
    conversionTrackingStatus: adsText(ads.conversionTrackingStatus || ads.conversion_tracking_status || body.conversionTrackingStatus || body.conversion_tracking_status, 'unverified'),
    stopRules: adsList(ads.stopRules || ads.stop_rules)
  };
}

function adsJsonFromText(text = '') {
  const raw = adsText(text);
  const start = raw.indexOf('{');
  const end = raw.lastIndexOf('}');
  for (const candidate of [raw, start >= 0 && end > start ? raw.slice(start, end + 1) : '']) {
    if (!candidate) continue;
    try {
      const parsed = JSON.parse(candidate);
      if (parsed && typeof parsed === 'object') return parsed;
    } catch {}
  }
  return null;
}

function adsOpenAiText(payload = {}) {
  if (typeof payload === 'string') return payload;
  const direct = [
    payload.output_text,
    typeof payload.text === 'string' ? payload.text : '',
    payload.content
  ].map((item) => adsValueText(item)).find(Boolean);
  if (direct) return direct;
  return (Array.isArray(payload.output) ? payload.output : []).flatMap((item) => Array.isArray(item?.content) ? item.content : [])
    .map((part) => adsValueText(part?.text || part?.value || part?.content))
    .filter(Boolean)
    .join('\n');
}

async function adsOpenAiDelivery(kind = '', definition = {}, body = {}, source = {}, context = {}) {
  const apiKey = adsText(source.OPENAI_API_KEY || source.BUILTIN_OPENAI_API_KEY);
  if (!apiKey || typeof fetch !== 'function') return null;
  const baseUrl = adsText(source.OPENAI_BASE_URL || source.BUILTIN_OPENAI_BASE_URL, 'https://api.openai.com/v1').replace(/\/+$/, '');
  const packet = {
    requested_language: body.output_language || body.outputLanguage || 'Infer from user request.',
    agent: {
      kind,
      name: definition.seedProfile?.name || 'Ads Planner Agent',
      role: definition.modelRole || 'ads planning',
      purpose: definition.agentPurpose || '',
      action_boundaries: Array.isArray(definition.agentActionBoundaries) ? definition.agentActionBoundaries : [],
      delivery_contract: definition.deliveryContract || {},
      output_sections: adsList(definition.outputSections),
      acceptance_checks: adsList(definition.acceptanceChecks),
      tool_strategy: adsObject(definition.toolStrategy),
      specialist_method: adsList(definition.specialistMethod),
      scope_boundaries: adsList(definition.scopeBoundaries),
      depth_policy: definition.depthPolicy || null,
      concision_rule: definition.concisionRule || null
    },
    user_request: adsPrompt(body),
    target_url: context.targetUrl || '',
    ads: context,
    delivery_quality_gate: {
      required_sections: adsList(definition.deliveryContract?.requiredDeliverySections),
      required_evidence: adsList(definition.deliveryContract?.requiredEvidence),
      must_label: adsList(definition.deliveryContract?.mustLabel),
      forbidden_claims: adsList(definition.deliveryContract?.forbiddenClaims),
      valid_delivery_check: adsText(definition.deliveryContract?.validDeliveryCheck)
    },
    instruction: 'Return JSON with summary, report_summary, bullets, next_action, file_markdown, content_type, and artifacts. Respect delivery_quality_gate: include required sections when relevant, label missing evidence/status, and avoid forbidden claims. Describe approval boundaries in the markdown only; the Ads SaaS or responsible execution agent must create its own authority request before external action.'
  };
  const response = await fetch(`${baseUrl}/responses`, {
    method: 'POST',
    headers: { authorization: `Bearer ${apiKey}`, 'content-type': 'application/json' },
    body: JSON.stringify({
      model: adsText(source.OPENAI_MODEL || source.BUILTIN_OPENAI_MODEL, 'gpt-5.4-nano'),
      input: [
        { role: 'system', content: [{ type: 'input_text', text: definition.systemPrompt || '' }] },
        { role: 'user', content: [{ type: 'input_text', text: JSON.stringify(packet) }] }
      ]
    })
  });
  if (!response?.ok) return null;
  const generated = adsJsonFromText(adsOpenAiText(await response.json().catch(() => null) || {}));
  if (!generated?.file_markdown && !generated?.fileMarkdown) return null;
  return {
    summary: adsText(generated.summary, 'Ads plan prepared.'),
    reportSummary: adsText(generated.report_summary || generated.reportSummary, generated.summary || 'Ads plan prepared.'),
    bullets: adsList(generated.bullets),
    nextAction: adsText(generated.next_action || generated.nextAction),
    fileMarkdown: adsValueText(generated.file_markdown || generated.fileMarkdown),
    contentType: adsText(generated.content_type || generated.contentType, 'ads_plan'),
    artifacts: Array.isArray(generated.artifacts) ? generated.artifacts.filter((item) => item && typeof item === 'object') : []
  };
}

function adsFallbackMarkdown(context = {}, japanese = false) {
  const stopRules = context.stopRules.length ? context.stopRules.map((rule) => `- ${rule}`).join('\n') : '- Stop if spend, CPA, or compliance risk exceeds the approved cap.';
  if (japanese) {
    return [
      '# Ads plan',
      '',
      '## 目的',
      context.objective || '未指定。配信前に目的の確認が必要。',
      '',
      '## 配信前の計測ブロッカー',
      `- CVイベント: ${context.conversionEvent || '未指定。'}`,
      `- CV計測状態: ${context.conversionTrackingStatus || '未確認'}。`,
      '- 判定: CV計測がコネクタまたは管理画面証跡で確認されるまで、キャンペーン作成・入稿・配信・消化は不可。',
      '- 次の証跡: GA4/Ads conversion action、tag firing、test conversion、またはAds SaaSの計測確認ログ。',
      '',
      '## 対象',
      context.audience || '未指定。配信前に対象ユーザーの確認が必要。',
      '',
      '## 媒体',
      `${context.provider || '未指定'}。コネクタ状態: 未確認。`,
      '',
      '## キャンペーン構成',
      '- キャンペーン: 1 つの獲得目的を主要コンバージョンに紐づける。',
      context.targetUrl ? `- LP URL: ${context.targetUrl}` : '- LP URL: 未指定。承認済み URL がない限り開始不可。',
      '- 広告グループ: キーワードまたは訴求根拠が揃い次第、高意図の課題/オーディエンス単位で分割する。',
      '- アセット: 見出し、説明文、クリエイティブ、計測パラメータは承認まで草案扱い。',
      '',
      '## 予算上限と CPA 仮説',
      `- 予算上限: ${context.budgetCap || '未承認'}`,
      `- 目標 CPA: ${context.targetCpa || '未設定'}`,
      '- 出稿状態: 未承認かつ未消化。',
      '',
      '## 停止ルール',
      stopRules,
      '',
      '## クリエイティブアセット案',
      '- 訴求軸: 依頼文と対象ユーザーに基づく仮説。未検証の効果主張は使用不可。',
      '- 見出し/説明文: Ads SaaS投入前にブランド、法務、LP整合を確認する草案。',
      '- 否認リスク: 実績、比較、保証、価格表現は根拠がない限り未使用。',
      '',
      '## Ads SaaS 引き継ぎ',
      '- 実行前に必要: Ads アカウント接続、予算上限承認、アセット承認、LP URL、CV 計測、出稿承認者。',
      '- 引き継ぎ項目: 媒体、目的、対象、LP URL状態、予算上限、目標CPA、停止ルール、CVイベント、未解決項目。',
      '- 実行状態: 未作成、未入稿、未配信、未消化。',
      '',
      '## 承認と配信境界',
      '- 承認条件: 予算上限、停止ルール、LP URL、CV計測、広告文、出稿者を明示承認する。',
      '- Ads Plannerの範囲: 計画と引き継ぎのみ。Ads SaaSまたは実行担当が別途権限リクエストを作る。',
      '',
      '## 実行ステータスラベル',
      '- campaign_created: false / submitted: false / launched: false / spent: false。',
      `- connector_status: 未確認。conversion_tracking_status: ${context.conversionTrackingStatus || '未確認'}。`,
      '',
      '## 計測計画',
      '- 主要指標: 計測確認後の CPA または有効コンバージョン率。',
      '- 確認タイミング: 初回消化チェック、停止ルール判定、CV 品質チェック。',
      '- 担当: Campaign Operations が結果を集約し、継続・修正・停止を判断する。',
      '',
      '## Boundary',
      '- 広告費を使う前にSaaS接続、予算、停止条件、承認が必要。',
      '- 実際の広告作成/入札/配信はAds SaaSと明示承認を通す。'
    ].join('\n');
  }
  return [
    '# Ads plan',
    '',
    '## Objective',
    context.objective,
    '',
    '## Pre-launch measurement blocker',
    `- Conversion event: ${context.conversionEvent || 'missing.'}`,
    `- Conversion tracking status: ${context.conversionTrackingStatus || 'unverified'}.`,
    '- Decision: do not create, submit, launch, or spend until conversion tracking is verified by connector or admin proof.',
    '- Proof needed next: GA4/Ads conversion action, tag firing, test conversion, or Ads SaaS tracking-verification log.',
    '',
    '## Audience',
    context.audience || 'Not specified; confirm the buyer segment before launch.',
    '',
    '## Provider',
    `${context.provider || 'not specified'}. Connector status: not verified.`,
    '',
    '## Campaign structure',
    '- Campaign: one acquisition objective tied to the target conversion.',
    context.targetUrl ? `- Landing URL: ${context.targetUrl}` : '- Landing URL: missing; cannot launch until target URL is approved.',
    '- Ad groups: split by highest-intent audience/problem segment after keyword or creative evidence is supplied.',
    '- Assets: headlines, descriptions, creative, and tracking parameters remain draft-only until approved.',
    '',
    '## Budget cap and CPA assumption',
    `- Budget cap: ${context.budgetCap || 'not approved'}`,
    `- Target CPA: ${context.targetCpa || 'not set'}`,
    '- Spend status: not approved and not spent.',
    '',
    '## Stop rules',
    stopRules,
    '',
    '## Creative asset packet',
    '- Message angle: draft hypothesis based on the request and audience; do not use unverified performance claims.',
    '- Headlines/descriptions: draft-only until brand, legal, landing-page fit, and proof are approved.',
    '- Disapproval risk: claims about results, comparisons, guarantees, or pricing need evidence before use.',
    '',
    '## Ads SaaS handoff',
    '- Required before execution: connected Ads account, approved budget cap, approved assets, target URL, conversion tracking, and launch approver.',
    '- Handoff fields: provider, objective, audience, landing URL status, budget cap, target CPA, stop rules, conversion event, and unresolved gaps.',
    '- Execution status: not created, not submitted, not launched, not spent.',
    '',
    '## Approval and launch boundary',
    '- Approval condition: budget cap, stop rules, landing URL, conversion tracking, ad copy, and launch owner are explicitly approved.',
    '- Ads Planner scope: planning and handoff only; Ads SaaS or an execution owner must request separate authority.',
    '',
    '## Execution status labels',
    '- campaign_created: false / submitted: false / launched: false / spent: false.',
    `- connector_status: not verified. conversion_tracking_status: ${context.conversionTrackingStatus || 'not verified'}.`,
    '',
    '## Measurement plan',
    '- Primary metric: CPA or qualified conversion rate after tracking is verified.',
    '- Review cadence: first spend checkpoint, stop-rule checkpoint, and conversion-quality checkpoint.',
    '- Owner: Campaign Operations should collect results and decide continue, revise, or stop.',
    '',
    '## Boundaries',
    '- SaaS connection, budget, stop rules, and approval are required before spend.',
    '- Actual ad creation, bidding, or delivery must go through Ads SaaS and explicit approval.'
  ].join('\n');
}

const ADS_PLANNER_AGENT_PURPOSE = 'Prepare paid acquisition plans, budget guardrails, stop rules, and Ads SaaS handoff without launching, bidding, spending, pausing, or editing ads.';

const ADS_PLANNER_AGENT_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    id: 'prepare_ads_plan',
    mode: 'planning_only',
    requires: Object.freeze(['campaign_objective', 'audience', 'provider_or_provider_gap', 'budget_status']),
    prepares: Object.freeze(['campaign_structure', 'budget_guardrails', 'target_cpa_assumptions', 'stop_rules']),
    produces: Object.freeze(['ads_plan_packet']),
    cannotClaim: Object.freeze(['ads_launched', 'budget_spent', 'bids_changed']),
    authorityBoundary: 'Any spend or Ads SaaS write requires Ads SaaS authority, approved budget, exact assets, and launch confirmation.'
  }),
  Object.freeze({
    id: 'prepare_ads_saas_handoff',
    mode: 'handoff_only',
    requires: Object.freeze(['approved_plan', 'provider', 'budget_cap', 'stop_rules']),
    prepares: Object.freeze(['ads_saas_fields', 'approval_checklist']),
    produces: Object.freeze(['ads_saas_handoff_packet']),
    cannotClaim: Object.freeze(['campaign_created', 'campaign_submitted']),
    authorityBoundary: 'The handoff is not an execution receipt.'
  }),
  Object.freeze({
    id: 'prepare_creative_asset_packet',
    mode: 'draft_only',
    requires: Object.freeze(['campaign_objective', 'audience', 'landing_page_or_gap', 'claim_evidence_status']),
    prepares: Object.freeze(['message_angle', 'headline_description_drafts', 'claim_risk_labels']),
    produces: Object.freeze(['approval_ready_ad_asset_packet']),
    cannotClaim: Object.freeze(['creative_approved', 'policy_review_passed', 'performance_claim_verified']),
    authorityBoundary: 'Creative drafts remain unapproved until the owner or Ads SaaS supplies proof and approval.'
  }),
  Object.freeze({
    id: 'prepare_launch_approval_handoff',
    mode: 'approval_gate',
    requires: Object.freeze(['ads_saas_handoff_packet', 'budget_cap', 'stop_rules', 'conversion_tracking_status', 'launch_owner']),
    prepares: Object.freeze(['pre_launch_measurement_blocker', 'launch_approval_checklist', 'missing_execution_inputs', 'execution_status_labels']),
    produces: Object.freeze(['launch_approval_handoff_packet', 'measurement_blocker_packet']),
    cannotClaim: Object.freeze(['approval_granted', 'campaign_ready_to_launch', 'conversion_tracking_verified']),
    authorityBoundary: 'Launch readiness is a checklist, not permission to create, submit, launch, or spend.'
  })
]);

const ADS_PLANNER_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Objective', 'Pre-launch measurement blocker', 'Audience', 'Provider', 'Campaign structure', 'Budget cap and CPA assumption', 'Stop rules', 'Creative asset packet', 'Ads SaaS handoff', 'Approval and launch boundary', 'Execution status labels', 'Measurement plan']),
  requiredEvidence: Object.freeze(['target URL or missing URL label', 'budget approval status', 'conversion event and tracking status', 'tracking proof required before launch label', 'claim evidence status']),
  mustLabel: Object.freeze(['unapproved budget', 'missing provider', 'connector gap', 'unverified conversion tracking', 'tracking verification blocker', 'draft-only creative']),
  forbiddenClaims: Object.freeze(['created ads', 'launched ads', 'spent budget', 'changed bids', 'creative approved without owner proof', 'conversion tracking verified without connector proof', 'campaign ready to launch without approval', 'launchable campaign while conversion tracking is unverified']),
  validDeliveryCheck: 'A valid ads delivery is approval-gated, puts unverified conversion tracking in a pre-launch blocker, includes a draft creative packet and execution-status labels, and cannot read as an executed or launch-ready campaign.'
});

const AGENT_PROVIDER = Object.freeze({
  health({ kind = '', definition = {}, source = {} } = {}) {
    return {
      ok: true,
      service: 'ads_planner_agent',
      kind,
      mode: 'provider_contract',
      provider: 'agent_file',
      generation_provider: adsText(source.OPENAI_API_KEY || source.BUILTIN_OPENAI_API_KEY) ? 'agent_configured' : 'openai_unconfigured',
      file_name: definition.fileName || null,
      model_role: definition.modelRole || null,
      execution_layer: definition.executionLayer || null,
      task_types: adsList(definition.seedProfile?.taskTypes),
      capabilities: adsList(definition.seedProfile?.capabilities),
      agent_purpose: definition.agentPurpose || null,
      action_boundaries: Array.isArray(definition.agentActionBoundaries) ? definition.agentActionBoundaries : [],
      delivery_contract: definition.deliveryContract || {},
      tool_strategy: adsObject(definition.toolStrategy),
      specialist_method: adsList(definition.specialistMethod),
      scope_boundaries: adsList(definition.scopeBoundaries),
      depth_policy: definition.depthPolicy || null,
      concision_rule: definition.concisionRule || null
    };
  },

  async runJob({ kind = 'ads_planner', definition = {}, body = {}, source = {} } = {}) {
    const context = adsContext(body);
    if (context.prompt === 'No prompt provided.') {
      return { accepted: true, status: 'failed', error: 'missing_required_deliverable: ads planning requires a campaign brief or ads objective.', failure_category: 'missing_required_deliverable' };
    }
    const generated = await adsOpenAiDelivery(kind, definition, body, source, context);
    const japanese = adsJapanese(context.prompt);
    return {
      accepted: true,
      status: 'completed',
      summary: generated?.summary || (japanese ? '広告計画を作成しました。' : 'Ads plan prepared.'),
      report: {
        summary: generated?.reportSummary || 'Prepared an approval-gated ads plan.',
        bullets: generated?.bullets?.length ? generated.bullets : ['Spend requires approval.', 'Ads SaaS owns execution.', 'Campaign Operations owns measurement.'],
        nextAction: generated?.nextAction || 'Confirm provider, budget cap, target CPA, stop rules, and connector status before any spend.',
        ...(generated?.artifacts?.length ? { artifacts: generated.artifacts } : {})
      },
      files: [{ name: definition.fileName || 'ads-planner-delivery.md', type: 'text/markdown', content: generated?.fileMarkdown || adsFallbackMarkdown(context, japanese), source_task_type: kind, content_type: generated?.contentType || 'ads_plan' }],
      usage: { total_cost_basis: 7, compute_cost: 2, tool_cost: 1, labor_cost: 4, api_cost: 0 },
      return_targets: ['chat', 'api'],
      runtime: { mode: 'provider_contract', provider: 'agent_file', kind, service: 'ads_planner_agent', file_name: definition.fileName || null, generation_provider: 'openai_responses' }
    };
  }
});

const AGENT_DEFINITION = {
  provider: AGENT_PROVIDER,
  agentPurpose: ADS_PLANNER_AGENT_PURPOSE,
  agentActionBoundaries: ADS_PLANNER_AGENT_ACTION_BOUNDARIES,
  deliveryContract: ADS_PLANNER_DELIVERY_CONTRACT,
  fileName: 'ads-planner-delivery.md',
  healthService: 'ads_planner_agent',
  modelRole: 'paid acquisition planning, budget guardrails, stop rules, and Ads SaaS handoff',
  executionLayer: 'planning',
  taskRouting: {
    expansionTasks: ['research', 'writer', 'landing', 'data_analysis'],
    softMatchTokens: ['ads_planner', 'paid_ads', 'google_ads', 'meta_ads', 'advertising', 'paid_acquisition'],
    inferencePatterns: [/google ads/i, /meta ads/i, /paid ads/i, /advertising/i, /広告運用/, /広告出稿/, /広告配信/],
    inferenceScore: 25,
    tagHints: ['marketing', 'ads', 'paid_acquisition']
  },
  seedProfile: {
    id: 'agent_ads_planner_01',
    name: 'ADS PLANNER AGENT',
    description: 'Built-in ads planning agent that prepares approval-gated paid acquisition plans, budget caps, stop rules, and Ads SaaS handoff packets without directly spending or publishing.',
    taskTypes: ['ads_planner', 'paid_ads', 'google_ads', 'meta_ads', 'advertising', 'paid_acquisition', 'marketing'],
    successRate: 0.9,
    avgLatencySec: 14,
    executionPattern: 'async',
    inputTypes: ['text', 'url', 'campaign', 'connector_context'],
    outputTypes: ['markdown', 'json', 'ads_plan'],
    clarification: 'multi_turn',
    capabilities: ['ads_plan', 'budget_guardrails', 'stop_rules', 'creative_asset_packet', 'ads_saas_handoff', 'launch_approval_handoff', 'measurement_plan'],
    metadata: {
      layer: 'planning',
      adapter_role: 'ads_planner',
      approval_mode: 'human_before_spend',
      provider_connectors_required_for_execution: ['ads_saas'],
      output_contract: ['objective', 'pre_launch_measurement_blocker', 'audience', 'provider', 'campaign_structure', 'budget_cap_and_cpa_assumption', 'stop_rules', 'creative_asset_packet', 'ads_saas_handoff', 'approval_and_launch_boundary', 'execution_status_labels', 'measurement_plan']
    }
  },
  systemPrompt: 'You are the built-in Ads Planner Agent in AIagent2. Prepare paid acquisition plans, campaign structure, budget caps, target CPA, stop rules, draft creative asset packets, approval boundaries, execution-status labels, and Ads SaaS handoff requirements. Put unverified conversion tracking in a top pre-launch blocker before campaign structure, budget, or creative recommendations, and state the proof needed before launch readiness. Never claim that ads were created, submitted, budgeted, bid, launched, paused, edited, approved, launch-ready, or conversion-tracking verified unless a connected Ads SaaS explicitly reports it. Do not spend money or request broad authority for another agent. The Ads SaaS or the responsible ads execution agent must request its own authority before external action.',
  deliverableHint: 'Deliver in the user requested language in a clear, user-readable format.',
  reviewHint: 'Keep the plan execution-ready but approval-gated: provider, budget, stop rules, measurement, and Ads SaaS handoff must be explicit.',
  outputSections: [
    'Objective',
    'Pre-launch measurement blocker',
    'Audience',
    'Provider',
    'Campaign structure',
    'Budget cap and CPA assumption',
    'Stop rules',
    'Creative asset packet',
    'Ads SaaS handoff',
    'Approval and launch boundary',
    'Execution status labels',
    'Measurement plan'
  ],
  inputNeeds: [
    'Campaign objective or conversion goal',
    'Target audience',
    'Target URL or landing-page gap',
    'Provider preference or provider gap',
    'Budget cap and approval status',
    'Stop rules or risk tolerance',
    'Conversion event and tracking status',
    'Claim evidence or creative approval gaps'
  ],
  acceptanceChecks: [
    'Budget and CPA are labeled as approved, assumed, or missing',
    'Provider and connector status are explicit',
    'Stop rules are concrete enough for an execution agent',
    'Creative copy is labeled draft-only with claim evidence gaps',
    'Unverified conversion tracking appears as a pre-launch blocker before launch handoff',
    'Ads SaaS handoff includes fields another execution surface can use',
    'Execution status labels state not created, not submitted, not launched, and not spent unless connector proof exists',
    'Measurement owner and conversion event are named',
    'No launch, spend, bid, campaign-created, creative-approved, or tracking-verified claim appears without connector proof'
  ],
  toolStrategy: {
    web_search: 'only_when_competitor_or_current_ads_benchmark_changes_plan',
    source_mode: 'campaign_brief_ads_saas_context_and_measurement_inputs',
    note: 'Use supplied campaign and Ads SaaS context first. If budget, account, or conversion tracking is missing, return a handoff gap instead of inventing execution readiness.'
  },
  specialistMethod: [
    'Convert the request into one paid acquisition objective and one primary conversion event.',
    'If conversion tracking is missing or unverified, put that blocker before launch, spend, and campaign-structure details.',
    'Label provider, account, budget, target URL, measurement, and connector status before proposing spend.',
    'Prepare campaign structure, budget guardrails, target CPA assumptions, stop rules, and approval gates as separate sections.',
    'Draft creative assets with explicit claim-evidence and approval labels.',
    'Return an Ads SaaS handoff packet that another authorized execution surface can use without treating the plan as already launched.'
  ],
  scopeBoundaries: [
    'Do not spend money, create ads, change bids, or launch campaigns directly.',
    'Do not bypass budget, stop-rule, account, or compliance approval.',
    'Do not present the campaign as launchable while conversion tracking is missing or unverified.',
    'Do not claim creative, tracking, or launch readiness without owner or connector evidence.',
    'Do not make unsupported claims in ads copy.'
  ],
  depthPolicy: 'Default to one launchable campaign lane with explicit budget, stop rules, and measurement. Go deeper only when multiple providers, audiences, or conversion events are explicitly in scope.',
  concisionRule: 'Avoid broad paid-media lectures; state the plan, guardrails, execution gaps, handoff packet, and next approval.'
};

AGENT_DEFINITION.manifest = Object.freeze({
  schema_version: 'agent-manifest/v1',
  kind: 'ads_planner',
  aliases: ['paid_ads', 'google_ads', 'meta_ads', 'advertising', 'paid_acquisition'],
  name: AGENT_DEFINITION.seedProfile.name,
  description: AGENT_DEFINITION.seedProfile.description,
  agent_role: 'worker',
  task_types: AGENT_DEFINITION.seedProfile.taskTypes,
  capabilities: AGENT_DEFINITION.seedProfile.capabilities,
  healthcheckUrl: '/sample-agents/ads_planner/health',
  healthcheck_url: '/sample-agents/ads_planner/health',
  jobEndpoint: '/sample-agents/ads_planner/jobs',
  job_endpoint: '/sample-agents/ads_planner/jobs',
  endpoints: Object.freeze({ health: '/sample-agents/ads_planner/health', jobs: '/sample-agents/ads_planner/jobs' }),
  metadata: Object.freeze({
    agent_purpose: AGENT_DEFINITION.agentPurpose,
    action_boundaries: AGENT_DEFINITION.agentActionBoundaries,
    delivery_contract: AGENT_DEFINITION.deliveryContract,
    tool_strategy: AGENT_DEFINITION.toolStrategy,
    specialist_method: AGENT_DEFINITION.specialistMethod,
    scope_boundaries: AGENT_DEFINITION.scopeBoundaries,
    depth_policy: AGENT_DEFINITION.depthPolicy,
    concision_rule: AGENT_DEFINITION.concisionRule,
    sample: true,
    sampleKind: 'ads_planner',
    sample_kind: 'ads_planner',
    category: 'ads_planner',
    provider: 'agent_file',
    execution_scope: 'agent_file_manifest',
    workflow_layer: 'planning',
    routable: true,
    externalProviderRequired: false,
    external_provider_required: false
  })
});

export default Object.freeze(AGENT_DEFINITION);
