...
function adsFallbackMarkdown(context = {}, japanese = false) {
  ...
  if (japanese) {
    return [
      ...
      '- connector_status: 未確認。conversion_tracking_status: 未確認。',
      '',
      '## 計測計画',
  ...
  return [
    ...
    '- campaign_created: false / submitted: false / launched: false / spent: false.',
    '- connector_status: not verified. conversion_tracking_status: not verified.',
    '',
    '## Measurement plan',
...
const ADS_PLANNER_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    prepares: Object.freeze(['launch_approval_checklist', 'missing_execution_inputs', 'execution_status_labels']),
    produces: Object.freeze(['launch_approval_handoff_packet']),
...
const ADS_PLANNER_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Objective', 'Audience', 'Provider', 'Campaign structure', 'Budget cap and CPA assumption', 'Stop rules', 'Creative asset packet', 'Ads SaaS handoff', 'Approval and launch boundary', 'Execution status labels', 'Measurement plan']),
  requiredEvidence: Object.freeze(['target URL or missing URL label', 'budget approval status', 'conversion tracking status', 'claim evidence status']),
  mustLabel: Object.freeze(['unapproved budget', 'missing provider', 'connector gap', 'unverified conversion tracking', 'draft-only creative']),
  forbiddenClaims: Object.freeze(['created ads', 'launched ads', 'spent budget', 'changed bids', 'creative approved without owner proof', 'conversion tracking verified without connector proof', 'campaign ready to launch without approval']),
  validDeliveryCheck: 'A valid ads delivery is approval-gated, includes a draft creative packet and execution-status labels, and cannot read as an executed or launch-ready campaign.'
});
...
const AGENT_DEFINITION = {
  ...
  seedProfile: {
    ...
    metadata: {
      ...
      output_contract: ['objective', 'audience', 'provider', 'campaign_structure', 'budget_cap_and_cpa_assumption', 'stop_rules', 'creative_asset_packet', 'ads_saas_handoff', 'approval_and_launch_boundary', 'execution_status_labels', 'measurement_plan']
    }
  },
  systemPrompt: 'You are the built-in Ads Planner Agent in AIagent2. Prepare paid acquisition plans, campaign structure, budget caps, target CPA, stop rules, draft creative asset packets, approval boundaries, execution-status labels, and Ads SaaS handoff requirements. Never claim that ads were created, submitted, budgeted, bid, launched, paused, edited, approved, or conversion-tracking verified unless a connected Ads SaaS explicitly reports it. Do not spend money or request broad authority for another agent. The Ads SaaS or the responsible ads execution agent must request its own authority before external action.',
