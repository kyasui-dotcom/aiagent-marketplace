const AGENT_PROVIDER = Object.freeze({
  ...
  async runJob({ kind = '', definition = {}, body = {}, source = {} } = {}) {
    ...
    let delivery = await agentProviderGenerateDelivery(kind, definition, body, source, { webSources, leaderSynthesis });
...
async function agentProviderGenerateDelivery(kind = '', definition = {}, body = {}, source = {}, options = {}) {
  ...
  const requestPacket = {
    ...
    output_contract: leaderEvaluationRequired
      ? 'Return JSON with summary, report_summary, bullets, next_action, file_markdown, content_type, artifacts, and approval_requests. file_markdown must be one end-user-facing growth plan, not an orchestration log. It must hide internal agent names, task ids, handoff labels, connector/app ingestion status, Publisher/SaaS terms, and snake_case keys. Translate prior work into business-readable perspectives such as access analytics, market research, channel planning, SEO/page copy, landing-page review, and source/list work. Separate confirmed facts, open questions, priority diagnosis, recommended actions, 2-week execution plan, and inputs needed next. Set content_type to cmo_leader_delivery.'
      : 'Return only user-facing delivery text and structured handoff data. Respect delivery_quality_gate: include required sections when relevant, label missing evidence/status, and avoid forbidden claims. Do not include workflow prompts, provider implementation notes, raw handoff context, or internal QA text.'
  };
  const systemText = leaderEvaluationRequired
    ? 'You are writing a final CMO growth recommendation for the business owner. Judge the prior work yourself and integrate only the useful evidence, but never expose internal agent names, specialist matrices, handoff summaries, connector context labels, Publisher/SaaS status, app ingestion status, task ids, or snake_case fields in the Markdown. The user should see one polished plan: executive summary, confirmed facts, what cannot yet be judged, priority diagnosis, recommended actions, 2-week execution plan, and inputs needed next. If no external execution proof exists, simply avoid claiming publishing, posting, scheduling, sending, repository writes, or app execution. Return JSON only because the request packet explicitly asks for structured data; the file_markdown value itself must be clean end-user prose.'
...
function cmoLeaderEvaluationContext(kind = '', definition = {}, body = {}, options = {}) {
  ...
  return {
    ...
    evaluation_contract: [
      ...
      'State which agent outputs were adopted, held, or rejected, with short reasons.',
      'Separate verified facts, assumptions, gaps, and blocked actions.',
      'Do not invent metrics, proof, testimonials, trial status, or external execution.',
      'Any Publisher material is a chat-prepared handoff draft unless publisher_ingest_proof is present.'
    ],
    required_user_facing_sections: japanese
      ? ['先に結論', '採用判断表', '統合版の最終提案', 'Publisher下書き状態', '根拠と未確認事項', '次のアクション']
      : ['Direct answer', 'Adoption matrix', 'Integrated final recommendation', 'Publisher draft status', 'Evidence and gaps', 'Next actions'],
    handoff_boundary: {
...
function cmoLeaderEnsureCheckpointSynthesisMarkdown(markdown = '', leaderSynthesis = null) {
  ...
  const note = japanese
    ? [
        '## リーダー統合メモ',
        `- 完了済みのspecialist成果物を根拠として確認し、次担当 **${nextOwner}** にhandoffします。`,
        '- このcheckpointは実行や公開ではなく、prior specialist evidenceを次レイヤーへ渡す判断です。'
      ].join('\n')
    : [
        '## Leader synthesis note',
        `- Prior completed specialist work products were reviewed as evidence and handed off to **${nextOwner}**.`,
        '- This checkpoint is a synthesis/approval gate for the next layer, not external execution or publishing.'
      ].join('\n');
...
const CMO_LEADER_AGENT_PURPOSE = 'Lead marketing strategy and specialist orchestration while keeping channel execution, Publisher ingest, and SaaS actions owned by the responsible agent or app.';
...
const CMO_LEADER_AGENT_ACTION_BOUNDARIES = Object.freeze(Object.entries(CMO_AGENT_ACTION_CONTRACTS).map(([taskType, contract]) => Object.freeze({
  ...
  prepares: Object.freeze([contract.deliverable]),
  produces: Object.freeze([`${taskType}_leader_handoff_packet`]),
...
const CMO_LEADER_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['ICP and conversion goal', 'Positioning promise', 'Evidence and gaps', 'Specialist adoption matrix', 'Planned action queue', 'Publisher/SaaS handoff status', 'Next owner']),
  requiredEvidence: Object.freeze(['source or specialist outputs used', 'publisher ingest proof when claimed']),
  mustLabel: Object.freeze(['assumptions', 'blocked actions', 'handoff draft only']),
  forbiddenClaims: Object.freeze(['published', 'Publisher item created without proof', 'external app ingested without proof']),
  validDeliveryCheck: 'A valid CMO delivery separates strategy, specialist preparation, handoff drafts, and executed proof.'
});
...
const AGENT_DEFINITION = {
  ...
  "outputSections": [
    ...
    "Lane decision memo",
    "Capability-based specialist dispatch packets",
    "Leader approval queue",
    "Manifest-matched SaaS app handoff packet",
  ...
  "inputNeeds": [
    ...
    "Registered agent manifests for the relevant layers",
    "App manifests or SaaS handoff availability",
    "What the leader can approve for the matched SaaS app surface"
  ],
  "acceptanceChecks": [
    ...
    "Each planned action names owner/capability, artifact, matched SaaS app surface, and stop rule",
    "Specialist selection is described as manifest/capability based rather than sample-agent priority",
  ...
  "nextAction": "End with the chosen media, why it beats the next-best lane, the capability-based specialist dispatch packets, the leader approval queue, the concrete matched-SaaS-app handoff packet, the first campaign experiment, and the growth metric.",
  ...
  "escalationTriggers": [
    ...
    "Leader approval rules or manifest-matched SaaS app handoff readiness are unclear",
  ...
  "minimumQuestions": [
    ...
    "Which app surface and assets are actually ready for this lane?",
    "Which specialist outputs should the leader be allowed to approve or route to the manifest-matched SaaS app surface?",
  ...
  "reviewChecks": [
    ...
    "Chosen lane, approval owner, matched app surface, and next artifact are explicit",
    "Metric is explicit"
  ],
  "depthPolicy": "Default to ICP, positioning, one chosen media lane, one leader approval queue, and one planned action table. Go deeper when specialist briefs, writing order, manifest-matched SaaS app readiness, and competitor/channel evidence must align.",
  "concisionRule": "Avoid generic marketing frameworks; tie each tactic to ICP, positioning, proof, metric, research evidence, and the leader-to-app-handoff path. Prefer short packet-style rows over loose strategy prose when execution is implied.",
  "toolStrategy": {
    "web_search": "default",
  ...
  "specialistMethod": [
    ...
    "Collect specialist drafts back into a leader approval queue so the leader decides what gets routed to the manifest-matched SaaS app surface or operators.",
  ...
  "scopeBoundaries": [
    ...
    "Do not let specialists imply autonomous publishing when the manifest-matched SaaS app surface owns the external publish step.",
