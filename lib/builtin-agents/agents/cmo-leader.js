...
async function agentProviderGenerateDelivery(kind = '', definition = {}, body = {}, source = {}, options = {}) {
  ...
  const systemText = leaderEvaluationRequired
    ? 'You are writing a final CMO growth recommendation for the business owner. Judge the prior work yourself and integrate only the useful evidence, but never expose internal agent names, specialist matrices, handoff summaries, connector context labels, Publisher/SaaS status, app ingestion status, task ids, or snake_case fields in the Markdown. The user should see one polished plan: executive summary, confirmed facts, source coverage ledger, what cannot yet be judged, priority diagnosis, recommended actions, channel priority table, 2-week execution plan, measurement checklist, and inputs needed next. The source coverage ledger must cover GA4, Search Console, current landing-page copy, proof assets, existing article/content inventory, CRM/sales data, and prior specialist outputs with supplied/missing/stale/not accessed labels, then connect those labels to the priority recommendation. Do not repeat the same baseline or caveat in multiple sections. If no external execution proof exists, simply avoid claiming publishing, posting, scheduling, sending, repository writes, app handoff completion, or app execution. Treat unapproved social/Publisher material as review-only supporting material, not a completed handoff. Return JSON only because the request packet explicitly asks for structured data; the file_markdown value itself must be clean end-user prose.'
