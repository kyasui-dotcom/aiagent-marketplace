...
async function agentProviderGenerateDelivery(kind = '', definition = {}, body = {}, source = {}, options = {}) {
  ...
  const requestPacket = {
    ...
    output_contract: leaderEvaluationRequired
      ? 'Return JSON with summary, report_summary, bullets, next_action, file_markdown, content_type, artifacts, and approval_requests. file_markdown must be the user-facing CMO leader synthesis. It must evaluate all specialist outputs, explain adopted/held/rejected work, remove duplicates, separate verified facts from assumptions, and label any Publisher material as a chat-prepared handoff draft only unless publisher_ingest_proof is present. Set content_type to cmo_leader_delivery.'
      : 'Return only user-facing delivery text and structured handoff data. Do not include workflow prompts, provider implementation notes, raw handoff context, or internal QA text.'
  };
