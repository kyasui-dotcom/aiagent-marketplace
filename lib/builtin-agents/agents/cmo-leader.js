...
export const CMO_TASK_INFERENCE_RULES = Object.freeze([
  ...
  Object.freeze({ taskType: 'seo_gap', patterns: Object.freeze([/(content gap|seo gap|keyword gap|search intent|キーワードギャップ|コンテンツギャップ|検索意図)/i]) })
]);
...
export function cmoPreparationTasksForActions(actions = [], text = '') {
  ...
  for (const task of ['writing', 'writer', 'seo_gap', 'landing', 'list_creator', 'reddit', 'indie_hackers']) {
...
export function cmoNormalizeWorkflowPlannedTasks(context = {}) {
  ...
  if (/(directory|listing|citation|掲載|ディレクトリ|サイテーション)/i.test(text)) push('seo_gap');
...
export function defaultCmoActionTaskFromText(text = '') {
  ...
  if (/(directory|listing|citation|掲載媒体|媒体掲載|ディレクトリ|サイテーション)/i.test(safe)) return 'seo_gap';
  return '';
}
...
export function cmoRequestedActionTasksFromText(text = '') {
  ...
  if (signals.directoryExecutionIntent) push('seo_gap');
  if (signals.citationExecutionIntent) push('seo_gap');
...
export function cmoWorkflowTaskSignalScore(task = '', text = '') {
  ...
  const signalPatterns = {
    seo_gap: [/seo|serp|organic|search intent|owned\/search|検索|自然検索|検索流入|メタ|meta title|h1|内部リンク|比較lp|比較ページ/i],
...
export function cmoWorkflowReplanSelection(candidateTasks = [], sourceText = '', layer = 1, actionLayerStart = 5) {
...
export function cmoInferTaskSequence(context = {}) {
  ...
  if (isCmoExternalExecutionIntent(taskType, prompt) || isAgentTeamLaunchIntent(taskType, prompt)) {
    ['growth', 'seo_gap', 'landing', 'writing'].forEach(pushUniqueExpanded);
    if (signals.directoryExecutionIntent || signals.citationExecutionIntent) pushUniqueExpanded('seo_gap');
  ...
  if (signals.directoryExecutionIntent) pushRequestedExecutor('seo_gap');
  if (signals.citationExecutionIntent) pushRequestedExecutor('seo_gap');
  ...
  if (expandedTeam.includes('writing') && expandedTeam.includes('list_creator')) {
    ...
    const insertAt = Math.max(reordered.indexOf('landing'), reordered.indexOf('seo_gap')) + 1;
...
export function cmoEnsureLeaderWorkflowActionTasks(context = {}) {
  ...
  const planningPreferences = requestedActions.some((task) => ['seo_gap', 'landing', 'writing'].includes(task)) || plannerCandidateActions.length || cmoMediaPlanningPreferredFromText(text)
...
export const CMO_LEADER_BEHAVIOR = Object.freeze({
  ...
  replanSelection: cmoWorkflowReplanSelection,
...
const AGENT_DEFINITION = {
  ...
  "deliverableHint": "Write sections for marketing objective, structured handoff digest, research and evidence first pass, ICP, competitor and channel diagnosis, positioning, chosen media and why, lane decision memo, specialist dispatch packets, leader approval queue, manifest-matched SaaS app handoff packet, planned action table, metrics, risks, and next action.",
