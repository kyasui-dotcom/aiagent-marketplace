...
const CMO_AWARENESS_EXPANSION_PATTERN = /(brand awareness|awareness|general recognition|public recognition|market recognition|認知(?:度)?[^\n。.!?]{0,24}(?:上げ|高め|広げ|拡大|獲得|形成|増や|伸ば|改善|強化|もっていく|持っていく)|(?:一般的|広い|幅広い)[^\n。.!?]{0,16}認知(?:度)?|知名度[^\n。.!?]{0,24}(?:上げ|高め|広げ|拡大|獲得|形成|増や|伸ば|改善|強化|もっていく|持っていく))/i;
const CMO_LEADER_INTENT_PATTERN = /(cmo|chief marketing|marketing leader|free web growth|free marketing|organic growth|organic acquisition|customer\s+(?:acquisition|aquitisi?tion|acquistion)|lead\s+gen(?:eration)?|more\s+(?:customers|sales|revenue)|increase\s+(?:customers|sales|revenue)|need\s+more\s+sales|no[-\s]?ads?|without ads|agent team|launch team|multi[-\s]?agent launch|cross[-\s]?channel|all channels|マーケ責任者|cmo的|マーケ部長|広告費.*(なし|使わない|ゼロ)|無料.*(web|ウェブ|施策|集客|流入|マーケ|SEO)|集客(?:したい|を(?:増や|伸ば|改善|強化)|施策|戦略)|(?:ユーザー|登録|問い合わせ|リード|流入|認知).*(?:増や|伸ば|獲得|改善|強化)|複数.*(agent|エージェント).*告知|まとめて.*(告知|投稿|発信))/i;
...
export const CMO_TASK_INFERENCE_RULES = Object.freeze([
  ...
  Object.freeze({ taskType: 'cmo_leader', score: 85, patterns: Object.freeze([CMO_AWARENESS_EXPANSION_PATTERN]) }),
...
export function cmoLeaderTaskTypeForText(text = '') {
  ...
  return CMO_LEADER_INTENT_PATTERN.test(source) || cmoAwarenessExpansionIntentFromText(source) ? 'cmo_leader' : '';
}
...
export function cmoAwarenessExpansionIntentFromText(text = '') {
  return CMO_AWARENESS_EXPANSION_PATTERN.test(String(text || ''));
}

export function cmoMediaPlanningPreferredFromText(text = '') {
  const source = String(text || '');
  return cmoAwarenessExpansionIntentFromText(source)
    || /(priority channels?|preferred channels?|channel mix|media mix|referral sites?|directories?|directory listing|sns|social media|social\b|community|communities|媒体|チャネル|優先チャネル|優先媒体|紹介サイト|外部掲載|掲載先|SNS|ソーシャル|コミュニティ)/i.test(source);
}
...
export function cmoPlannerCandidateActionTasksFromText(taskType = '', text = '') {
  ...
  const awarenessIntent = cmoAwarenessExpansionIntentFromText(source);
  ...
  push('landing');
  push('writing');
  if (awarenessIntent) push('seo_specialist');
...
export function defaultCmoActionTaskFromText(text = '') {
  const safe = String(text || '').trim();
  if (cmoAwarenessExpansionIntentFromText(safe)) return 'writing';
