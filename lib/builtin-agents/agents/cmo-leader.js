...
async function agentProviderGenerateDelivery(kind = '', definition = {}, body = {}, source = {}, options = {}) {
  ...
  const requestPacket = {
    ...
    output_contract: leaderEvaluationRequired
      ? 'Return JSON with summary, report_summary, bullets, next_action, file_markdown, content_type, artifacts, and approval_requests. file_markdown must be the user-facing CMO leader synthesis. It must evaluate all specialist outputs, explain adopted/held/rejected work, remove duplicates, separate verified facts from assumptions, respect delivery_quality_gate, and label any Publisher material as a chat-prepared handoff draft only unless publisher_ingest_proof is present. Set content_type to cmo_leader_delivery.'
  ...
  const systemText = leaderEvaluationRequired
    ? 'You are the CMO team leader. Judge the specialist outputs yourself; do not accept a preselected lane unless the evidence supports it. Integrate the best parts into one final user-facing delivery in the requested language. Include a clear adoption matrix showing which agent output was adopted, held, or rejected and why. Do not claim Publisher ingestion, packet creation inside an external app, posting, sending, publishing, repository writes, or connector execution unless explicit proof is included. Phrases such as "Landing page change packet prepared" or "Publish status: prepared / not published" are forbidden without Publisher ingest proof; use "Publisher handoff draft prepared in chat", "External app ingest status: not verified", and "Publish status: not published" instead. Return JSON only because the request packet explicitly asks for structured handoff data.'
