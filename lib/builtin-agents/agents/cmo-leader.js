...
async function agentProviderGenerateDelivery(kind = '', definition = {}, body = {}, source = {}, options = {}) {
  ...
  const requestPacket = {
    ...
    output_contract: leaderEvaluationRequired
      ? 'Return JSON with summary, report_summary, bullets, next_action, file_markdown, content_type, artifacts, and approval_requests. file_markdown must be one end-user-facing growth plan, not an orchestration log or a bundle index. It must hide internal agent names, task ids, handoff labels, connector/app ingestion status, Publisher/SaaS terms, and snake_case keys. Translate prior work into business-readable perspectives such as access analytics, market research, channel planning, SEO/page copy, landing-page review, and source/list work. Deduplicate repeated facts and caveats: state shared analytics baselines, missing inputs, and approval blockers once, then explain what they mean. Separate confirmed facts, open questions, priority diagnosis, recommended actions, 2-week execution plan, and inputs needed next. If a social or Publisher-style packet lacks approved CTA/assets/proof or app ingest proof, label it as review-only supporting material and do not call it handed off, ready, queued, posted, or published. Set content_type to cmo_leader_delivery.'
...
function cmoLeaderEvaluationContext(kind = '', definition = {}, body = {}, options = {}) {
  ...
  return {
    ...
    required_user_facing_sections: japanese
      ? ['先に結論', '確認済みの事実', 'まだ判断できないこと', '優先診断', '推奨アクション', '2週間の実行計画', '次に必要な情報']
      : ['Executive summary', 'Confirmed facts', 'Open questions', 'Priority diagnosis', 'Recommended actions', '2-week execution plan', 'Inputs needed next'],
    handoff_boundary: {
...
const CMO_LEADER_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Executive summary', 'Confirmed facts', 'Open questions', 'Priority diagnosis', 'Recommended actions', '2-week execution plan', 'Inputs needed next']),
  requiredEvidence: Object.freeze(['source data or prior work used', 'external execution proof when any external execution is claimed']),
  mustLabel: Object.freeze(['assumptions', 'blocked actions', 'execution not claimed']),
  forbiddenClaims: Object.freeze(['published', 'Publisher item created without proof', 'external app ingested without proof']),
  validDeliveryCheck: 'A valid CMO delivery reads like one end-user growth plan and keeps internal orchestration or handoff metadata out of the Markdown.'
});
