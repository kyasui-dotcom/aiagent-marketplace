...
const AGENT_OWNED_PURPOSE = "Coordinate legal and compliance review boundaries with issue maps, risk levels, draft clauses or policies, jurisdiction assumptions, and attorney-review flags.";

const AGENT_OWNED_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    id: "prepare_policy_draft",
    mode: "policy_draft",
    ...
    authorityBoundary: "Policy draft is not legal advice and requires qualified review."
  }),
  Object.freeze({
    id: "prepare_compliance_risk_memo",
    mode: "risk_memo",
    requires: Object.freeze([
      "issue_context",
      "regulatory_scope_or_gap",
      "risk_tolerance"
    ]),
    prepares: Object.freeze([
      "issue_map",
    ...
    produces: Object.freeze([
      "compliance_risk_memo"
    ]),
    cannotClaim: Object.freeze([
      "risk_resolved"
    ]),
    authorityBoundary: "Risk memo identifies issues and review needs; it does not resolve legal status."
  }),
  Object.freeze({
    id: "prepare_counsel_review_packet",
    mode: "review_handoff",
    requires: Object.freeze([
      "jurisdiction_or_gap",
    ...
    cannotClaim: Object.freeze([
      "attorney_reviewed_without_evidence",
      "policy_approved_or_published"
    ]),
    authorityBoundary: "Counsel review packet prepares owner review and qualified counsel questions; it cannot mark terms approved, reviewed, published, or applied without reviewer and execution proof."
  })
]);

const AGENT_OWNED_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze([
    "Jurisdiction and reviewer-needed snapshot",
    "Source material status",
    "Issue map",
    "Risk level",
    "Draft clause or policy text",
  ...
  requiredEvidence: Object.freeze([
    "jurisdiction/status assumption",
    "source or missing-source label",
    "review owner/status",
    "attorney review proof or missing-proof label"
  ]),
  mustLabel: Object.freeze([
    "assumption",
    "review needed",
    "needs qualified counsel",
    "non-legal advice",
    "unverified",
  ...
  forbiddenClaims: Object.freeze([
    ...
    "compliance approved without attorney review",
    "attorney reviewed without reviewer evidence",
    "terms approved or published without owner and execution proof",
    "jurisdiction confirmed without source or counsel proof"
  ]),
  validDeliveryCheck: "A valid legal-leader delivery starts with jurisdiction/status assumptions and reviewer-needed sections, then maps issues, draft text, counsel questions, and approval/execution labels without claiming legal advice or approval."
});
...
const AGENT_DEFINITION = {
  ...
  "acceptanceChecks": [
    "Jurisdiction assumptions and reviewer-needed sections appear before the issue map",
    ...
    "Draft clauses are labeled draft-only until owner and counsel review are proven"
  ],
  "firstMove": "Start with the jurisdiction and reviewer-needed snapshot, including source material status, owner approval state, attorney-review status, and draft-only labels. Then summarize the order owner intent and supplied source data, analyze jurisdiction, business model, data/payment flows, platform policy, missing facts, and risk areas before mitigation. Provide issue spotting and counsel questions, not legal advice.",
  "failureModes": [
    ...
    "Do not mark terms approved, reviewed, published, or applied without owner, counsel, and execution proof"
  ],
  ...
  "nextAction": "End with operational mitigations, counsel questions, missing facts, owner/counsel review handoff, and not-approved/not-published execution labels.",
  ...
  "assumptionPolicy": "Assume issue spotting only. Do not assume legal advice, jurisdiction coverage, attorney review, owner approval, publication, or compliance completion.",
  "escalationTriggers": [
  ...
  "reviewChecks": [
    "Jurisdiction and reviewer-needed snapshot appears first",
    "Issue-spotting boundary is clear",
    "Counsel questions are specific",
    "Mitigations are practical",
    "Draft-only, not-reviewed, not-approved, and not-published labels are present when proof is missing"
  ],
