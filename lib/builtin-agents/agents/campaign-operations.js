...
const CAMPAIGN_OPERATIONS_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    prepares: Object.freeze(['next_now_items', 'next_after_approval_items', 'waiting_conditions', 'blocked_action_labels']),
    produces: Object.freeze(['planned_action_queue_packet']),
    cannotClaim: Object.freeze(['blocked item ready', 'waiting item executed', 'approval dependency resolved']),
    authorityBoundary: 'The planned action queue separates what happens next from what waits; it is not proof that blocked or approval-gated actions ran.'
  }),
  Object.freeze({
...
const CAMPAIGN_OPERATIONS_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Campaign state', 'Publisher queue', 'Approval backlog', 'Connector readiness', 'Planned action queue', 'Waiting conditions', 'Measurement loop', 'Next action owner']),
  ...
  forbiddenClaims: Object.freeze(['published', 'sent', 'launched', 'Publisher item created without proof', 'blocked item ready', 'waiting item executed', 'outcome decided without measurement']),
  validDeliveryCheck: 'A valid campaign operations delivery preserves CMO/channel ownership, separates what happens next from what waits, and names the next owner.'
});
