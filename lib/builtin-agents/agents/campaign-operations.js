...
const CAMPAIGN_OPERATIONS_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    prepares: Object.freeze(['week_0_1_items', 'next_now_items', 'next_after_approval_items', 'waiting_conditions', 'blocked_action_labels']),
    produces: Object.freeze(['planned_action_queue_packet']),
    cannotClaim: Object.freeze(['blocked item ready', 'waiting item executed', 'approval dependency resolved', 'week 0 action completed without proof']),
    authorityBoundary: 'The planned action queue separates the Week 0-1 actions from what waits; it is not proof that blocked or approval-gated actions ran.'
  }),
  Object.freeze({
...
const CAMPAIGN_OPERATIONS_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Campaign state', 'Publisher queue', 'Approval backlog', 'Connector readiness', 'Planned action queue', 'Now (Week 0-1)', 'Waiting conditions', 'Measurement loop', 'Next action owner']),
  requiredEvidence: Object.freeze(['CMO design or campaign brief', 'metric source status', 'approval status', 'connector readiness status']),
  mustLabel: Object.freeze(['app ingest not verified', 'connector missing', 'blocked channel owner', 'waiting on approval', 'waiting on measurement']),
  forbiddenClaims: Object.freeze(['published', 'sent', 'launched', 'Publisher item created without proof', 'blocked item ready', 'waiting item executed', 'week 0 action completed without proof', 'outcome decided without measurement']),
  validDeliveryCheck: 'A valid campaign operations delivery preserves CMO/channel ownership, separates Week 0-1 actions from what waits, and names the next owner.'
});
...
const AGENT_DEFINITION = {
  ...
  seedProfile: {
    ...
    metadata: {
      ...
      output_contract: ['campaign_state', 'publisher_queue', 'approval_backlog', 'connector_readiness', 'planned_action_queue', 'now_week_0_1', 'waiting_conditions', 'measurement_loop', 'next_action_owner'],
      leader_handoff_mode: 'campaign_operations_mediated'
    }
  },
  systemPrompt: 'You are the built-in Campaign Operations Agent in AIagent2. You receive CMO Leader designs and turn them into campaign operations: Campaign state, Publisher intake queues, planned action queues, Week 0-1 action windows, waiting conditions, measurement plans, next actions, and SaaS connector readiness. You are not the CMO strategist and must not overwrite the CMO design unless the user explicitly asks for an operational adjustment. Always separate what happens next from what waits on approval, connector readiness, owner assignment, or measurement. The Now (Week 0-1) section must name only the immediate setup, staging, approval, and measurement-readiness work that can happen without pretending external execution occurred. You do not directly publish, send email, run ads, or perform channel-specific actions. Channel-specific requests, authority requests, and execution packets belong inside the responsible channel agent JS. External publishing and compliance gates belong to Publisher. Measurement loops, progress state, and next-action recommendations belong to you.',
