...
function campaignContextFromBody(body = {}) {
  ...
  const assets = rawAssets
    .map((asset, index) => {
      ...
      return { title, channel, status, measurementStatus, owner, queueLabel };
    });
  return {
    prompt,
...
function campaignOperationsMarkdown(context = {}, japanese = false) {
  ...
  const assetLines = context.assets.length
    ? context.assets.map((asset) => `- ${asset.title} (${asset.channel}): queue=${asset.queueLabel}; status=${asset.status}; measurement=${asset.measurementStatus || 'not supplied'}; owner=${asset.owner}.`).join('\n')
    : '- No channel asset list supplied; request asset title, channel, approval status, measurement status, owner, and proof link before queueing.';
  const assetLinesJa = context.assets.length
    ? context.assets.map((asset) => `- ${asset.title} (${asset.channel}): queue=${asset.queueLabel}; status=${asset.status}; measurement=${asset.measurementStatus || '未提供'}; owner=${asset.owner}。`).join('\n')
    : '- チャネル別アセット一覧が未提供。入稿前にアセット名、チャネル、承認状態、測定状態、担当者、証跡リンクを確認する。';
  if (japanese) {
    return [
      `# ${context.title}`,
      '',
      ...
      '- Publisher 候補: アセットステータスキューで approved-ready と draft-needs-approval を分ける。measurement-waiting は測定待ち、blocked は入稿対象外にする。',
  ...
  return [
    ...
    '- Publisher candidates: separate approved-ready assets from draft-needs-approval assets; keep measurement-waiting assets in results review and blocked assets out of Publisher intake.',
...
const CAMPAIGN_OPERATIONS_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Campaign state', 'Publisher queue', 'Asset status queue', 'Approval backlog', 'Connector readiness', 'Planned action queue', 'Now (Week 0-1)', 'Next (Week 1-3)', 'Waiting conditions', 'Measurement loop', 'Next action owner']),
  requiredEvidence: Object.freeze(['CMO design or campaign brief', 'asset approval status per queue item', 'asset measurement status per measured/waiting item', 'metric source status', 'approval status', 'connector readiness status', 'Week 0-1 completion or readiness status']),
  mustLabel: Object.freeze(['approved-ready', 'draft-needs-approval', 'measurement-waiting', 'blocked', 'app ingest not verified', 'connector missing', 'blocked channel owner', 'waiting on approval', 'waiting on measurement', 'Week 1-3 gated', 'measurement-gated expansion']),
