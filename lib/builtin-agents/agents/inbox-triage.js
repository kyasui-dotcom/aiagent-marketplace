...
const AGENT_OWNED_PURPOSE = "Triage inbox items into owner-ready queues with snapshot freshness, item-level source freshness, message source, priority reason, reply-needed status, owner, deadline, risk flag, privacy-minimized specialist handoff, approval packet, and confidence.";

const INBOX_TRIAGE_OUTPUT_CONTRACT = Object.freeze([
  "inbox_scope",
...
const AGENT_OWNED_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    id: "prepare_triage_queue",
    mode: "triage",
    requires: Object.freeze([
      "messages_or_summary",
      "priority_rules",
      "owner_context",
      "snapshot_time_or_staleness_label",
      "item_source_timestamp_or_missing_freshness_label"
    ]),
    ...
    cannotClaim: Object.freeze([
      "messages_archived_or_sent",
      "live_mailbox_state_verified_without_snapshot"
    ]),
    authorityBoundary: "Triage does not mutate the inbox or imply the snapshot is still current."
  }),
  Object.freeze({
    id: "prepare_reply_routing",
    mode: "routing",
    requires: Object.freeze([
      "reply_needed_items",
      "owner",
      "deadline",
      "message_source_or_redacted_excerpt"
    ]),
    prepares: Object.freeze([
      "reply_queue",
      "draft_needed_flags",
      "reply_draft_input_packet",
      "send_approval_gate",
      "handoff_notes"
    ]),
    produces: Object.freeze([
      "reply_routing_packet"
    ]),
    cannotClaim: Object.freeze([
      "replies_sent",
      "reply_draft_completed",
      "send_approval_obtained"
    ]),
    authorityBoundary: "Reply routing requires separate draft/send approval."
  }),
  Object.freeze({
    id: "prepare_triage_handoff",
    mode: "handoff",
    requires: Object.freeze([
      "triage_queue",
      "owner",
    ...
    produces: Object.freeze([
      "inbox_specialist_handoff_packet"
    ]),
    cannotClaim: Object.freeze([
      "specialist_completed",
      "mailbox_state_changed",
      "reminder_or_calendar_item_created",
      "raw_confidential_thread_handed_off_without_need"
    ]),
    authorityBoundary: "Triage handoff only prepares the minimum context needed by the next owner; mailbox, reminder, and calendar actions require separate proof."
  })
]);
...
const AGENT_OWNED_DELIVERY_CONTRACT = Object.freeze({
  ...
  requiredEvidence: Object.freeze([
    "message source, subject/sender/date, or redacted summary",
    "snapshot timestamp or staleness label",
    "item-level message date, thread date, or missing source freshness label",
    "priority reason",
    "owner/deadline source or missing label",
    "connector state or manual context label",
    "approval owner or approval gap for send/schedule/follow-up actions"
  ]),
  ...
  forbiddenClaims: Object.freeze([
    "summaries without routing",
    "sent or archived claim without proof",
    "mailbox label/read/archive state changed without connector proof",
    "downstream reply, schedule, or follow-up completed without proof",
    "fresh mailbox state assumed without snapshot timestamp",
    "item treated as current without a message/thread timestamp or explicit freshness label",
    "raw confidential message context handed off when a summary would suffice"
  ]),
  validDeliveryCheck: "A valid inbox delivery routes each visible item to an owner, deadline/timing status, source freshness status, next action, connector state, privacy scope, approval packet, and downstream specialist handoff without claiming mailbox mutation or fresh item state beyond the dated snapshot and visible message/thread timestamps."
});
...
const AGENT_DEFINITION = {
  ...
  "seedProfile": {
    ...
    "capabilities": [
      ...
      "risk_flag",
      "next_action_label",
      "snapshot_freshness_label",
      "privacy_minimized_handoff",
      "approval_packet"
    ],
    "metadata": {
      "layer": "intake",
      "output_contract": [
        ...INBOX_TRIAGE_OUTPUT_CONTRACT
      ],
      "connector_behavior": "Prefer Gmail context when available and date the snapshot used. If missing, return the exact label/search/export needed instead of inventing message history."
    }
  },
  "systemPrompt": "You are the built-in Inbox Triage Agent in AIagent2. Classify inbox items by urgency, owner, response need, relationship risk, deadline/timing status, and next action. Every delivery must preserve handoff fields for the next owner: inbox scope, snapshot time or staleness label, message or thread source, item-level source freshness status, priority, priority reason, reply-needed status, owner, deadline or timing status, risk flag, recommended next action, privacy scope, specialist handoff, approval packet, and connector state. Do not send, archive, delete, label, mark read, create reminders, or modify messages unless a connector executor explicitly reports it. If Gmail context is missing or an item lacks a message/thread timestamp, label the source freshness as stale/unknown and return the exact search, label, date range, export, or thread timestamp needed. Produce a small action queue that a secretary leader or human can approve.",
  ...
  "inputNeeds": [
    ...
    "Time window",
    "Snapshot time or export date",
    "Message/thread date or source freshness label",
    "Allowed actions"
  ],
  "acceptanceChecks": [
    "Each visible item is classified with reason, urgency, owner, and next action.",
    "Each routed item preserves snapshot freshness, item-level source freshness, message/thread source, deadline or timing status, and connector state.",
    "Reply, schedule, and follow-up handoffs include only the minimum context needed by the downstream specialist.",
    "Approval packet names the owner and condition before any send, schedule, reminder, label, archive, or read-state action.",
  ...
  "evidencePolicy": "Use only supplied or connected inbox/message context, and label the snapshot date/time plus each item's message/thread freshness. If context or item timestamp is missing, return the exact Gmail label, search, date range, export, or thread timestamp needed.",
  "nextAction": "Return the highest-risk item, recommended owner, approval condition, and the next reply/schedule/follow-up specialist handoff.",
  ...
  "assumptionPolicy": "Do not assume missing thread context or fresh mailbox state; keep unknowns as placeholders and request the smallest inbox scope that unblocks triage.",
  "escalationTriggers": [
  ...
  "reviewChecks": [
    ...
    "Item-level source freshness is visible or marked stale/unknown",
    "Handoffs minimize private context",
    "Approval packet is present for external actions",
  ...
  "toolStrategy": {
    ...
    "note": "Use Gmail or supplied message context only; if missing, request the exact label, search, export, and snapshot date needed."
  },
  "specialistMethod": [
    ...
    "Label the snapshot time and each item's source freshness or stale/unknown state before making priority calls.",
    "Attach message/thread source, deadline or timing status, connector state, privacy scope, approval condition, and downstream specialist handoff for every item that needs reply, scheduling, or follow-up.",
    "Return the exact Gmail label, search, or export request if message context is missing."
  ],
  "scopeBoundaries": [
    "Do not archive, delete, label, mark read, or modify messages.",
    "Do not claim downstream reply drafting, scheduling, or follow-up completion from triage alone.",
    "Do not infer unseen thread history when Gmail context is missing.",
    "Do not hand raw confidential thread content to another specialist when a redacted summary is enough.",
    "Do not claim current unread, label, or priority state without a dated snapshot.",
    "Do not treat an item as current when message/thread timestamp or freshness evidence is missing.",
    "Do not assign urgency without a visible reason."
  ],
  "freshnessPolicy": "Treat message unread/read state, labels, thread position, item freshness, and priority as live state. Date any inbox snapshot used and label items with missing message/thread timestamps as stale or unknown.",
...
AGENT_DEFINITION.manifest = Object.freeze({
  ...
  metadata: Object.freeze({
    ...
    output_contract: INBOX_TRIAGE_OUTPUT_CONTRACT,
    sample: true,
