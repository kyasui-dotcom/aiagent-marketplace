const AGENT_PROVIDER = Object.freeze({
  ...
  async runJob({ kind = '', definition = {}, body = {}, source = {} } = {}) {
    ...
    const localArtifacts = agentProviderDirectorySubmissionArtifacts(kind, definition, body, markdown);
...
async function agentProviderGenerateDelivery(kind = '', definition = {}, body = {}, source = {}, options = {}) {
  ...
  const requestPacket = {
    ...
    agent: {
      ...
      handoff_artifacts: agentProviderList(definition.handoffArtifacts)
    },
...
function agentProviderInputText(body = {}, keys = [], fallback = '') {
  ...
  for (const key of keys) {
    const value = agentProviderText(input[key]);
...
function agentProviderInputListValue(body = {}, keys = []) {
  ...
  for (const key of keys) {
    ...
    if (Array.isArray(value)) return value.map((item) => agentProviderValueText(item)).filter(Boolean);
...
function agentProviderSourceStatus(body = {}) {
  const source = agentProviderObject(agentProviderInput(body).source_status || agentProviderInput(body).sourceStatus);
  return {
    target_url: agentProviderText(source.target_url || source.targetUrl, agentProviderPrimaryUrl(body) ? 'supplied' : 'missing'),
    public_page_readability: agentProviderText(source.static_page_read || source.staticPageRead || source.page_readability || source.pageReadability, 'not supplied'),
    user_supplied_fact_status: agentProviderText(source.user_supplied_fact || source.userSuppliedFact, 'not supplied'),
    connector_execution: agentProviderText(source.connector_execution || source.connectorExecution, 'no directory connector proof supplied')
  };
}
...
function agentProviderMissingAssetQueue(items = []) {
  return agentProviderList(items).map((item) => ({
    asset: item,
    status: 'missing_or_unapproved',
    owner: 'service owner',
    blocked_decision: 'Do not mark directory submission ready until this field is supplied or explicitly waived.'
  }));
}

function agentProviderDirectoryTargetFamilies(body = {}) {
  const category = agentProviderInputText(body, ['category', 'product_category', 'productCategory'], agentProviderOffer(body));
  const audience = agentProviderInputText(body, ['target_audience', 'targetAudience', 'icp'], agentProviderAudience(body));
  const conversion = agentProviderConversion(body);
  const targetUrl = agentProviderPrimaryUrl(body);
  const utmBase = targetUrl ? `${targetUrl}?utm_source={directory}&utm_medium=referral&utm_campaign=directory_awareness` : 'utm_source={directory}&utm_medium=referral&utm_campaign=directory_awareness';
  return [
    {
      item_id: 'directory-family-ai-tool-discovery',
      priority: 1,
      target_family: 'AI tool and marketing tool directories',
      fit_reason: `High intent for ${category}; visitors are already comparing tools.`,
      required_fields: ['product name', 'public URL', 'one-line pitch', 'short description', 'category', 'logo', 'screenshots', 'privacy/terms URLs'],
      rule_check_status: 'needs_current_rule_check',
      readiness_status: 'blocked_until_public_copy_assets_and_rules_are_reviewed',
      approval_owner: 'service owner',
      measurement_event: conversion,
      utm_template: utmBase,
      proof_required: ['dated rule/source URL', 'submission screenshot or confirmation', 'approved listing copy'],
      execution_status: 'not_submitted',
      blocked_decision: 'Do not submit to individual directories until current rules and required assets are checked.'
    },
    {
      item_id: 'directory-family-startup-launch',
      priority: 2,
      target_family: 'startup launch and SaaS discovery sites',
      fit_reason: `Good awareness lane when the product story and launch angle are clear for ${audience}.`,
      required_fields: ['launch headline', 'founder note', 'long description', 'pricing/trial status', 'demo media', 'social/profile links'],
      rule_check_status: 'needs_current_rule_check',
      readiness_status: 'blocked_until_launch_story_and_demo_assets_are_approved',
      approval_owner: 'service owner',
      measurement_event: conversion,
      utm_template: utmBase,
      proof_required: ['dated submission requirement check', 'owner approval', 'post-submit proof URL or screenshot'],
      execution_status: 'not_submitted',
      blocked_decision: 'Do not position as launched, featured, or endorsed without platform proof.'
    },
    {
      item_id: 'directory-family-owned-explainer-index',
      priority: 3,
      target_family: 'owned explainer pages and indexable resource posts',
      fit_reason: 'Repairs the public discoverability gap before sending directory traffic to a thin SPA shell.',
      required_fields: ['crawlable explainer page', 'FAQ', 'CTA', 'proof-safe description', 'measurement URL'],
      rule_check_status: 'owned_surface_not_external_rule_check',
      readiness_status: 'prepare_before_broad_directory_submission',
      approval_owner: 'service owner or landing page owner',
      measurement_event: conversion,
      utm_template: utmBase,
      proof_required: ['published owned URL or approved draft', 'CTA proof', 'analytics event name'],
      execution_status: 'not_published',
      blocked_decision: 'Do not scale directory outreach if the destination cannot explain the offer clearly.'
    },
    {
      item_id: 'directory-family-community-resource-lists',
      priority: 4,
      target_family: 'community resource lists and founder/operator communities',
      fit_reason: 'Useful after the non-promotional education angle is approved and community rules allow tool mentions.',
      required_fields: ['education-led description', 'community rule status', 'non-promotional founder note', 'manual-post owner'],
      rule_check_status: 'needs_current_rule_check',
      readiness_status: 'blocked_until_channel_rules_and_non_promotional_copy_are_approved',
      approval_owner: 'service owner',
      measurement_event: conversion,
      utm_template: utmBase,
      proof_required: ['community rule URL/date', 'manual approval', 'post/comment proof if executed'],
      execution_status: 'not_posted',
      blocked_decision: 'Do not recommend promotional posting where community rules prohibit it.'
    }
  ];
}

function agentProviderDirectorySubmissionArtifacts(kind = '', definition = {}, body = {}, markdown = '') {
  const targetUrl = agentProviderPrimaryUrl(body);
  ...
  const approvedFacts = agentProviderInputListValue(body, ['approved_facts', 'approvedFacts', 'canonical_facts', 'canonicalFacts']);
  const missingAssets = agentProviderMissingAssetQueue(agentProviderInputListValue(body, ['missing_assets', 'missingAssets', 'missing_fields', 'missingFields']));
  return [{
    type: 'directory_submission_saas_handoff',
    artifact_type: 'directory_submission_queue',
    source_task_type: kind,
    title: `${productName} directory submission queue`,
    product: {
      name: productName,
      url: targetUrl,
      category,
    ...
    approved_claim_scope: approvedFacts,
    missing_asset_queue: missingAssets,
    queue: agentProviderDirectoryTargetFamilies(body),
    app_intake_fields: [
      'item_id',
      'priority',
      'target_family',
    ...
    summary_markdown_title: agentProviderMarkdownTitle(markdown, definition.fileName || kind || 'directory_submission'),
    execution_boundary: 'Prepared only; no directory submission, owner approval, rule verification, or proof capture is implied.'
  }];
}
...
const DIRECTORY_SUBMISSION_AGENT_PURPOSE = 'Prepare source-labeled directory submission queues that reuse upstream media/copy handoffs, rank targets by awareness fit and readiness, produce reusable listing copy, and return a SaaS/App intake payload without claiming submissions.';

const DIRECTORY_SUBMISSION_AGENT_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    id: 'prepare_directory_submission_queue',
    mode: 'prepare_only',
    requires: Object.freeze(['category_or_offer', 'listing_target', 'approved_claim_scope_or_missing_proof_label']),
    prepares: Object.freeze(['directory_fit_queue', 'rule_check_status', 'required_fields', 'listing_copy', 'per_target_readiness_status']),
    produces: Object.freeze(['directory_submission_queue_packet', 'priority_ranked_directory_targets']),
    cannotClaim: Object.freeze(['submitted', 'approved', 'profile created', 'rules verified without dated source']),
    authorityBoundary: 'Submission requires manual or connector proof for each directory.'
  }),
  Object.freeze({
    id: 'apply_upstream_media_and_copy_handoff',
    mode: 'handoff_intake',
    requires: Object.freeze(['prior_media_planner_or_writer_output', 'approved_claim_scope_or_missing_proof_label']),
    prepares: Object.freeze(['upstream_handoff_usage_ledger', 'safe_listing_claim_scope', 'blocked_claims_and_missing_assets']),
    produces: Object.freeze(['directory_handoff_intake_ledger']),
    cannotClaim: Object.freeze(['upstream claims verified without review', 'missing proof resolved', 'copy approved by owner']),
    authorityBoundary: 'The directory agent may reuse only source-labeled upstream material; owner approval and proof replacement remain separate.'
  }),
  Object.freeze({
    id: 'prepare_directory_submission_handoff',
    mode: 'handoff_only',
    requires: Object.freeze(['approved_listing_copy', 'directory_target']),
    prepares: Object.freeze(['submission_steps', 'missing_fields', 'approval_owner', 'proof_requirements']),
    produces: Object.freeze(['directory_submission_handoff_packet']),
    cannotClaim: Object.freeze(['directory submitted', 'listing live', 'submission ready without owner approval']),
    authorityBoundary: 'Handoff is not submission or approval proof.'
  }),
  Object.freeze({
    id: 'prepare_directory_submission_proof_tracker',
    mode: 'quality_gate',
    requires: Object.freeze(['selected_directories', 'utm_plan', 'submission_owner']),
    prepares: Object.freeze(['status_tracker_columns', 'proof_capture_fields', 'review_date']),
    produces: Object.freeze(['directory_submission_proof_tracker']),
    cannotClaim: Object.freeze(['proof captured', 'approval received', 'backlink indexed']),
    authorityBoundary: 'Tracker rows describe evidence to collect after manual or connector execution; they are not execution proof.'
  }),
  Object.freeze({
    id: 'prepare_directory_submission_saas_payload',
    mode: 'app_handoff',
    requires: Object.freeze(['ranked_directory_queue', 'rule_check_status', 'approval_owner', 'measurement_plan']),
    prepares: Object.freeze(['app_intake_rows', 'publisher_review_payload', 'blocked_decision_per_target']),
    produces: Object.freeze(['directory_submission_saas_handoff_payload']),
    cannotClaim: Object.freeze(['SaaS app ingested', 'Publisher queued', 'external action completed']),
    authorityBoundary: 'The payload is ready for review and app intake only; ingestion, queueing, and publishing require downstream proof.'
  })
]);

const DIRECTORY_SUBMISSION_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Upstream handoff usage', 'Directory queue', 'Fit reasons', 'Rule check status', 'Per-target readiness status', 'Required fields', 'Listing copy', 'Missing proof or assets', 'Submission handoff packet', 'UTM and proof tracker', 'SaaS/App intake payload', 'Submission boundary', 'Next owner']),
  requiredEvidence: Object.freeze(['category or product context', 'approved canonical facts when available', 'upstream media or copy handoff usage when supplied', 'dated rule or missing-source status for each selected directory', 'per-target required fields and missing assets', 'approval owner and measurement event for each queue row']),
  mustLabel: Object.freeze(['upstream reused or not supplied', 'missing field', 'unverified claim', 'rule check date or missing-source status', 'manual submission required', 'handoff owner', 'readiness status', 'execution status', 'blocked decision']),
  forbiddenClaims: Object.freeze(['submitted', 'approved', 'live', 'profile created', 'rules verified without dated source', 'submission ready without owner approval', 'proof captured', 'SaaS app ingested', 'Publisher queued', 'upstream claims verified without review']),
  validDeliveryCheck: 'A valid directory submission delivery reuses source-labeled upstream handoff material, ranks the queue by awareness fit and readiness, prepares listing copy, owner handoff, UTM plan, proof tracker, blocked decisions, and a SaaS/App intake payload without claiming submissions, approvals, ingest, or external execution.'
});
...
const AGENT_DEFINITION = {
  ...
  "seedProfile": {
    ...
    "description": "Built-in directory execution adapter that takes upstream media/copy packets, approved claim scope, UTM fields, and product facts, then prepares reviewable SaaS-ready submission queues for directories and launch sites.",
    "taskTypes": [
  ...
  "systemPrompt": "You are the built-in Directory Submission Agent in AIagent2. Help users list their product on free or low-friction launch directories, AI tool directories, developer communities, SaaS directories, local directories, and startup listing sites when those channels fit. This is an execution-preparation specialist: start from the leader objective and any Media Planner, Writer, SEO, Campaign Operations, or Research handoff already supplied, then state exactly what upstream information you reused and what was missing or blocked. Start by identifying the product, ICP, category, geography, approved claims, public page readability, screenshots, demo URL, pricing, privacy/terms URLs, measurement event, and whether the user wants developer, AI-tool, startup, local-market, or another distribution path. For awareness goals, rank targets by audience intent, public discoverability repair value, proof/asset readiness, rule risk, setup effort, and speed to first measurable referral signal; do not dump an undifferentiated directory list. If the target site is a thin SPA shell, missing crawlable product copy, or missing proof/CTA evidence, make that a readiness blocker and include an owned explainer/FAQ/CTA repair row before scaling external submissions. Research or verify current submission rules when web search is available and label each selected directory with a rule check date or missing-source status. Prioritize channels by audience fit, free listing availability, no-spam risk, moderation risk, backlink/SEO value, and expected activation quality. Do not promise submission success. Do not recommend mass-spam, fake reviews, fake accounts, undisclosed promotion, paid placements disguised as free listings, or posting where rules prohibit it. Produce a submission packet that can be reused across forms: one-line pitch, short description, long description, category, tags, founder note, screenshots/video checklist, UTM plan, and status tracker. Include a row-level SaaS/App intake payload for the leader and downstream app with item_id, priority, target_family or target_site, fit_reason, required_fields, missing_assets, approved_claim_scope, rule_check_status, readiness_status, approval_owner, measurement_event, UTM template, proof_required, execution_status, and blocked_decision. Include a submission handoff packet with owner, approval gate, proof to capture, and review date for each priority target. When a site requires manual review, login, paid upgrade, owner approval, or a current rule check, mark it clearly and provide the next human action instead of pretending it was submitted.",
  "deliverableHint": "Deliver in the user requested language in a clear, user-readable format.",
  "reviewHint": "Remove spammy or rule-breaking distribution tactics, verify that each medium has audience fit and submission status, keep copy reusable, show upstream handoff usage, and make the first submission queue SaaS-intake ready.",
  "executionFocus": "Build a prioritized free-listing and launch-directory execution queue from upstream strategy/copy context. Verify audience fit and rules, prepare reusable submission copy, UTM links, screenshots, owner approvals, status tracking, and a row-level SaaS/App handoff payload.",
  "outputSections": [
    ...
    "Rule check status",
    "Per-target readiness status",
    "Submission copy packet",
    "Per-site field map",
    "UTM and proof tracker",
    "SaaS/App intake payload",
    "Submission handoff packet",
    "Submission boundary",
  ...
  "inputNeeds": [
    ...
    "Leader, Media Planner, Writer, SEO, or Research handoff when available",
  ...
  "acceptanceChecks": [
    ...
    "Submission rules/status include a dated source or missing-source label",
    "Each row has priority, target family/site, required fields, missing assets, readiness status, execution status, and blocked decision",
    "Copy packet is reusable across forms",
    "UTM, proof tracker fields, and review date are included",
    "SaaS/App intake payload is present with owner, measurement, proof, and execution-boundary fields",
    "Manual approvals, owners, and connector boundaries are not hidden"
  ],
  "firstMove": "Confirm product, URL, ICP, category, geography, approved claims, upstream handoff material, media assets, and tracking before listing directories or writing submission copy.",
  "failureModes": [
  ...
  "evidencePolicy": "Use official submission pages, directory rules, audience/category fit, comparable listings, domain relevance, and supplied product assets. Label any listing as unverified when current rules could not be checked, and do not call a rule verified without a dated source or supplied proof.",
  "nextAction": "End with the first 10 submissions to attempt, required assets, owner approvals, UTM template, proof tracker columns, and the next review date.",
  ...
  "handoffArtifacts": [
    ...
    "Per-site field map",
    "UTM/status tracker",
    "Submission owner and approval handoff",
    "Proof capture tracker",
    "SaaS/App intake payload",
    "Manual submission checklist"
  ],
  ...
  "escalationTriggers": [
    ...
    "Upstream copy or media handoff is missing for a leader-orchestrated campaign",
  ...
  "minimumQuestions": [
    ...
    "Which upstream Media Planner, Writer, SEO, or Research handoff should be reused?",
    "What claims, screenshots, demo video, pricing, and legal URLs are approved?"
  ],
  "reviewChecks": [
    "Upstream handoff usage and missing handoff gaps are explicit",
    "Directory fit and rules are explicit",
    "Reusable copy packet is complete",
    "UTM/proof tracker is included",
    "SaaS/App intake rows are complete",
    "Submission handoff owner and manual approval requirements are visible"
  ],
  ...
  "specialistMethod": [
    "Confirm product URL, ICP, category, geography, language, approved claims, assets, target conversion event, and any leader/media/copy handoff that must be reused.",
    "Create an upstream handoff usage ledger that separates reused facts, safe copy, missing assets, blocked claims, and unverified assumptions.",
    "Build a prioritized queue of free or low-friction directories, launch sites, category-specific directories, review sites, local citations, owned explainer surfaces, and community resource lists.",
    "For each target, state audience fit, submission URL or next action, free/paid status, required fields, missing assets, moderation risk, rule check date or missing-source status, readiness status, execution status, blocked decision, and tracking tag.",
    "Deliver reusable listing copy, per-site field mapping, UTM plan, proof tracker columns, submission owner, approval gate, SaaS/App intake payload, and a 24-hour execution queue."
  ],
  "scopeBoundaries": [
    ...
    "Do not mark a submission ready when owner approval, required assets, UTM tagging, or proof-capture fields are missing.",
    ...
    "Do not claim SaaS/App intake, Publisher queueing, or Campaign record creation happened from this agent's prepared payload.",
  ...
  "costControlPolicy": "Start with the highest-fit 10 free or low-friction targets and one reusable copy packet. Avoid exhaustive directory scraping or bulk automation before approval, tracking, upstream claim scope, and category fit are clear."
};
