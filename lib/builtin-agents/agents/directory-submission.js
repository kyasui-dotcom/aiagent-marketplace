...
const DIRECTORY_SUBMISSION_AGENT_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    ...
    prepares: Object.freeze(['directory_fit_queue', 'rule_check_status', 'required_fields', 'listing_copy']),
...
const DIRECTORY_SUBMISSION_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Directory queue', 'Fit reasons', 'Rule check status', 'Required fields', 'Listing copy', 'Missing proof or assets', 'Submission handoff packet', 'UTM and proof tracker', 'Submission boundary', 'Next owner']),
  requiredEvidence: Object.freeze(['category or product context', 'approved canonical facts when available', 'dated rule or missing-source status for each selected directory']),
  mustLabel: Object.freeze(['missing field', 'unverified claim', 'rule check date or missing-source status', 'manual submission required', 'handoff owner']),
  forbiddenClaims: Object.freeze(['submitted', 'approved', 'live', 'profile created', 'rules verified without dated source', 'submission ready without owner approval', 'proof captured']),
  validDeliveryCheck: 'A valid directory submission delivery prepares a queue, listing copy, handoff owner, UTM plan, and proof tracker without claiming submissions or approvals.'
});
...
const AGENT_DEFINITION = {
  ...
  "specialistMethod": [
    ...
    "For each target, state audience fit, submission URL or next action, free/paid status, required fields, moderation risk, rule check date or missing-source status, and tracking tag.",
    "Deliver reusable listing copy, per-site field mapping, UTM plan, proof tracker columns, submission owner, approval gate, and a 24-hour execution queue."
  ],
  "scopeBoundaries": [
