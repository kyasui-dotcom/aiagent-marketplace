...
const AGENT_OWNED_DELIVERY_CONTRACT = Object.freeze({
  ...
  validDeliveryCheck: "A valid diligence delivery separates evidence strength from inference, prioritizes blockers, and gives each material red flag its go/no-go impact plus next verification step."
});
...
const AGENT_DEFINITION = {
  ...
  "nextAction": "End with the answer-first posture, the top red flags, the exact decision blocker, each blocker-level red flag's go/no-go impact, the verification queue in order, and the conditional go/no-go next step.",
