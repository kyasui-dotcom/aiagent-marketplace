...
const X_POST_AGENT_PURPOSE = 'Prepare X-native posts, threads, replies, cadence, schedule packets, approval checklist, and connector handoff without claiming unapproved X actions.';
...
const X_POST_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Positioning and voice', 'Exact post text', 'Thread or short-post set', 'Reply hooks', 'Account and link policy', 'Pre-publish review', 'Schedule packet', 'Approval checklist', 'Connector handoff boundary', 'Publish readiness handoff', 'Execution status labels']),
  requiredEvidence: Object.freeze(['voice/source examples or assumption label', 'target account or account gap', 'claim/proof source status', 'link/UTM policy or link gap', 'connector status or connector gap']),
  mustLabel: Object.freeze(['draft only', 'approval required', 'not approved', 'proof gap', 'link policy gap', 'connector gap', 'not queued']),
  forbiddenClaims: Object.freeze(['posted', 'scheduled', 'queued', 'liked', 'replied', 'ready to post without approval proof', 'approved without owner evidence']),
  validDeliveryCheck: 'A valid X delivery contains exact text, account/link policy, pre-publish review, schedule recommendations, and explicit approval/connector proof boundaries.'
});
...
const AGENT_DEFINITION = {
  ...
  "evidencePolicy": "Use founder voice, positioning, comparable posts, audience, approved claim proof, link policy, account status, and prior engagement signals when available. Missing proof or link/account policy must stay labeled as a gap.",
  "nextAction": "End with the first post draft, the pre-publish review, the leader or human approval packet needed for publishing, the reply plan, cadence, and the metric to watch.",
