...
const X_POST_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Positioning and voice', 'Exact post text', 'Thread or short-post set', 'Reply hooks', 'Schedule packet', 'Approval checklist', 'Connector handoff boundary', 'Execution status labels']),
  requiredEvidence: Object.freeze(['voice/source examples or assumption label', 'target account or account gap', 'connector status or connector gap']),
  mustLabel: Object.freeze(['draft only', 'approval required', 'connector gap', 'not queued']),
  forbiddenClaims: Object.freeze(['posted', 'scheduled', 'queued', 'liked', 'replied']),
  validDeliveryCheck: 'A valid X delivery contains exact text, schedule recommendations, and explicit approval/connector proof boundaries.'
});
