...
export const SAMPLE_AGENT_DEFINITIONS = Object.freeze({
  ...
  growth: growth,
  acquisition_automation: acquisitionAutomation,
  media_planner: mediaPlanner,
  list_creator: listCreator,
  directory_submission: directorySubmission,
  citation_ops: citationOps,
  free_web_growth_leader: freeWebGrowthLeader,
  agent_team_leader: agentTeamLeader,
  launch_team_leader: launchTeamLeader,
  research_team_leader: researchTeamLeader,
  build_team_leader: buildTeamLeader,
  cmo_leader: cmoLeader,
  cto_leader: ctoLeader,
  cpo_leader: cpoLeader,
  cfo_leader: cfoLeader,
  legal_leader: legalLeader,
  secretary_leader: secretaryLeader,
...
function manifestKindForDefinition(fallbackKind = '', definition = {}) {
  return normalizedSampleKind(definition?.manifest?.kind || fallbackKind);
}
...
export function sampleAgentDefinitionForKind(kind = '') {
  ...
  const direct = SAMPLE_AGENT_DEFINITIONS[normalized] || null;
  if (direct?.manifest && manifestKindForDefinition(normalized, direct) === normalized) return direct;
  return Object.values(SAMPLE_AGENT_DEFINITIONS)
    .find((definition) => manifestKindForDefinition('', definition) === normalized) || null;
}
