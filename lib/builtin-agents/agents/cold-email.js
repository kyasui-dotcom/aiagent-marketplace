...
const COLD_EMAIL_AGENT_PURPOSE = 'Prepare compliant, source-backed cold outreach work: qualified lead queue, sender setup, company-specific draft sequence, approval gate, and ESP/CRM handoff without claiming import or send execution.';
...
const COLD_EMAIL_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    mode: 'handoff_only',
    requires: Object.freeze(['approved_recipient_list', 'approved_copy', 'sender_mailbox', 'connector_status', 'send_cap']),
    prepares: Object.freeze(['send_checklist', 'suppression_check', 'unsubscribe_requirement', 'measurement_plan', 'execution_status_labels']),
    produces: Object.freeze(['cold_email_send_handoff_packet']),
    cannotClaim: Object.freeze(['sent', 'scheduled', 'queued', 'CRM imported', 'automation activated']),
...
const COLD_EMAIL_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Target segment and offer', 'Lead source status', 'Qualification queue', 'Sender and mailbox readiness', 'Company-specific sequence', 'Exact recipient approval', 'Compliance and opt-out notes', 'Approval/send boundary', 'Connector handoff packet', 'Measurement plan', 'Execution status labels']),
  requiredEvidence: Object.freeze(['lead source or explicit source gap', 'sender identity or mailbox gap', 'target segment', 'lawful outreach/consent basis', 'exact recipient approval or recipient gap', 'connector status or connector gap']),
  mustLabel: Object.freeze(['source not verified', 'recipient approval missing', 'compliance review needed', 'approval required', 'send handoff only', 'not imported', 'not queued']),
  forbiddenClaims: Object.freeze(['sent', 'scheduled', 'queued', 'CRM imported', 'sequence launched', 'automation activated']),
  validDeliveryCheck: 'A valid cold email delivery is source-backed, recipient-specific, compliance-aware, and explicit that no import/send/sequence execution happened without connector proof.'
});
...
const AGENT_DEFINITION = {
  ...
  "acceptanceChecks": [
    ...
    "Output says not imported, not queued, and not sent unless connector proof is supplied",
  ...
  "nextAction": "End with the first reviewed companies to contact, sender mailbox to use, exact first email to approve, connector action requested, not-imported/not-queued/not-sent labels, send cap, reply triage rule, and the conversion metric to watch.",
