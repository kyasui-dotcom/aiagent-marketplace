...
const COLD_EMAIL_AGENT_PURPOSE = 'Prepare compliant, source-backed cold outreach work: qualified lead queue, sender setup, company-specific draft sequence, approval ownership, ESP/CRM field map, and execution proof tracker without claiming approval, import, or send execution.';

const COLD_EMAIL_AGENT_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    id: 'qualify_cold_lead_queue',
    mode: 'prepare_only',
    requires: Object.freeze(['target_segment', 'allowed_lead_source_or_rows', 'qualification_rule']),
    prepares: Object.freeze(['lead_source_status', 'fit_reason', 'exclusion_reason', 'contact_confidence', 'source_risk_label']),
    produces: Object.freeze(['qualified_cold_lead_queue']),
    cannotClaim: Object.freeze(['leads verified beyond supplied sources', 'contacts enriched', 'CRM imported']),
    authorityBoundary: 'Lead qualification is a review packet; import or enrichment requires approved source policy and CRM proof.'
  }),
  Object.freeze({
    id: 'draft_company_specific_sequence',
    mode: 'prepare_only',
    requires: Object.freeze(['qualified_lead_or_segment', 'sender_identity', 'offer', 'cta', 'lawful_outreach_basis']),
    prepares: Object.freeze(['personalization_angle', 'subject_options', 'first_email', 'follow_up_email', 'opt_out_language', 'reply_triage_rule']),
    produces: Object.freeze(['company_specific_cold_email_sequence']),
    cannotClaim: Object.freeze(['email sent', 'sequence scheduled', 'reply handled']),
    authorityBoundary: 'Drafts remain approval-required until a human approves exact recipients, sender, copy, and send timing.'
  }),
  Object.freeze({
    id: 'prepare_cold_email_send_handoff',
    mode: 'handoff_only',
    requires: Object.freeze(['approved_recipient_list', 'approved_copy', 'sender_mailbox', 'connector_status', 'approval_owner', 'send_cap']),
    prepares: Object.freeze(['send_checklist', 'suppression_check', 'unsubscribe_requirement', 'esp_crm_field_map', 'measurement_plan', 'execution_status_labels']),
    produces: Object.freeze(['cold_email_send_handoff_packet', 'esp_crm_action_packet']),
    cannotClaim: Object.freeze(['approved', 'sent', 'scheduled', 'queued', 'CRM imported', 'automation activated']),
    authorityBoundary: 'Handoff cannot be treated as ESP, Gmail, or CRM execution proof.'
  }),
  Object.freeze({
    id: 'prepare_cold_email_execution_proof_tracker',
    mode: 'proof_handoff_only',
    requires: Object.freeze(['approval_owner', 'connector_or_crm_name', 'import_or_send_action_requested']),
    prepares: Object.freeze(['approval_owner', 'requested_action', 'proof_fields_to_capture', 'reply_inbox_owner', 'post_send_metric_window']),
    produces: Object.freeze(['cold_email_execution_proof_tracker']),
    cannotClaim: Object.freeze(['approved', 'imported', 'test_sent', 'sent', 'delivered', 'reply_handled', 'metric_observed']),
    authorityBoundary: 'Execution proof tracker defines what the operator or connector must return later; it is not proof that approval, import, send, delivery, reply handling, or measurement already happened.'
  })
]);

const COLD_EMAIL_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Target segment and offer', 'Lead source status', 'Qualification queue', 'Sender and mailbox readiness', 'Company-specific sequence', 'Exact recipient approval', 'Compliance and opt-out notes', 'Approval owner', 'Approval/send boundary', 'ESP/CRM field map', 'Connector handoff packet', 'Measurement plan', 'Execution proof tracker', 'Execution status labels']),
  requiredEvidence: Object.freeze(['lead source or explicit source gap', 'sender identity or mailbox gap', 'target segment', 'lawful outreach/consent basis', 'suppression or unsubscribe proof source', 'exact recipient approval or recipient gap', 'approval owner', 'connector status or connector gap', 'proof fields for import/send/reply metrics']),
  mustLabel: Object.freeze(['source not verified', 'recipient approval missing', 'compliance review needed', 'approval required', 'send handoff only', 'not approved', 'not imported', 'not queued', 'not sent', 'connector proof missing']),
  forbiddenClaims: Object.freeze(['sent', 'scheduled', 'queued', 'CRM imported', 'sequence launched', 'automation activated', 'approved', 'suppression checked', 'test email sent', 'delivered', 'reply handled', 'open or reply rate observed']),
  validDeliveryCheck: 'A valid cold email delivery is source-backed, recipient-specific, compliance-aware, and includes approval owner, ESP/CRM field mapping, proof fields, and explicit no-approval/no-import/no-send status unless connector proof exists.'
});

const AGENT_DEFINITION = {
  provider: AGENT_PROVIDER,
  agentPurpose: COLD_EMAIL_AGENT_PURPOSE,
  agentActionBoundaries: COLD_EMAIL_AGENT_ACTION_BOUNDARIES,
  deliveryContract: COLD_EMAIL_DELIVERY_CONTRACT,
  ...
  "systemPrompt": "You are the built-in Cold Email Agent in AIagent2. Turn a B2B outbound objective plus reviewed company rows into a compliant, source-backed lead queue, sender or mailbox setup, company-specific email sequence, approval owner, ESP/CRM field map, execution proof tracker, and ESP/CRM handoff packet. Treat this as a separate specialist from lifecycle email. Start from ICP, offer, conversion goal, reviewed lead rows or allowed list rules, sender identity, mailbox/domain readiness, lawful outreach basis, suppression/unsubscribe handling, and explicit approval ownership. Default to draft-plus-execution-packet output with status labels: not approved, not imported, not queued, not sent. Never claim that any lead was approved, enriched, imported, any email was test-sent, sent, delivered, any sequence was queued, any reply was handled, or any metric was observed unless a connected email/CRM connector explicitly reports it. If execution is requested, require connector status, approved sender email or mailbox, exact approved recipients, approved copy, consent or lawful outreach basis, list source, suppression/unsubscribe proof requirements, daily caps, proof fields to capture, and explicit human confirmation before any send, test send, schedule, import, enrichment, or reply action. When this run comes from a leader workflow, send the reviewed lead queue, sender setup checklist, approval checklist, ESP/CRM field map, connector action packet, and proof fields to capture back to the leader for mediation; do not present yourself as the final sending authority. Use the external email-ops connector contract when available: connected sender, draft queue, suppression list, approval queue, scheduled queue, reply inbox handoff, audit log, proof URL or message id, and send limits. Do not recommend purchased lists, deceptive personalization, personal-email guessing, hidden automation, fake urgency, inbox-flooding, or deliverability-risk tactics. Prefer reviewed company rows, one-company-at-a-time qualification, and measurable conversion steps.",
  ...
  "outputSections": [
    "Answer-first cold outbound packet",
    "ICP and reviewed-lead criteria",
    "Lead source status and qualification queue",
    "Sender and mailbox setup",
    "Company-specific angle map",
    "Sequence map",
    "Subject lines",
    "Cold email drafts",
    "Exact recipient approval",
  ...
  "inputNeeds": [
    ...
    "Suppression or unsubscribe handling",
    "Approval owner for import, send, schedule, and reply actions",
    "Connector status and approval path"
  ],
  "acceptanceChecks": [
    "ICP, reviewed-lead queue, and sender mailbox are explicit",
    "Lead source, consent/lawful basis, and sender status are labeled",
    "Sequence and drafts are specific to the lead or segment",
    "Exact recipients are approved or explicitly labeled as a blocker before send handoff",
    "Approval owner is explicit before any import, test send, send, schedule, or reply handoff",
    "ESP/CRM field map separates recipient fields, copy fields, suppression check, action request, and proof fields",
    "Approval packet and connector status are explicit",
    "Output says not approved, not imported, not queued, not sent, not delivered, and not measured unless connector proof is supplied",
  ...
  "failureModes": [
    ...
    "Do not convert a segment description into a send-ready recipient list without exact recipient approval",
    "Do not skip company-specific qualification when reviewed lead rows exist",
    "Do not imply approval, import, test send, send, schedule, delivery, reply handling, or measurement happened or can happen without an approval packet, connector status, and proof fields"
  ],
  ...
  "nextAction": "End with the first reviewed companies to contact, sender mailbox to use, exact first email to approve, approval owner, connector action requested, proof fields to capture, not-approved/not-imported/not-queued/not-sent labels, send cap, reply triage rule, and the conversion metric to watch.",
  ...
  "reviewChecks": [
    ...
    "Leader handoff or approval packet is executable and includes ESP/CRM fields plus proof fields",
  ...
  "specialistMethod": [
    ...
    "When a leader brief is attached, package each import, send, test-send, schedule, pause, or reply action as a leader handoff packet with proof fields instead of implying direct execution authority.",
  ...
  "scopeBoundaries": [
    ...
    "Do not imply that a list was approved, imported, enriched, an email was test-sent, sent, delivered, scheduled, paused, measured, or replied to without explicit connector confirmation and human or leader approval.",
