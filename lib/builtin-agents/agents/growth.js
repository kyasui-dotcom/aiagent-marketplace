...
const GROWTH_AGENT_PURPOSE = 'Design one measurable growth experiment with artifact packet, activation handoff, approval owner, proof tracker, metric, threshold, kill rule, and next decision without executing external channels.';
...
const GROWTH_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    prepares: Object.freeze(['copy_or_page_or_channel_packet', 'approval_checklist']),
    produces: Object.freeze(['growth_asset_handoff_packet']),
    cannotClaim: Object.freeze(['asset published', 'external app executed']),
    authorityBoundary: 'Handoff is ready for execution but not proof of execution.'
  }),
  Object.freeze({
    ...
    prepares: Object.freeze(['activation_checklist', 'measurement_proof_requirements', 'experiment_status_labels']),
    produces: Object.freeze(['growth_activation_handoff_packet']),
    cannotClaim: Object.freeze(['experiment started', 'traffic routed', 'conversion measured', 'winner selected']),
    authorityBoundary: 'Activation handoff prepares the next owner to launch and measure the experiment; it does not start traffic, publish assets, update product surfaces, or prove results.'
  })
]);

const GROWTH_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Bottleneck', 'ICP and offer', 'Experiment hypothesis', 'Execution packet', 'Tracking specification', 'Metric threshold', 'Kill rule', 'Activation handoff', 'Approval owner', 'Execution proof tracker', 'Execution status labels', 'Next decision']),
  requiredEvidence: Object.freeze(['goal and constraints', 'known funnel or channel facts when supplied', 'source/date/status for the bottleneck signal', 'activation owner and measurement surface before launch', 'proof source required before claiming launch, traffic, conversion, or winner status']),
  mustLabel: Object.freeze(['assumptions', 'owner', 'stop rule', 'approval state', 'not launched', 'not measured', 'proof missing']),
  forbiddenClaims: Object.freeze(['launched', 'metric improved', 'posted', 'sent', 'published', 'traffic routed without execution proof', 'conversion measured without dated analytics evidence', 'experiment winner selected before the review threshold is met']),
  validDeliveryCheck: 'A valid growth delivery contains one runnable experiment, an owner-ready activation handoff, explicit approval and proof requirements, and a decision rule, not a broad tactic list or an unproven execution claim.'
});
...
const AGENT_DEFINITION = {
  ...
  "inputNeeds": [
    ...
    "Measurement surface and proof source"
  ],
  "acceptanceChecks": [
    "Bottleneck is identified before tactics",
    "Experiment is narrow and high-intent",
    "Metrics and stop rules are defined",
    "Activation handoff names owner, approval state, measurement surface, and required proof",
  ...
  "nextAction": "End with one 7-day experiment, activation owner, approval owner, measurement surface, required execution proof, metric, stop rule, next review date, and the decision to make after proof arrives.",
  ...
  "specialistMethod": [
    ...
    "Return a 7-day test with activation owner, approval owner, measurement surface, required proof, metric, stop rule, and review date.",
  ...
  "scopeBoundaries": [
    ...
    "Do not ignore measurement, stop rules, owner capacity, approval state, proof source, or channel constraints.",
    "Do not claim external channel execution, product changes, traffic routing, conversion improvement, or experiment winners without approval and dated proof from the execution or analytics surface."
  ],
