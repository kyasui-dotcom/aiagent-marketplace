...
const LIST_CREATOR_AGENT_PURPOSE = 'Prepare source-traceable prospect or target lists with row-level evidence, qualification reasons, field schema, review status, downstream handoff, and import/outreach proof boundary.';
...
const LIST_CREATOR_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    produces: Object.freeze(['crm_or_outreach_import_review_packet']),
...
const LIST_CREATOR_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Target segment', 'Source policy', 'Qualification rules', 'Field schema', 'Row-level source ledger', 'Exclusion and duplicate review', 'Review status', 'Approval owner', 'Import/outreach boundary', 'Downstream handoff packet', 'Execution proof tracker', 'Next owner']),
  requiredEvidence: Object.freeze(['source policy', 'qualification criteria', 'source URL and source date or explicit gap per row', 'observed fit signal per row', 'public contact source or contact-missing label', 'exclusion or duplicate-risk reason when applicable', 'approval owner before import or outreach', 'proof fields for CRM import, enrichment, queueing, or outreach']),
  mustLabel: Object.freeze(['unverified row', 'source needed', 'contact missing', 'duplicate risk', 'approval required', 'not approved', 'not imported', 'not enriched', 'not contacted', 'connector proof missing']),
  ...
  validDeliveryCheck: 'A valid list creator delivery is traceable row by row, reviewable before any import or outreach, and includes approval ownership, downstream field mapping, and execution-proof requirements.'
});
...
const AGENT_DEFINITION = {
  ...
  "seedProfile": {
    ...
    "description": "Built-in lead-list creation agent that turns ICP, public sources, and homepage qualification into reviewable company-by-company lead rows, targeting notes, row-level evidence, and proof-ready handoff packets for outbound specialists.",
    "taskTypes": [
      "list_creator",
      "lead_sourcing",
      "lead_qualification",
  ...
  "nextAction": "End with the requested companies to review, the batch/count estimate, why each fits, the target role hypothesis, the public email or safe contact path, the contact-source URL, the import-ready field map, approval owner, proof fields to capture, not-approved/not-imported/not-enriched/not-contacted labels, and whether the next handoff should go to cold_email, CRM import review, or manual review.",
