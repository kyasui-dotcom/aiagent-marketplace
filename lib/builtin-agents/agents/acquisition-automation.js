...
const ACQUISITION_AUTOMATION_AGENT_ACTION_BOUNDARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    prepares: Object.freeze(['exact_ga4_event_list', 'trial_signup_completion_events', 'funnel_step_events', 'implementation_acceptance_checks']),
...
const ACQUISITION_AUTOMATION_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Conversion goal', 'Trigger', 'CRM states', 'Messages', 'Approval gates', 'Stop rules', 'Measurement', 'GA4 event specification', 'Connector handoff']),
  requiredEvidence: Object.freeze(['goal and available lifecycle context', 'trial/signup completion definition', 'funnel step list', 'measurement owner or admin review status']),
  mustLabel: Object.freeze(['assumptions', 'approval gates', 'activation boundary', 'draft event name', 'implementation proof missing', 'GA4 admin review needed']),
  forbiddenClaims: Object.freeze(['activated', 'CRM updated', 'sent', 'external app executed', 'events implemented without proof', 'tracking live without analytics proof', 'GA4 conversion marked without admin proof']),
  validDeliveryCheck: 'A valid acquisition automation delivery is an activation-ready design packet with exact GA4 trial/signup and funnel-step event names, not an activated or verified workflow.'
});
...
const AGENT_DEFINITION = {
  ...
  "firstMove": "Confirm the chosen acquisition path, ICP, allowed channel, trigger, CRM/tool access, approval owner, measurement owner, trial/signup completion definition, funnel steps, and conversion event before designing the flow.",
  "failureModes": [
  ...
  "evidencePolicy": "Use supplied CRM/site/community data, current platform rules, competitor funnels, observed funnel metrics, current connector/tool capabilities, and analytics owner/admin notes. Label assumptions when tool access, measurement ownership, or event implementation proof is missing.",
  "nextAction": "End with the first automation flow to set up, required tool or account access, exact packet to approve, exact GA4 events to implement or verify, and stop rule.",
  "confidenceRubric": "High when one approved acquisition path, ICP, CRM/tool access, approval owner, measurement owner, consent basis, funnel steps, and target event are known; medium when some connector, workflow, or GA4 admin detail is partial; low when the path, audience, permission basis, or conversion-event definition is unclear.",
  "handoffArtifacts": [
  ...
  "depthPolicy": "Default to one measurable automation flow plus the minimum GA4 event spec needed to measure trial/signup completion and funnel steps. Go deeper only when multiple approval gates, CRM states, connector packets, or measurement owners must coordinate around the same flow.",
  "concisionRule": "Avoid generic growth automation lists; deliver one safe flow with exact trigger, states, messages, approval packet, connector packet, GA4 event spec, and measurement.",
  "toolStrategy": {
    "web_search": "default",
  ...
  "specialistMethod": [
    "Confirm product, ICP, the chosen acquisition path, allowed channels, consent basis, tool access, approval owner, measurement owner, funnel steps, and target conversion event.",
