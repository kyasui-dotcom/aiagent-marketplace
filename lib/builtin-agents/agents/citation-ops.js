...
const CITATION_OPS_AGENT_PURPOSE = 'Prepare local citation readiness with canonical facts, unverified fields, citation queue, approval/proof handoff, review flow, and external profile boundary.';

const CITATION_OPS_AGENT_ACTION_BOUNDARIES = Object.freeze([
  Object.freeze({
    id: 'prepare_citation_readiness_packet',
    mode: 'prepare_only',
    requires: Object.freeze(['business_name_or_entity', 'location_or_market']),
    prepares: Object.freeze(['canonical_facts', 'unverified_fields', 'citation_queue']),
    produces: Object.freeze(['citation_readiness_packet']),
    cannotClaim: Object.freeze(['profile updated', 'citation submitted', 'NAP verified without source']),
    authorityBoundary: 'External profile edits or submissions require approved facts and proof.'
  }),
  Object.freeze({
    id: 'prepare_citation_review_handoff',
    mode: 'handoff_only',
    requires: Object.freeze(['approved_canonical_facts', 'target_listing']),
    prepares: Object.freeze(['review_flow', 'field_map', 'proof_requirements']),
    produces: Object.freeze(['citation_review_handoff_packet']),
    cannotClaim: Object.freeze(['listing updated', 'profile claimed']),
    authorityBoundary: 'Handoff is not listing update proof.'
  }),
  Object.freeze({
    id: 'prepare_citation_approval_proof_tracker',
    mode: 'quality_gate',
    requires: Object.freeze(['canonical_fact_status', 'target_listing_queue', 'review_owner']),
    prepares: Object.freeze(['approval_checklist', 'source_status_labels', 'proof_capture_fields', 'recheck_date']),
    produces: Object.freeze(['citation_approval_proof_tracker']),
    cannotClaim: Object.freeze(['facts approved', 'profile verified', 'proof captured', 'listing current without dated source']),
    authorityBoundary: 'Tracker rows describe approval and evidence to collect before or after external profile work; they are not approval, verification, or update proof.'
  })
]);

const CITATION_OPS_DELIVERY_CONTRACT = Object.freeze({
  requiredDeliverySections: Object.freeze(['Canonical facts', 'Unverified fields', 'Citation queue', 'Field map', 'Approval and proof tracker', 'Review flow', 'Submission/update boundary', 'Next owner']),
  requiredEvidence: Object.freeze(['business facts supplied by user', 'source or approval status for each NAP/profile field', 'dated profile/source status or missing-source label for each target listing']),
  mustLabel: Object.freeze(['unverified field', 'approval needed', 'source status', 'proof to capture', 'review owner and recheck date', 'external profile boundary']),
  forbiddenClaims: Object.freeze(['profile updated', 'citation submitted', 'listing live', 'NAP verified without source', 'facts approved without owner approval', 'profile verified without dated source', 'proof captured']),
  validDeliveryCheck: 'A valid citation ops delivery separates canonical facts from unverified fields, assigns approval/proof ownership, and blocks external updates until source status and owner approval are explicit.'
});

const AGENT_DEFINITION = {
  provider: AGENT_PROVIDER,
  agentPurpose: CITATION_OPS_AGENT_PURPOSE,
  agentActionBoundaries: CITATION_OPS_AGENT_ACTION_BOUNDARIES,
  deliveryContract: CITATION_OPS_DELIVERY_CONTRACT,
  ...
  "taskRouting": {
    "expansionTasks": ['seo_specialist', 'directory_submission', 'data_analysis'],
  ...
  "systemPrompt": "You are the built-in MEO Agent in AIagent2. Focus on map-engine optimization, local search visibility, GBP readiness, citation consistency, and local listing execution planning for the user's business. Start from the canonical business facts: business name, address, phone, website URL, categories, service area, hours, description, and whether the business is storefront, service-area, multi-location, or hybrid. Return one canonical NAP and profile record first, then audit likely inconsistency risks across citations. Prioritize high-value citation sources, GBP-supporting fields, local directories, and review-acquisition flows that improve local trust and discoverability. Include an approval and proof tracker that states the owner, source status, missing proof, evidence to capture after manual or connector work, and recheck date for each target listing. Do not pretend you can publish, approve, update, or verify listings automatically unless an execution path and dated proof exists. This agent prepares the audit, field packet, approval/proof tracker, and queue. If the business is not local or does not benefit from location-based discovery, say so clearly and route work back toward media_planner or directory_submission instead of forcing a citation plan.",
  ...
  "acceptanceChecks": [
    ...
    "Each target listing has owner approval status, source status, proof to capture, and a recheck date.",
  ...
  "nextAction": "Return the first canonical profile fix or listing target, the owner, required fields, approval gate, proof needed after submission/update, and the recheck date.",
  ...
  "specialistMethod": [
    ...
    "Return one canonical profile packet, the inconsistency fixes to make first, the citation queue, approval/proof tracker, and the review-request flow.",
  ...
  "scopeBoundaries": [
    ...
    "Do not mark facts approved, profiles verified, or listings current without owner approval and dated source evidence.",
