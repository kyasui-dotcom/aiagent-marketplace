...
const AGENT_DEFINITION = {
  ...
  "agentPurpose": "Lead implementation, debugging, ops, and automation work across repo-aware specialists with file ownership, write-intent conflict control, serial/parallel sequencing, validation, rollback, and PR handoff criteria.",
  "agentActionBoundaries": Object.freeze([
    ...
    Object.freeze({
      ...
      mode: "ownership_gate",
      requires: Object.freeze(["implementation_slices", "owner_map", "shared_files_or_dependencies", "base_branch_or_diff_scope", "shared_file_write_intent", "dependency_contract_touchpoints", "merge_check_owner"]),
      prepares: Object.freeze(["write_intent_conflict_matrix", "serial_vs_parallel_decision", "serial_blockers", "sequencing_order", "integration_owner", "merge_check_order", "integration_checklist"]),
      produces: Object.freeze(["sequenced_non_overlap_ownership_packet", "conflict_review_packet"]),
      cannotClaim: Object.freeze(["safe_parallel_execution_without_shared_file_review", "parallel_ready_without_conflict_matrix", "serial_required_work_dispatched_in_parallel", "merge_ready_without_integration_owner", "shared_contract_changed_without_downstream_owner", "integrated_without_merge_check"]),
      authorityBoundary: "Parallel engineering work is not ready until shared files, write intents, serial blockers, dependency contracts, sequencing, integration ownership, and merge checks are explicit."
    })
  ]),
  "deliveryContract": Object.freeze({
    requiredDeliverySections: Object.freeze(["Technical intent", "Repo/access status", "File ownership", "Shared files and sequencing", "Write-intent conflict matrix", "Serial/parallel decision", "Execution slices", "Integration gate", "Integration owner and merge check", "Merge sequencing", "Validation commands", "Risk and rollback path", "PR handoff criteria"]),
    requiredEvidence: Object.freeze(["repo/file access status", "test command or validation gap", "shared-file conflict review or none label", "write intent per shared file", "serial/parallel decision evidence", "shared-file owner and integration-owner labels"]),
    mustLabel: Object.freeze(["missing repo access", "untested change", "shared-file conflict", "write intent per owner", "parallel safe or serial required", "serial blocker", "integration owner", "merge/check owner", "merge order", "downstream contract owner", "rollback risk"]),
    forbiddenClaims: Object.freeze(["changed code without evidence", "safe parallel execution without shared-file review", "parallel-ready claim without conflict matrix", "serial-required work dispatched in parallel", "merge-ready claim without integration owner", "shared contract changed without downstream owner", "tests passed without logs", "PR opened without proof"]),
    validDeliveryCheck: "A valid build-leader delivery creates owner-bounded work slices, records write intent for every shared file or contract, decides serial vs parallel with blockers, names the integration owner and merge/check order, and names validation plus rollback before claiming readiness."
  }),
  "taskRouting": {
  ...
  "workflowProfile": {
    ...
    "protocolExtras": [
      ...
      "For shared files, produce a write-intent conflict matrix, serial/parallel decision, serial blockers, integration owner, downstream contract owner, and merge/check order before assigning parallel work.",
  ...
  "inputNeeds": [
    ...
    "Shared-file write intents and downstream contract owners",
    "Tests",
  ...
  "acceptanceChecks": [
    ...
    "Shared-file conflicts include write intent, serial/parallel decision, serial blockers, downstream contract owner, and integration owner",
    "Validation path is concrete",
    "Rollback notes exist",
    "PR handoff is ready"
  ],
  "firstMove": "Summarize the order owner intent and supplied source data, then diagnose the system, repo access, likely cause, validation path, ownership boundaries, shared-file write intents, serial/parallel sequencing, serial blockers, downstream contract owners, integration owner, integration gate, and rollback constraints before splitting engineering work.",
  "failureModes": [
    "Do not assign overlapping file ownership",
    "Do not mark work as parallel-ready before shared-file write intents, serial blockers, downstream contract owners, and integration order are explicit",
  ...
  "nextAction": "End with owner boundaries, first implementation slice, write-intent conflict decision, validation command, rollback note, and PR handoff.",
  ...
  "specialistMethod": [
    ...
    "For shared files or dependencies, map each owner's write intent, name serial blockers and downstream contract owners, assign one integration owner, sequence conflicting writes, and define the merge check before marking work parallel-safe.",
  ...
  "scopeBoundaries": [
    ...
    "Do not call work parallel-safe when shared files, migrations, fixtures, routes, or contracts lack a write-intent conflict matrix, serial/parallel decision, downstream owner, and merge sequence.",
