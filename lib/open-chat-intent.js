...
function boundedOpenChatInteger(value = '', fallback = 0, min = 0, max = Number.MAX_SAFE_INTEGER) {
  const raw = String(value ?? '').trim();
  if (!raw) return fallback;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || (parsed <= 0 && min > 0)) return fallback;
  return Math.max(min, Math.min(max, Math.round(parsed)));
}

function openChatIntentTimeoutMs(source = {}) {
  return boundedOpenChatInteger(openChatIntentEnvValue(source, 'OPEN_CHAT_INTENT_TIMEOUT_MS'), 8000, 1000, 20000);
}

function openChatIntentContextLimit(source = {}, key = '', fallback = 40, min = 1, max = 500) {
  return boundedOpenChatInteger(openChatIntentEnvValue(source, key), fallback, min, max);
}
...
function openChatIntentSystemPrompt(userLanguage = 'English', uiLabels = {}) {
  return [
    ...
    'For thin broad requests such as "I want more customers", "do marketing", "improve this", or similarly short goals without a target/product/audience/source context, do not invent those missing details. Use action="ask_clarifying_question".',
...
export function createOpenChatIntentSupport(deps = {}) {
  ...
  async function authorizeOpenChatIntentLlm(storage, request, env = {}) {
    let current = lightweightCurrentFromSession(await getSession(request, env));
    if (!current?.user && typeof currentOrderRequesterContext === 'function') {
      const apiCurrent = await currentOrderRequesterContext(storage, request, env, { lightweight: true });
      if (apiCurrent?.apiKeyStatus === 'valid') current = apiCurrent;
      if (apiCurrent?.apiKeyStatus === 'invalid') {
        return {
          ok: false,
          statusCode: 401,
      ...
      if (apiCurrent?.apiKeyStatus === 'disabled') {
        return {
          ok: false,
          statusCode: 403,
          error: 'CAIt developer API and API key access are temporarily disabled.',
          source: 'api-key'
        };
      }
    }
  ...
  async function lazyOpenChatRuntimeState(storage, current = null, env = {}) {
    const agentLimit = openChatIntentContextLimit(env, 'OPEN_CHAT_INTENT_AGENT_CONTEXT_LIMIT', 160, 20, 500);
    const reviewedLimit = openChatIntentContextLimit(env, 'OPEN_CHAT_INTENT_REVIEWED_CONTEXT_LIMIT', 40, 0, 120);
    const memoryLimit = openChatIntentContextLimit(env, 'OPEN_CHAT_INTENT_MEMORY_CONTEXT_LIMIT', 40, 0, 120);
    const [agents, trainingTranscripts, memory] = await Promise.all([
      typeof storage.listAgents === 'function' ? storage.listAgents({ limit: agentLimit }) : null,
      reviewedLimit && typeof storage.listChatTranscripts === 'function' ? storage.listChatTranscripts({ reviewStatus: 'fixed', limit: reviewedLimit }) : null,
      current?.login && memoryLimit ? d1ChatMemoryTranscriptsForCurrent(env, current, memoryLimit) : []
    ]);
  ...
  async function classifyOpenChatIntentWithOpenAi(body = {}, env = {}, options = {}) {
    ...
    const conversationContext = Array.isArray(body.conversation_context || body.conversationContext)
      ...
      : [];
    const timeoutMs = openChatIntentTimeoutMs(env);
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        return { ok: false, available: false, source: 'openai', code: 'openai_intent_timeout', error: `OpenAI intent classification timed out after ${timeoutMs}ms` };
      }
      return { ok: false, available: false, source: 'openai', error: String(error?.message || error) };
    } finally {
