...
function audienceFromContent(content = '') {
  const text = String(content || '');
  const audiences = [];
  for (const match of text.matchAll(/Target audience\s*[:：]\s*-?\s*Target audience\s*[:：]\s*([^\n]+)/gi)) {
    audiences.push(safeText(match[1], 120));
  }
...
function genericDeliveryHeading(title = '') {
  ...
  if (!text) return true;
  return /^(landing page critique|seo agent|writer|writing|x ops connector|reddit|indie hackers|instagram|directory submission|.* provider delivery|.* delivery)$/.test(text);
}
...
function inferItemType({ surface = '', taskType = '', fileName = '', content = '', artifact = {} } = {}) {
  ...
  if (surface === 'publisher') {
    ...
    if (/seo_gap|seo-agent|seo_article|seo article/.test(taskFile)) return 'seo_article';
...
function surfaceForTask(taskType = '', inferredSurface = '', raw = {}) {
  ...
  if (/seo_gap|writing|writer|landing|landing_page|directory_submission|x_ops|x_post|twitter|social_post|instagram|reddit|indie_hackers/.test(task)) return 'publisher';
...
function normalizeDeliveryItem(raw = {}, job = {}, index = 0) {
  ...
  const itemType = safeText(raw.itemType || raw.item_type || inferItemType({ surface, taskType, fileName, content: rawContent, artifact: raw }), 80);
...
export function sanitizeDeliveryItemForSurface(item = {}) {
  ...
  const itemType = safeText(item.itemType || item.item_type || '', 80).toLowerCase();
