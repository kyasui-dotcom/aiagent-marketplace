const X_OAUTH_READ_SCOPES = ['tweet.read', 'users.read'];
const X_OAUTH_WRITE_SCOPES = ['tweet.read', 'tweet.write', 'users.read', 'offline.access'];
...
function xOAuthScopesForCapabilities(capabilities = []) {
  const requested = Array.isArray(capabilities) ? capabilities : String(capabilities || '').split(/[,\s]+/);
  const needsWrite = requested.some((item) => /x\.(?:post|schedule_post|write|tweet_write)|tweet\.write|post/i.test(String(item || '')));
  return needsWrite ? X_OAUTH_WRITE_SCOPES : X_OAUTH_READ_SCOPES;
}

export function buildXAuthorizeUrl(env = {}, { callbackUrl = '', state = '', codeChallenge = '', capabilities = [] } = {}) {
  ...
  url.searchParams.set('scope', xOAuthScopesForCapabilities(capabilities).join(' '));
...
export async function xConnectorFromOAuthToken(env = {}, profile = {}, token = {}, existing = {}) {
  ...
  const scopes = [...new Set([
    ...normalizeString(existing?.scopes).split(/\s+/),
    ...normalizeString(token.scope || token.scopes).split(/\s+/)
...
export function validateXPostExecutionApproval(connector = null, action = {}) {
