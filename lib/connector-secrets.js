...
export async function googleConnectorFromOAuthToken(env = {}, identity = {}, token = {}, existing = {}, requestedScopes = '') {
  const now = new Date().toISOString();
  const expiresIn = Number(token?.expires_in || 0);
  const tokenScopes = normalizeString(token?.scope);
  const scopes = [...new Set([
    ...normalizeString(existing?.scopes).split(/\s+/),
    ...tokenScopes.split(/\s+/),
    ...(!tokenScopes ? normalizeString(requestedScopes).split(/\s+/) : [])
