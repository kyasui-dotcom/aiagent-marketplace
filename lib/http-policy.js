...
export function rateLimitSpecForPath(pathname = '', method = 'GET') {
  ...
  if (pathname === '/auth/status' && verb === 'GET') return { name: 'auth-status', limit: 600, windowMs: 60_000 };
  ...
  if (pathname === API_ROUTES.DELIVERIES_PREPARE_FOLLOWUP_ORDER && verb === 'POST') return { name: 'delivery-prepare-followup-order', limit: 60, windowMs: 60_000 };
  ...
  if (pathname === API_ROUTES.PUBLISHER_CAMPAIGN_INGEST && verb === 'POST') return { name: 'publisher-campaign-ingest', limit: 60, windowMs: 60_000 };
  ...
  if (pathname === API_ROUTES.CONNECTORS_WORDPRESS_CONNECT && verb === 'POST') return { name: 'wordpress-connect', limit: 20, windowMs: 10 * 60_000 };
  if (pathname === API_ROUTES.CONNECTORS_WORDPRESS_CREATE_DRAFT && verb === 'POST') return { name: 'wordpress-create-draft', limit: 20, windowMs: 10 * 60_000 };
  ...
  if (pathname === API_ROUTES.CAMPAIGNS && verb === 'POST') return { name: 'campaign-create', limit: 60, windowMs: 60_000 };
  if (/^\/api\/campaigns\/[^/]+$/.test(pathname) && verb === 'POST') return { name: 'campaign-update', limit: 120, windowMs: 60_000 };
  if (/^\/api\/campaigns\/[^/]+\/metrics$/.test(pathname) && verb === 'POST') return { name: 'campaign-metrics', limit: 120, windowMs: 60_000 };
  if (/^\/api\/campaigns\/[^/]+\/integrations$/.test(pathname) && verb === 'POST') return { name: 'campaign-integrations', limit: 120, windowMs: 60_000 };
  if (/^\/api\/campaigns\/[^/]+\/lead-source$/.test(pathname) && verb === 'POST') return { name: 'campaign-lead-source', limit: 120, windowMs: 60_000 };
  if (/^\/api\/campaigns\/[^/]+\/ads$/.test(pathname) && verb === 'POST') return { name: 'campaign-ads', limit: 80, windowMs: 60_000 };
  ...
  if (pathname.startsWith('/api/payjp/') && verb === 'POST') return { name: 'payjp-action', limit: 40, windowMs: 60_000 };
