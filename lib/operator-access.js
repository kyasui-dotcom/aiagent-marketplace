...
export function runtimePolicy(env) {
  ...
  const developerApiEnabled = boolFlag(
    env?.CAIT_DEVELOPER_API_ENABLED ?? env?.CAIT_EXTERNAL_API_ENABLED ?? env?.CAIT_API_KEYS_ENABLED,
    false
  );
  const cliEnabled = boolFlag(env?.CAIT_CLI_ENABLED, developerApiEnabled);
  const mcpEnabled = boolFlag(env?.CAIT_MCP_ENABLED, false);
  ...
  return {
    ...
    cliEnabled,
    mcpEnabled,
    developerSurfacesPaused: !developerApiEnabled,
    exposeJobSecrets,
