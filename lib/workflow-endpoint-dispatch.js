export function createWorkflowEndpointDispatchHelpers(dependencies = {}) {
  ...
  async function dispatchJobToAssignedAgent(job, agent, env) {
    ...
    const internalDispatchResult = typeof runInternalEndpointDispatch === 'function'
      ? await runInternalEndpointDispatch({ endpoint, dispatchEndpoint, payload, env, agent, job, timeoutMs, headers: dispatchHeaders })
      : null;
    const dispatchResult = internalDispatchResult || await postJsonWithTimeout(dispatchEndpoint, payload, timeoutMs, dispatchHeaders);
    const { response, body } = dispatchResult;
    const observedEndpoint = dispatchResult.endpoint || dispatchEndpoint;
    if (!response.ok) {
