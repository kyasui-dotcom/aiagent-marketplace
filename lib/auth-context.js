export function createAuthContextHelpers(deps = {}) {
  ...
  function sessionAccountRepairLogin(session = null, account = null) {
    ...
    return String(
      ...
      || account?.login
      || session?.user?.login
      || defaultLoginForAuthUser(identityUser, sessionAuthProvider(session))
      || ''
    ).trim().toLowerCase();
  }
  ...
  async function currentUserContext(request, env, options = {}) {
    ...
    const sessionLogin = sessionAccountRepairLogin(session, null);
    if (!sessionLogin) return { session: null, user: null, login: '', authProvider: 'guest' };
    if (sessionAuthProvider(session) === 'e2e') {
      return {
        session,
        user: session.user || null,
        login: session.accountLogin || session.user?.login || sessionLogin,
        authProvider: 'e2e',
        account: null,
        githubIdentity: null,
        googleIdentity: null,
  ...
  async function oauthCallbackCurrentContext(storage, request, env, session = null) {
    ...
    const sessionLogin = sessionAccountRepairLogin(existingSession, null);
    if (!sessionLogin) {
    ...
    if (sessionAuthProvider(existingSession) === 'e2e') {
      return {
        ...
        login: existingSession.accountLogin || existingSession.user?.login || sessionLogin,
        authProvider: 'e2e',
        account: null,
        githubIdentity: null,
        googleIdentity: null,
    ...
    const login = String(existingSession.accountLogin || existingSession.user?.login || sessionLogin).trim().toLowerCase();
  ...
  function disabledOrderApiKeyContext() {
    return {
      session: null,
      user: null,
      login: '',
  ...
  async function currentOrderRequesterContext(storage, request, env, options = {}) {
    ...
    if (!runtimePolicy(env).developerApiEnabled) return disabledOrderApiKeyContext();
  ...
  function requireOrderWriteAccess(current, env) {
    ...
    if (current?.apiKeyStatus === 'disabled') return { error: 'CAIt developer API and API key access are temporarily disabled.', statusCode: 403, current, policy };
