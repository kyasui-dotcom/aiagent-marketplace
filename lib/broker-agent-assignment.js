export function createBrokerAgentAssignmentHelpers({
  ...
  function assignAgentForTask(agents, taskType, budgetCap = 0, requestedAgentId = '', options = {}) {
    ...
    const exactSpecialistCandidateExists = !requestedIsLeader && verified.some((agent) => {
      if (agentRecordIsWorkflowLeader(agent)) return false;
      return taskMatchForAgent(agent, taskType, options).exact === true;
    });
    ...
    const ranked = verified
      .map((agent) => {
        ...
        if (exactSpecialistCandidateExists && match.exact !== true) return null;
        return {
          agent,
