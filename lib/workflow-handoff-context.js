...
export function createWorkflowHandoffContext(deps = {}) {
  ...
  function workflowHandoffPhaseRules(job = {}, workflow = {}) {
    ...
    const rules = [
      ...
      'Use prior user-facing deliverables as the primary reference material. The supporting fact index and execution program are only to prevent evidence loss; do not copy their labels or internal structure into the user delivery.'
    ];
    if (isWorkflowLeaderTask(task)) {
      rules.push('Leader checkpoint/final summary must read the prior user-facing deliverables first, then synthesize an accountable user-facing delivery. Internal fact-index fields must remain internal.');
      rules.push('Leader checkpoint/final summary must choose the next executable lane or final accountable delivery after reviewing the incoming user-facing work; do not delegate this synthesis to orchestration.');
  ...
  function workflowHandoffPromptContext(job = {}) {
    ...
    const includeMarkdownExcerpt = ['planning', 'preparation', 'action', 'implementation', 'checkpoint', 'final_summary'].includes(currentPhase);
    ...
    const lines = [
      ...
      priorRuns.length
        ? `USER-FACING PRIOR DELIVERABLES (primary reference material): use at least ${minimumPriorUse} distinct prior work item${minimumPriorUse === 1 ? '' : 's'} when available.`
        : 'USER-FACING PRIOR DELIVERABLES (primary reference material): none yet.',
      ...
      unavailablePriorRuns.length
        ...
        : '',
      ...unavailableLines,
      'Required output behavior: produce a user-facing Markdown deliverable for this phase, explicitly use the prior user-facing work where available, preserve source URLs when provided, and return concrete decisions/artifacts. If required prior work is unusable, say what is missing in user-facing terms instead of exposing internal handoff fields.',
