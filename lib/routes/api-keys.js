...
export function createApiKeyRouteHandlers(deps = {}) {
  ...
  async function touchApiKeyEvent(storage, type, message, meta = {}) {
    try {
      await touchEvent(storage, type, message, meta);
    } catch (error) {
      logError('api key event write failed', error);
    }
  }

  async function currentApiKeyUser(request, env) {
    if (typeof getSession === 'function' && typeof lightweightCurrentFromSession === 'function') {
      return lightweightCurrentFromSession(await getSession(request, env));
    }
    return currentUserContext(request, env);
  }
  ...
  async function authorizeCaitAdminApiKeyIssuer(request, env) {
    ...
    const current = await currentApiKeyUser(request, env);
  ...
  async function createOrderApiKey(storage, request, env) {
    ...
    await touchApiKeyEvent(storage, 'API_KEY', `${current.login} issued ${created.apiKey.mode} CAIt API key ${created.apiKey.label}`);
    return {
      ok: true,
  ...
  async function createAdminOrderApiKey(storage, request, env) {
    ...
    await touchApiKeyEvent(storage, 'API_KEY', `${authorization.actor} issued ${created.apiKey.mode} CAIt API key ${created.apiKey.label} for ${target.login}`, {
  ...
  async function revokeOrderApiKey(storage, request, env, keyId) {
    ...
    await touchApiKeyEvent(storage, 'API_KEY', `${current.login} revoked CAIt API key ${revoked.apiKey.label}`);
    return {
      ok: true,
