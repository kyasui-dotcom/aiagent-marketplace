export function createFeedbackChatRouteHandlers(deps = {}) {
  ...
  async function recordChatSessionSnapshot(storage, request, env) {
    ...
    let current = await currentUserContext(request, env);
    if (!current?.login && typeof currentOrderRequesterContext === 'function') {
      const apiCurrent = await currentOrderRequesterContext(storage, request, env, { lightweight: true });
      if (apiCurrent?.apiKeyStatus === 'invalid') return { error: 'Invalid API key', statusCode: 401 };
      if (apiCurrent?.apiKeyStatus === 'disabled') return { error: 'CAIt developer API and API key access are temporarily disabled.', code: 'developer_api_disabled', statusCode: 403 };
    ...
    if (!chatSessionSnapshotAccountKey(body, body?.session && typeof body.session === 'object' ? body.session : body)) {
      body = { ...body, accountKey: current.login };
    }
