...
export function createDeliveryRouteHandlers(deps = {}) {
  ...
  function followupContractFromRequest(body = {}) {
    const deliverable = body?.deliverable && typeof body.deliverable === 'object' ? body.deliverable : {};
    if (body?.followup_contract && typeof body.followup_contract === 'object') return body.followup_contract;
    if (body?.followupContract && typeof body.followupContract === 'object') return body.followupContract;
    if (deliverable?.followup_contract && typeof deliverable.followup_contract === 'object') return deliverable.followup_contract;
    if (deliverable?.followupContract && typeof deliverable.followupContract === 'object') return deliverable.followupContract;
    return {};
  }

  function explicitFollowupTaskTypeFromRequest(job = null, body = {}) {
    const deliverable = body?.deliverable && typeof body.deliverable === 'object' ? body.deliverable : {};
    const draft = body?.draft && typeof body.draft === 'object' ? body.draft : {};
    const contract = followupContractFromRequest(body);
    const candidates = [
      body.followup_task_type,
      body.followupTaskType,
      contract.task_type,
      contract.taskType,
      deliverable.followup_task_type,
      deliverable.followupTaskType,
      deliverable.meta?.followup_task_type,
      deliverable.meta?.followupTaskType,
      draft.followup_task_type,
      draft.followupTaskType,
      job?.taskType
    ];
    return String(candidates.find((item) => String(item || '').trim()) || 'research').trim() || 'research';
  }

  function explicitFollowupOrderStrategyFromRequest(job = null, body = {}) {
    const deliverable = body?.deliverable && typeof body.deliverable === 'object' ? body.deliverable : {};
    const draft = body?.draft && typeof body.draft === 'object' ? body.draft : {};
    const contract = followupContractFromRequest(body);
    const explicit = String(
      body.order_strategy
      || body.orderStrategy
      || contract.order_strategy
      || contract.orderStrategy
      || deliverable.followup_order_strategy
      || deliverable.followupOrderStrategy
      || draft.followup_order_strategy
      || draft.followupOrderStrategy
      || ''
    ).trim();
    if (explicit) return normalizeOrderStrategy(explicit);
    return job?.jobKind === 'workflow' || job?.workflow ? 'multi' : 'single';
  }

  function compactFollowupSelectionLines(body = {}) {
    const deliverable = body?.deliverable && typeof body.deliverable === 'object' ? body.deliverable : {};
    const draft = body?.draft && typeof body.draft === 'object' ? body.draft : {};
    return [
      ['Delivery type', deliverable.type || body.content_type || body.contentType],
      ['Selected next step', draft.nextStep],
      ['Selected channel', draft.channel],
      ['Selected target', draft.target],
      ['Selected mode', draft.actionMode || draft.executionMode || draft.publishMode],
      ['Selected repository', draft.repoFullName],
      ['Selected recipient', draft.recipientEmail],
      ['Selected subject', draft.emailSubject],
      ['Search Console site', draft.googleSearchConsoleSite],
      ['GA4 property', draft.googleGa4Property],
      ['Google Drive file', draft.googleDriveFileId],
      ['Google Calendar', draft.googleCalendarId],
      ['Gmail label', draft.googleGmailLabelId]
    ]
      .map(([label, value]) => {
  ...
  function buildGenericDeliveryFollowupOrderBody(job = null, body = {}) {
    ...
    const taskType = explicitFollowupTaskTypeFromRequest(job, body);
    const orderStrategy = explicitFollowupOrderStrategyFromRequest(job, body);
    let promptLines = [];
    ...
    if (deliverable.content || String(body.answer || body.followup_answer || body.followupAnswer || '').trim()) {
      promptLines = [
        `Follow-up request for previous order ${job?.id || ''}:`,
        '',
        `Previous delivery: ${title}`,
        ...compactFollowupSelectionLines(body),
        String(body.answer || body.followup_answer || body.followupAnswer || '').trim()
          ? `User follow-up answer: ${String(body.answer || body.followup_answer || body.followupAnswer || '').trim()}`
          : '',
        '',
        'Use input._broker.conversation.previousJob and the selected UI values above as context. Follow the assigned agent or leader contract for task-specific output, routing, approvals, and next steps.'
      ];
    } else {
      const answer = String(body.answer || body.followup_answer || body.followupAnswer || '').trim();
      promptLines = [
        ...
        '',
        'Use the previous order context and the assigned agent or leader contract to decide the next delivery.'
      ];
    }
    return {
      ok: true,
