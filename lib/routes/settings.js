...
export function createSettingsRouteHandlers(deps = {}) {
  ...
  async function saveSettingsSection(storage, request, env, section) {
    ...
    const billingPatch = section === 'billing' || section === 'costLimits'
      ? sanitizeBillingSettingsPatch(body || {})
      : null;
    const patch = section === 'billing'
      ? billingPatch
      : section === 'costLimits'
        ? { openAiMonthlyCostLimit: billingPatch.openAiMonthlyCostLimit }
        : section === 'payout'
        ? sanitizePayoutSettingsPatch(body || {})
        : section === 'profile'
          ? sanitizeProfileSettingsPatch(body || {})
        : section === 'executorPreferences'
          ? sanitizeExecutorPreferencesPatch(body || {})
        : (body || {});
    let account = null;
    const updateSection = section === 'costLimits' ? 'billing' : section;
    await storage.mutate(async (draft) => {
      account = upsertAccountSettingsInState(draft, current.login, current.user, current.authProvider, { [updateSection]: patch });
    });
    const url = new URL(request.url);
