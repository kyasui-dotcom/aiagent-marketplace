export function createBillingRouteHandlers(deps = {}) {
  ...
  async function createPayjpTenantOnboardingForCurrent(storage, request, env) {
    ...
    const locale = payjpLocaleFromRequest(request, env, body);
    ...
    const applicationReturnTo = String(body?.return_to || body?.returnTo || '').trim() || payjpReturnToUrl(request, env);
    const applicationUrl = payjpApplicationUrlWithReturnTo(application?.url || application?.application_url || '', request, env, applicationReturnTo);
    let updated = null;
    await storage.mutate(async (draft) => {
      ...
      updated = upsertAccountSettingsInState(draft, current.login, current.user, current.authProvider, {
        payjp: {
          ...
          tenantApplicationUrl: applicationUrl,
          tenantApplicationReturnTo: applicationReturnTo,
  ...
  async function finishPayjpPlatformCharge3DSForCurrent(storage, request, env) {
    ...
    return { ok: true, locale, i18n: payjpLocalizedCopy(locale), charge_id: charge?.id || chargeId, paid: Boolean(charge?.paid), account: sanitizeAccountSettingsForClient(updated) };
  }
