export function createAppRouteHandlers(deps = {}) {
  ...
  async function handlePublisherContextIngest(storage, request, env) {
    ...
    await storage.appendAppContext(record);
    const publisherRecords = publisherRecordsFromContext
      ? publisherRecordsFromContext(record.payload, {
        ownerLogin,
        appContextId: record.id,
        shape: shaped.shape,
        nowIso: nowIso()
      })
      : { items: [] };
    const savedPublisherRecords = typeof storage.appendPublisherRecords === 'function'
      ? await storage.appendPublisherRecords(publisherRecords)
      : { items: [] };
    await touchEvent(storage, 'PUBLISHER_CONTEXT_INGESTED', `${ownerLogin || 'api client'} pushed context into Publisher`, {
      ...
      publisher_items: savedPublisherRecords.items?.length || 0
    });
    if (current.apiKey?.id) await recordOrderApiKeyUsage(storage, current, request);
    return json({
      ok: true,
      ...
      publisher_items: (savedPublisherRecords.items || []).map((item) => publicPublisherItem ? publicPublisherItem(item) : item),
      chat_url: appContextChatUrl(request, record)
    }, 201);
  }

  async function handleListPublisherItems(storage, request, env) {
    const current = await currentAgentRequesterContext(storage, request, env);
    const policy = runtimePolicy(env);
    if (!current.user && current.apiKeyStatus === 'invalid') return json({ error: 'Invalid API key' }, 401);
    if (!current.user && current.apiKeyStatus !== 'valid' && !policy.openWriteApiEnabled) return json({ error: 'Login or CAIt API key required' }, 401);
    if (typeof storage.listPublisherItems !== 'function') return json({ ok: true, items: [] });
    ...
    const validationStatus = String(url.searchParams.get('validation_status') || url.searchParams.get('validationStatus') || '').trim();
    const logins = new Set(loginsForCurrentAccount(current));
    const admin = typeof canViewAdminDashboard === 'function' && canViewAdminDashboard(current, env);
    const items = await storage.listPublisherItems({
      limit,
      channel,
      validationStatus,
    ...
    return json({
      ok: true,
      items: items.map((item) => publicPublisherItem ? publicPublisherItem(item) : item)
    });
  }
  ...
  async function handleListAppContexts(storage, request, env) {
    ...
    const contexts = (await storage.listAppContexts({
      ...
      .filter((record) => admin || logins.has(String(record.ownerLogin || '').trim().toLowerCase()))
