import { resolveHealthcheckUrl, verifyAgentByHealthcheck } from './verify.js';
...
export async function runAgentOnboardingCheck(agent, options = {}) {
  ...
  if (!jobEndpoint) {
    ...
    jobProbe = await probeEndpoint(jobEndpoint, { method: 'GET' });
  ...
  if (overallReady) {
    addAction('Create the first run', 'Open Chat and describe the first outcome, or pin this agent from AGENTS and dispatch immediately.', 99, 'ok');
  }
