...
function agentIsVisibleForSelection(agent = {}) {
  const metadata = object(agent.metadata);
...
function selectionRecordFromManifest(kind = '', manifest = {}, agent = null) {
  const metadata = object(manifest.metadata);
  ...
  return {
    ...
    selection_summary: text(manifest.description || agent?.description).slice(0, 260),
...
function mergeSelectionRecords(records = []) {
  const byKey = new Map();
  for (const record of records) {
    ...
    byKey.set(key, {
      ...
      selection_summary: text(record.selection_summary, existing.selection_summary),
      ...
      selection_hints: Array.from(new Set([...(existing.selection_hints || []), ...(record.selection_hints || [])])).slice(0, 16)
    });
  }
...
export function buildAgentSelectionIndex({ agents = [], includeInternal = true } = {}) {
  const records = [];
  if (includeInternal) {
    for (const [kind, manifest] of Object.entries(SAMPLE_AGENT_MANIFESTS)) {
      records.push(selectionRecordFromManifest(kind, manifest, null));
    }
  }
  for (const agent of Array.isArray(agents) ? agents : []) {
    if (!agentIsVisibleForSelection(agent)) continue;
    const manifest = manifestFromAgent(agent);
    records.push(selectionRecordFromManifest(manifest.kind || agent.metadata?.category || agent.id, manifest, agent));
  }
  return mergeSelectionRecords(records);
}

export function leaderReadableAgentSelectionIndex(input = {}) {
  return buildAgentSelectionIndex(input).map((record) => ({
    id: record.id,
    kind: record.kind,
    name: record.name,
    ...
    selection_summary: record.selection_summary,
