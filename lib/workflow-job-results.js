export function createWorkflowJobResultHandlers({
  ...
  async function failJob(storage, jobId, reason, extraLogs = [], options = {}) {
    ...
    const mutateFailure = typeof storage?.mutateBillingForJob === 'function'
      ? (mutator) => storage.mutateBillingForJob(jobId, mutator)
      : (mutator) => storage.mutate(mutator);
    const result = await mutateFailure(async (state) => {
      const job = state.jobs.find((item) => item.id === jobId);
      if (!job) return null;
