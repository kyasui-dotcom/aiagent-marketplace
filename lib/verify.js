import { SAMPLE_AGENT_KINDS } from './builtin-agents/agents/index.js';
import { nowIso } from './shared.js';

const MANAGED_SAMPLE_KINDS = new Set(SAMPLE_AGENT_KINDS);
...
function sampleKindFromUrl(value, type = 'any') {
  ...
  const directMatch = raw.match(/^\/(?:mock|sample-agents)\/([^/]+)\/(health|jobs)$/i);
  if (directMatch) {
    const kind = String(directMatch[1] || '').trim().toLowerCase();
    const urlType = String(directMatch[2] || '').trim().toLowerCase();
    if (!MANAGED_SAMPLE_KINDS.has(kind)) return '';
    if (type !== 'any' && urlType !== type) return '';
    return kind;
  }
  try {
    const parsed = new URL(raw);
    const match = parsed.pathname.match(/^\/(?:mock|sample-agents)\/([^/]+)\/(health|jobs)$/i);
    if (!match) return '';
...
export function sampleKindFromAgent(agent) {
  ...
  if (taggedSample && MANAGED_SAMPLE_KINDS.has(explicitKind)) return explicitKind;
  ...
  for (const kind of MANAGED_SAMPLE_KINDS) {
