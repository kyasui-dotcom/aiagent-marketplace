...
import {
  COMPOSITE_AGENT_COMPONENT_LIMIT,
...
function normalizeWorkflowLayerName(value = '') {
  const raw = String(value || '').trim().toLowerCase().replace(/[\s-]+/g, '_');
  if (['leader', 'orchestrator', 'orchestration'].includes(raw)) return 'leader';
  if (['data', 'analytics', 'data_analysis', 'measurement'].includes(raw)) return 'data';
  if (['research', 'analysis', 'evidence', 'source_collection'].includes(raw)) return 'research';
  if (['planning', 'planner', 'strategy', 'media_planning', 'plan'].includes(raw)) return 'planning';
  if (['preparation', 'prepare', 'prep', 'writing', 'copy', 'content_generation', 'landing', 'seo'].includes(raw)) return 'preparation';
  if (['action', 'execution', 'connector', 'publish', 'distribution', 'external_action'].includes(raw)) return 'action';
  if (['worker', 'specialist'].includes(raw)) return 'worker';
  return raw;
}

function workflowLayerNumberForName(value = '') {
  const layer = normalizeWorkflowLayerName(value);
  if (layer === 'leader') return 0;
  if (layer === 'data' || layer === 'research') return 1;
  if (layer === 'planning') return 2;
  if (layer === 'preparation') return 3;
  if (layer === 'action') return 4;
  return null;
}

function inferWorkflowLayerFromManifestSignals(input = {}) {
  const explicit = normalizeWorkflowLayerName(
    input.layer
    || input.agent_layer
    || input.workflow_layer
    || input.workflowLayer
    || input.sequence_phase
    || input.sequencePhase
    || ''
  );
  if (explicit && explicit !== 'worker') return explicit;
  ...
  if (String(input.agentRole || input.agent_role || '').toLowerCase() === 'leader' || /(^|_)(leader|orchestrat)/.test(text)) return 'leader';
  if (taskSet.has('data_analysis') || /(analytics|ga4|search_console|metric|measurement|funnel)/.test(text)) return 'data';
  if ([...taskSet].some((task) => ['x_post', 'instagram', 'reddit', 'indie_hackers', 'directory_submission', 'email_ops', 'cold_email', 'acquisition_automation', 'citation_ops'].includes(task))) return 'action';
  if ([...taskSet].some((task) => ['media_planner', 'growth', 'pricing'].includes(task))) return 'planning';
  if ([...taskSet].some((task) => ['writing', 'writer', 'landing', 'seo_specialist', 'seo'].includes(task))) return 'preparation';
  if ([...taskSet].some((task) => ['research', 'teardown', 'diligence', 'validation'].includes(task))) return 'research';
  if (/(publish|post|send|submit|github|pull_request|connector|oauth|x_post|reddit|indie|email|instagram|automation|external_action)/.test(text)) return 'action';
  if (/(writing|writer|copy|landing|seo_specialist|content|draft|creative|prepare)/.test(text)) return 'preparation';
  if (/(planner|planning|strategy|media_planner|roadmap|prioriti[sz]e)/.test(text)) return 'planning';
  if (/(research|source|evidence|teardown|diligence|validation|competitor)/.test(text)) return 'research';
  return 'worker';
}

function normalizeManifestTaskRouting(input = {}, rawMetadata = {}, taskTypes = [], pattern = {}, agentRole = 'worker') {
  const source = input.task_routing && typeof input.task_routing === 'object'
    ? input.task_routing
    : (input.taskRouting && typeof input.taskRouting === 'object'
        ? input.taskRouting
        : (rawMetadata.task_routing && typeof rawMetadata.task_routing === 'object'
            ? rawMetadata.task_routing
            : (rawMetadata.taskRouting && typeof rawMetadata.taskRouting === 'object' ? rawMetadata.taskRouting : {})));
  const layer = inferWorkflowLayerFromManifestSignals({
    ...source,
    layer: source.layer || source.workflow_layer || source.workflowLayer || rawMetadata.agent_layer || rawMetadata.layer || rawMetadata.workflow_layer || rawMetadata.workflowLayer,
    agentRole,
    name: input.name,
    description: input.description,
    taskTypes,
    tags: input.tags || input.team_tags || input.teamTags || rawMetadata.tags || rawMetadata.team_tags || rawMetadata.teamTags,
    capabilities: pattern.capabilities || []
  });
  const layerNumber = workflowLayerNumberForName(layer);
  return {
    ...(source && typeof source === 'object' ? source : {}),
    workflow_layer: layer,
    workflowLayer: layer,
    layer,
    ...(layerNumber !== null ? { layer_number: layerNumber, layerNumber } : {}),
    sequence_phase: layer === 'data' ? 'data' : (layer === 'worker' ? '' : layer),
    task_types: taskTypes,
    source: source.source || 'manifest_or_repo_analysis'
  };
}
...
export function normalizeManifest(input = {}, options = {}) {
  ...
  const taskRouting = normalizeManifestTaskRouting(input, rawMetadata, taskTypes, pattern, agentRole);
  ...
  const manifest = {
    ...
    metadata: {
      ...rawMetadata,
      agent_layer: rawMetadata.agent_layer || rawMetadata.layer || taskRouting.workflow_layer,
      workflow_layer: rawMetadata.workflow_layer || rawMetadata.workflowLayer || taskRouting.workflow_layer,
...
export function buildDraftManifestFromRepoAnalysis(input = {}) {
  ...
  const taskRouting = normalizeManifestTaskRouting({
    name: agentSkill?.name || packageJson?.name || pyprojectName || repoMeta?.name || 'github_agent',
    description,
    task_types: taskInference.taskTypes,
    tags,
    ...
    repository: repoMeta,
    source_files: Object.keys(files)
  }, taskInference.taskTypes, pattern, 'worker');
  const draftManifest = {
    schema_version: 'agent-manifest/v1',
    ...
    metadata: {
      ...
      tags,
      team_tags: tags,
      agent_layer: taskRouting.workflow_layer,
      workflow_layer: taskRouting.workflow_layer,
      task_routing: taskRouting,
      repository: {
        provider: 'github',
