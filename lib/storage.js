...
import { deliveryItemsFromJob } from './delivery-items.js';
import {
  DEFAULT_AGENT_SEEDS,
...
function mergeDeliveryItemSets(existing = [], incoming = [], limit = 5000) {
  ...
  for (const item of [...(Array.isArray(existing) ? existing : []), ...(Array.isArray(incoming) ? incoming : [])]) {
    ...
    if (!prior || deliveryItemTimestamp(normalized) >= deliveryItemTimestamp(prior)) merged.set(normalized.id, normalized);
  }
  return [...merged.values()]
...
const STORAGE_INIT_VERSION = '20260511_delivery_items_v1';
...
export function createD1LikeStorage(db, options = {}) {
  ...
  async function upsertJob(job) {
    ...
    await db.prepare(`INSERT INTO jobs (${columns.join(',')}) VALUES (${placeholders})
      ...
      .bind(...columns.map((column) => v[column]))
      .run();
    for (const item of deliveryItemsForJobs([job])) await upsertDeliveryItem(item);
  }
  async function upsertDeliveryItem(item) {
    const v = serializeDeliveryItem(item);
    await db.prepare(`INSERT OR REPLACE INTO delivery_items (id,owner_login,surface,item_type,status,title,summary,body,metadata_json,source_json,job_id,workflow_parent_id,workflow_task,workflow_agent_name,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
  ...
  async function loadPersistedState() {
    const [agentsRes, appsRes, jobsRes, deliveryItemsRes, eventsRes, accountsRes, feedbackReportsRes, chatTranscriptsRes, appContextsRes, recurringOrdersRes, emailDeliveriesRes, exactMatchActionsRes, appSettingsRes] = await Promise.all([
