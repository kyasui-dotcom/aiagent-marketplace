...
function dispatchCompletionStatusRank(status = '') {
  ...
  if (['leader_auto_retry_queued', 'retry_queued'].includes(safe)) return 6;
  if (['completion_sweep_running', 'dispatch_in_progress'].includes(safe)) return 5;
  if (['completion_queued'].includes(safe)) return 4;
  if (['dispatch_scheduled', 'leader_checkpoint_queued', 'leader_final_summary_queued', 'leader_adaptive_queued'].includes(safe)) return 3;
...
function mergeJobRecord(existing = null, incoming = null) {
  ...
  const existingActive = ['queued', 'claimed', 'running', 'dispatched'].includes(String(existing.status || '').trim().toLowerCase());
  const incomingTerminal = incomingRank === 4;
  ...
  const existingBlocked = String(existing.status || '').trim().toLowerCase() === 'blocked';
  const incomingBlocked = String(incoming.status || '').trim().toLowerCase() === 'blocked';
  ...
  const existingRetryMutation = incomingTerminal
    && existingActive
...
export function createD1LikeStorage(db, options = {}) {
  ...
  async function loadPersistedState() {
    const [agentsRes, appsRes, jobsRes, eventsRes, accountsRes, feedbackReportsRes, chatTranscriptsRes, appContextsRes, recurringOrdersRes, emailDeliveriesRes, exactMatchActionsRes, appSettingsRes] = await Promise.all([
