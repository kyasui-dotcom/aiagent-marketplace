...
const STORAGE_INIT_VERSION = '20260513_sample_provider_endpoint_v1';
...
export function createD1LikeStorage(db, options = {}) {
  ...
  if (!db) {
    ...
    return {
      ...
      async deleteAccountByLogin(login, options = {}) {
        ...
        const account = (inMemoryState.accounts || []).find((item) => String(item?.login || '').trim().toLowerCase() === safeLogin) || null;
        invalidateStateCache();
        inMemoryState.accounts = (inMemoryState.accounts || []).filter((item) => String(item?.login || '').trim().toLowerCase() !== safeLogin);
  ...
  async function loadPersistedState() {
    const [agentsRes, appsRes, jobsRes, deliveryItemsRes, eventsRes, accountsRes, feedbackReportsRes, chatTranscriptsRes, chatSessionsRes, appContextsRes, campaignsRes, recurringOrdersRes, emailDeliveriesRes, exactMatchActionsRes, appSettingsRes] = await Promise.all([
    ...
    async deleteAccountByLogin(login, options = {}) {
      return enqueueWrite(async () => {
        ...
        await db.prepare("UPDATE email_deliveries SET account_login=NULL, updated_at=? WHERE lower(account_login)=?")
          .bind(deletedAt, safeLogin)
          .run();
        await db.prepare('DELETE FROM accounts WHERE lower(login)=?')
          .bind(safeLogin)
          .run();
        return { deleted: Boolean(account), account: account ? structuredClone(account) : null };
      });
