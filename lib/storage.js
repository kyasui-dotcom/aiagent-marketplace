...
export function createD1LikeStorage(db, options = {}) {
  ...
  return {
    ...
    async mutateBillingForJob(jobId, mutator) {
      return enqueueWrite(async () => {
        await init();
        const job = await loadJobById(jobId);
        const agent = job?.assignedAgentId ? await loadAgentById(job.assignedAgentId) : null;
        const requesterLogin = String(requesterContextFromJob(job).login || '').trim().toLowerCase();
        const providerLogin = String(agent?.owner || '').trim().toLowerCase();
        const accountLogins = [...new Set([requesterLogin, providerLogin].filter(Boolean))];
        const accounts = [];
        for (const login of accountLogins) {
          const account = await loadAccountByLogin(login);
          if (account && !accounts.some((item) => String(item?.login || '').trim().toLowerCase() === String(account.login || '').trim().toLowerCase())) {
        ...
        const draft = {
          jobs: job ? [structuredClone(job)] : [],
          agents: agent ? [structuredClone(agent)] : [],
          accounts: accounts.map((account) => structuredClone(account))
        };
        const result = await mutator(draft);
        invalidateStateCache();
        const nextJob = (draft.jobs || []).find((item) => String(item?.id || '') === String(jobId || '')) || null;
        ...
        for (const account of draft.accounts || []) {
          if (!account?.login || accountIsDeleted(account)) continue;
          await upsertAccount(account);
          await syncAccountApiKeys(account);
        }
        return result;
