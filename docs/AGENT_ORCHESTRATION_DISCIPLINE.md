...
- Chat may guide, clarify, draft, guard, and explain before dispatch, but it must preserve the user's control: no external posting, sending, publishing, repository write, or money movement should happen without the explicit confirmation path and the responsible surface returning proof.
...
- Account registration, provider identity, agent registration, work execution, and scheduled agent work may remain available, but all in-app payment processing is removed. Do not add or keep checkout, card setup, saved payment methods, customer charges, subscriptions, invoices, payment-provider onboarding, automatic provider revenue split, or payout movement. Support may be donation-only outside CAIt; CAIt itself must not process donations, hold funds, or route money until a future payment design is explicitly approved and rebuilt; do not create unlimited free test-mode order execution for normal users.
- Public CLI/API-key access and MCP must stay behind explicit runtime flags such as `CAIT_DEVELOPER_API_ENABLED=1`, `CAIT_CLI_ENABLED=1`, or `CAIT_MCP_ENABLED=1`. When enabled for production, keep private user context, write actions, and app history behind CAIt session/API-key auth, and keep browser-owned app APIs separate from those external surfaces.
...
- External user self-service app registration is out of short-term scope. Still, keep manifest, handoff, and auth contracts ready for external apps; do not keep payment contracts active.
...
- Posting, publishing, sending, and execution approval are app responsibilities. Chat passes prepared data and handoff context, and may claim execution completion only when the app or connector returns proof.
