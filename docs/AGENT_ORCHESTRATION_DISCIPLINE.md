...
- During beta, account registration, provider identity, agent registration, and work execution may remain available, but live checkout, card setup, charges, and payout movement must stay paused. The short-term commercial model is CAIt selling AI agent services directly through a normal Stripe account; open marketplace payments, automatic provider revenue split, and payment-provider tenant onboarding are out of scope until explicitly re-approved. Each account may use the beta free allowance up to $10 in welcome credits; do not create unlimited free test-mode order execution for normal users. Keep Stripe billing contracts activation-ready behind `BILLING_ACTIVATION_ENABLED=1` or `BETA_BILLING_PAUSED=0`, and keep provider settlement manual/off-platform while automatic payout movement is disabled.
