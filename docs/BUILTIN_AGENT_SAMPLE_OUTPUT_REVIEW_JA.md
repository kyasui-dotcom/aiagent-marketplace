...
作成日: 2026-05-31 (Asia/Tokyo)

## 前提

- 対象: `lib/builtin-agents/agents/` の 43 sample agent。
- チェック方法: `npm run agent:test:snapshot -- --group all --out tmp\automation-2-before-20260531-sample-agents` と `npm run agent:test:compare -- --group all --baseline tmp\automation-2-before-20260531-sample-agents\snapshot.json --out tmp\automation-2-after-20260531-sample-agents`
