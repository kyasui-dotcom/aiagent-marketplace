...
作成日: 2026-06-10 (Asia/Tokyo)

## 前提

- 対象: `lib/builtin-agents/agents/` の 43 sample agent。
- チェック方法: `npm run agent:test:snapshot -- --group all --out tmp\automation-2-before-20260610-sample-agents` と `npm run agent:test:compare -- --group all --baseline tmp\automation-2-before-20260610-sample-agents\snapshot.json --out tmp\automation-2-after-20260610-sample-agents`
...
- 現状: 43件すべて `completed`。Before/After の変更は 0 件。前回 2026-06-09 証跡との差分も 0 件。公開注文可は 40 件、非公開 fixture は 3 件。
