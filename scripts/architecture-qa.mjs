...
const deliveryOrderBodyBuilderSource = read('public/delivery-order-body-builder.js');
...
assert.ok(deliveryActionContractSource.includes("from './delivery-order-body-builder.js'"), 'Delivery action contract should re-export delivery order body builders from their dedicated module');
assert.ok(!deliveryActionContractSource.includes('export function buildReportNextOrderBody'), 'Delivery action contract should not own report follow-up body building');
assert.ok(deliveryOrderBodyBuilderSource.includes('export function buildReportNextOrderBody'), 'Delivery order body builder should own report follow-up body building');
...
assert.ok(!deliveryActionContractSource.includes('export function buildDeliveryPublishOrderBody'), 'Delivery action contract should not own delivery publish order body building');
assert.ok(!deliveryActionContractSource.includes('function deliveryPublishTargetInstruction'), 'Delivery action contract should not own publish prompt instructions');
assert.ok(deliveryOrderBodyBuilderSource.includes('export function buildDeliveryPublishOrderBody'), 'Delivery order body builder should own delivery publish order body building');
assert.ok(deliveryOrderBodyBuilderSource.includes('export function deliveryPublishTargetInstruction'), 'Delivery order body builder should own publish prompt instructions');
