...
assert.ok(clientJs.includes("traffic_type: 'internal'"), 'Legacy client analytics should mark test traffic as internal.');
...
assert.ok(deliveryManagerHtml.includes('/delivery-manager.js?v=20260519a'), 'Delivery Manager should load the app-context receiving controller.');
...
assert.ok(chatJs.includes("content_type: artifactTypes[0] || file?.content_type || file?.contentType || file?.artifact_type || file?.artifactType || file?.type || ''"), 'App context handoff should preserve explicit delivery artifact type contracts before MIME fallbacks.');
...
assert.ok(worker.includes("url.pathname === '/mcp'") && mcpRoutes.includes('async function handleMcpRequest'), 'Worker should expose a public MCP endpoint through the MCP route module.');
