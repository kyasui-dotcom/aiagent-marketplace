...
assert.ok(chatJs.includes('MASAMUNE_STANDALONE_PRODUCT_URL'), 'Chat should know the standalone MASAMUNE product URL.');
assert.ok(chatJs.includes('marketing-ops.yasuikunihiro.workers.dev'), 'Chat should open the MASAMUNE marketing automation tool from direct MASAMUNE commands.');
assert.ok(chatJs.includes('CAIt Apps list'), 'Chat should explain that MASAMUNE is separate from the CAIt Apps list.');
assert.ok(!appManifestRegistryJs.includes('marketing-ops.yasuikunihiro.workers.dev'), 'MASAMUNE should stay a standalone CAIt API product, not a CAIt Apps registry listing.');
