...
const creatorHtmlPath = new URL('../public/creator.html', import.meta.url);
...
const chatConversationOwnerControllerPath = new URL('../public/chat-conversation-owner-controller.js', import.meta.url);
...
const chatPlanningProgressControllerPath = new URL('../public/chat-planning-progress-controller.js', import.meta.url);
const chatRuntimeStateControllerPath = new URL('../public/chat-runtime-state-controller.js', import.meta.url);
...
const analyticsHandoffTargetContractPath = new URL('../public/analytics-handoff-target-contract.js', import.meta.url);
...
const publisherDestinationContractPath = new URL('../public/publisher-destination-contract.js', import.meta.url);
const leadOpsJsPath = new URL('../public/lead-ops.js', import.meta.url);
const leadOpsHandoffTargetContractPath = new URL('../public/lead-ops-handoff-target-contract.js', import.meta.url);
const campaignOperationsJsPath = new URL('../public/campaign-operations.js', import.meta.url);
const campaignOperationsHandoffTargetContractPath = new URL('../public/campaign-operations-handoff-target-contract.js', import.meta.url);
const adsOpsJsPath = new URL('../public/ads-ops.js', import.meta.url);
const adsOpsHandoffTargetContractPath = new URL('../public/ads-ops-handoff-target-contract.js', import.meta.url);
const growthOpsJsPath = new URL('../public/growth-ops.js', import.meta.url);
const growthOpsHandoffTargetContractPath = new URL('../public/growth-ops-handoff-target-contract.js', import.meta.url);
const pricingOpsJsPath = new URL('../public/pricing-ops.js', import.meta.url);
const pricingOpsHandoffTargetContractPath = new URL('../public/pricing-ops-handoff-target-contract.js', import.meta.url);
...
const authContextPath = new URL('../lib/auth-context.js', import.meta.url);
...
execFileSync(process.execPath, ['--check', fileURLToPath(chatConversationOwnerControllerPath)], { stdio: 'pipe' });
...
execFileSync(process.execPath, ['--check', fileURLToPath(clientOrderRoutingControllerPath)], { stdio: 'pipe' });
...
execFileSync(process.execPath, ['--check', fileURLToPath(clientViewUtilsPath)], { stdio: 'pipe' });
...
execFileSync(process.execPath, ['--check', fileURLToPath(analyticsHandoffTargetContractPath)], { stdio: 'pipe' });
...
execFileSync(process.execPath, ['--check', fileURLToPath(publisherDestinationContractPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(leadOpsJsPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(leadOpsHandoffTargetContractPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(campaignOperationsJsPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(campaignOperationsHandoffTargetContractPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(adsOpsJsPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(adsOpsHandoffTargetContractPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(growthOpsJsPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(growthOpsHandoffTargetContractPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(pricingOpsJsPath)], { stdio: 'pipe' });
execFileSync(process.execPath, ['--check', fileURLToPath(pricingOpsHandoffTargetContractPath)], { stdio: 'pipe' });
...
const creatorHtml = readFileSync(creatorHtmlPath, 'utf8');
...
const chatConversationOwnerControllerJs = readFileSync(chatConversationOwnerControllerPath, 'utf8');
...
const chatPlanningProgressControllerJs = readFileSync(chatPlanningProgressControllerPath, 'utf8');
const chatRuntimeStateControllerJs = readFileSync(chatRuntimeStateControllerPath, 'utf8');
...
const analyticsHandoffTargetContractJs = readFileSync(analyticsHandoffTargetContractPath, 'utf8');
...
const publisherDestinationContractJs = readFileSync(publisherDestinationContractPath, 'utf8');
const leadOpsJs = readFileSync(leadOpsJsPath, 'utf8');
const leadOpsHandoffTargetContractJs = readFileSync(leadOpsHandoffTargetContractPath, 'utf8');
const campaignOperationsJs = readFileSync(campaignOperationsJsPath, 'utf8');
const campaignOperationsHandoffTargetContractJs = readFileSync(campaignOperationsHandoffTargetContractPath, 'utf8');
const adsOpsJs = readFileSync(adsOpsJsPath, 'utf8');
const adsOpsHandoffTargetContractJs = readFileSync(adsOpsHandoffTargetContractPath, 'utf8');
const growthOpsJs = readFileSync(growthOpsJsPath, 'utf8');
const growthOpsHandoffTargetContractJs = readFileSync(growthOpsHandoffTargetContractPath, 'utf8');
const pricingOpsJs = readFileSync(pricingOpsJsPath, 'utf8');
const pricingOpsHandoffTargetContractJs = readFileSync(pricingOpsHandoffTargetContractPath, 'utf8');
...
const authContextJs = readFileSync(authContextPath, 'utf8');
...
assert.ok(html.includes('href="/creator.html"'), 'Root footer should also link to the creator page.');
assert.ok(html.includes('Creator'), 'Root footer should keep the creator label.');
...
assert.ok(legalNoticeHtml.includes('https://aiagent-marketplace.net/legal-notice.html'), 'Legal notice should canonicalize to the legal notice URL.');
assert.ok(creatorHtml.includes('KunihiroYasui'), 'Creator page should show the creator name.');
assert.ok(creatorHtml.includes('https://x.com/KYasui0125'), 'Creator page should link to the public X account.');
assert.ok(creatorHtml.includes('https://aiagent-marketplace.net/creator.html'), 'Creator page should canonicalize to the creator URL.');
assert.ok(tokushohoRedirectHtml.includes('/legal-notice.html'), 'Old tokushoho URL should redirect to the legal notice URL.');
assert.ok(siteMapHtml.includes('https://aiagent-marketplace.net/legal-notice.html'), 'Site map structured data should keep the legal notice URL.');
assert.ok(siteMapHtml.includes('href="/legal-notice.html"'), 'Site map HTML should keep the legal notice URL.');
assert.ok(siteMapHtml.includes('https://aiagent-marketplace.net/creator.html'), 'Site map structured data should use the creator URL.');
assert.ok(siteMapHtml.includes('href="/creator.html"'), 'Site map HTML should use the creator URL.');
...
assert.ok(chatPlanningProgressControllerJs.includes("chatText('Plan status', 'プラン状況'"), 'Chat thinking state should render as a visible planning status card.');
assert.ok(chatPlanningProgressControllerJs.includes('Reading the request...'), 'Chat thinking state should use English planning-status copy on the English chat page.');
assert.ok(!chatJs.includes("label: 'Thinking...'"), 'Chat thinking state should not expose a separate Thinking label when a plan-status card is shown.');
...
assert.ok(chatBootstrapStateJs.includes('CHATUX_OPEN_CHAT_INTENT_TIMEOUT_MS = 6500'), 'Optional Open Chat intent classification should have a short UI timeout.');
assert.ok(chatJs.includes('async function requestOpenChatIntent') && chatJs.includes('signal: controller.signal'), 'Open Chat intent classification should be abortable from the browser.');
assert.ok(!chatJs.includes("api('/api/open-chat/intent'"), 'Open Chat intent classification should not block on the generic write/auth retry helper.');
assert.ok(
  chatJs.indexOf('renderActiveLeaderStatus: (...args) => renderActiveLeaderStatus(...args)') < chatJs.indexOf('const chatConversationOwnerController = createChatConversationOwnerController'),
  'Chat session startup must lazily resolve renderActiveLeaderStatus so Session Check cannot stall on module initialization.'
);
assert.ok(
  chatJs.indexOf('activeActorLabel: (...args) => activeActorLabel(...args)') < chatJs.indexOf('const chatConversationOwnerController = createChatConversationOwnerController'),
  'Chat runtime startup must lazily resolve activeActorLabel so authenticated chat can finish loading.'
);
assert.ok(
  chatEventBindingsControllerJs.includes('void refreshAuth({ maxAttempts: 1 });')
    && chatEventBindingsControllerJs.includes('window.setTimeout(ensureAuthRefreshProgress, 9000);'),
  'Chat startup auth status must not leave Checking session visible through multiple long retries.'
);
assert.ok(
  chatBootstrapStateJs.includes('authRefreshRequest: null')
    && chatJs.includes("els.authStatus.textContent = 'Session status unavailable. Retrying...'")
    && chatJs.includes('void refreshAuth({ force: true, maxAttempts: 1 });'),
  'Chat auth refresh watchdog must replace stale Checking session text and retry without overlapping normal refreshes.'
);
...
assert.ok(clientJs.includes('createClientParallelOrderController'), 'Client app should delegate parallel-order queue behavior to a dedicated controller.');
...
assert.ok(clientJs.includes("from './client-work-chat-thread-controller.js?v=20260601b'"), 'Client app should load the extracted work-chat thread controller cache key.');
...
assert.ok(chatJs.includes("from './chat-intake-controller.js?v=20260628c'"), 'Chat should load the extracted intake controller cache key.');
...
assert.ok(clientJs.includes("from './client-order-routing-controller.js?v=20260528c'"), 'Legacy client should load the extracted order routing controller cache key.');
assert.ok(clientOrderRoutingControllerJs.includes('function orderRoutingDecision'), 'Order routing decisions should live in the extracted controller.');
assert.ok(clientJs.includes("from './client-run-composer-controller.js?v=20260528d'"), 'Legacy client should load the extracted run composer controller cache key.');
assert.ok(clientRunComposerControllerJs.includes('function renderRunCreateStatus'), 'Run composer status rendering should live in the extracted controller.');
assert.ok(clientRunComposerControllerJs.includes('function renderRunEstimateCard'), 'Run composer estimate rendering should live in the extracted controller.');
assert.ok(!clientJs.includes('function renderRunCreateStatus'), 'Legacy client should delegate run composer status rendering to the extracted controller.');
...
assert.ok(chatJs.includes("from './chat-runtime-state-controller.js"), 'Chat runtime restore and persistence should be delegated to the runtime state controller.');
assert.ok(chatRuntimeStateControllerJs.includes('function chatRuntimeStateSnapshot'), 'Chat runtime state controller should own runtime snapshot construction.');
assert.ok(chatRuntimeStateControllerJs.includes('function applyRestoredChatSnapshot'), 'Chat runtime state controller should own restored snapshot application.');
assert.ok(!chatJs.includes('function chatRuntimeStateSnapshot'), 'Chat entrypoint should not retain runtime snapshot construction after the module split.');
...
assert.ok(chatUsageLibraryControllerJs.includes('contextContract: app.contextContract || app.context_contract || manifest.contextContract || manifest.context_contract || null'), 'Chat app manifest normalization should preserve app context contracts.');
assert.ok(chatUsageLibraryControllerJs.includes('contextContract: { ...(existing.contextContract || {}), ...(normalized.contextContract || {}) }'), 'Chat app manifest merging should not drop app context contracts.');
...
assert.ok(chatSchedulePanelControllerJs.includes('Completed order to rerun'), 'Chat schedules should be based on completed orders, not unconfirmed free-text requests.');
...
assert.ok(chatEventBindingsControllerJs.indexOf('void refreshChatSessionHistory({ force: true });') < chatEventBindingsControllerJs.indexOf('void refreshAuth({ maxAttempts: 1 });'), 'Chat should start loading the session list before the full auth status request.');
...
assert.ok(chatSessionSidebarControllerJs.includes('function scheduleRuntimeChatSessionPersist'), 'Chat should defer local/session persistence work until after the visible message append.');
assert.ok(chatSessionSidebarControllerJs.includes('windowRef.setTimeout') && chatSessionSidebarControllerJs.includes('scheduleRuntimeChatSessionPersist();'), 'Chat session persistence should be debounced instead of running synchronously for every message.');
...
assert.ok(loginHtml.includes('type="module" src="/login.js?v=20260628a"'), 'Login page should load the current login controller.');
...
assert.ok(loginJs.includes("const noticeText = els.trustNotice.querySelector('span');"), 'Login trust notice should not break the session check when the notice text node changes.');
...
assert.ok(analyticsHtml.includes('/analytics-console.js?v=20260602a'), 'Analytics Console should load the app-context receiving controller.');
assert.ok(analyticsJs.includes("analytics-handoff-target-contract.js?v=20260602a"), 'Analytics Console should import handoff target options from the contract module.');
assert.ok(analyticsJs.includes('analyticsHandoffTargetsForContext(target)'), 'Analytics Console should delegate handoff target ordering to the contract module.');
assert.ok(!analyticsHtml.includes('<option value="cmo_leader"') && !analyticsHtml.includes('<option value="seo_specialist"'), 'Analytics Console HTML must not hardcode concrete agent handoff targets.');
assert.ok(!analyticsJs.includes("target: 'cmo_leader'") && !analyticsJs.includes("handoff_targets: [target, 'seo_specialist', 'growth']"), 'Analytics Console controller must not hardcode concrete agent handoff target defaults or fallback arrays.');
assert.ok(analyticsHandoffTargetContractJs.includes('analyticsHandoffTargetOptions'), 'Analytics handoff target option ownership should live in the target contract module.');
assert.ok(analyticsHandoffTargetContractJs.includes('analyticsHandoffTargetsForContext'), 'Analytics handoff target ordering policy should live in the target contract module.');
...
assert.ok(leadOpsHtml.includes('/lead-ops.js?v=20260602a'), 'Lead Ops should load the app-context receiving controller.');
...
assert.ok(campaignOperationsHtml.includes('/campaign-operations.js?v=20260602a'), 'Campaign Operations should load the app-context receiving controller.');
assert.ok(campaignOperationsJs.includes("campaign-operations-handoff-target-contract.js?v=20260602a"), 'Campaign Operations should import handoff target ordering from the contract module.');
assert.ok(campaignOperationsJs.includes('campaignOperationsHandoffTargets()'), 'Campaign Operations should delegate handoff target ordering to the contract module.');
assert.ok(!campaignOperationsJs.includes("handoff_targets: ['cmo_leader', 'campaign_operations', 'analytics_console', 'publisher_approval_studio']"), 'Campaign Operations controller must not hardcode concrete handoff target arrays.');
assert.ok(campaignOperationsHandoffTargetContractJs.includes('campaignOperationsHandoffTargets'), 'Campaign Operations handoff target policy should live in the target contract module.');
...
assert.ok(adsOpsHtml.includes('/ads-ops.js?v=20260602a'), 'Ads Launch Console should load the app-context receiving controller.');
assert.ok(adsOpsJs.includes("ads-ops-handoff-target-contract.js?v=20260602a"), 'Ads Launch Console should import handoff target ordering from the contract module.');
assert.ok(adsOpsJs.includes('adsOpsHandoffTargets()'), 'Ads Launch Console should delegate handoff target ordering to the contract module.');
assert.ok(!adsOpsJs.includes("handoff_targets: ['ads_planner', 'ads_launch_console', 'campaign_operations', 'analytics_console']"), 'Ads Launch Console controller must not hardcode concrete handoff target arrays.');
assert.ok(adsOpsHandoffTargetContractJs.includes('adsOpsHandoffTargets'), 'Ads Launch Console handoff target policy should live in the target contract module.');
...
assert.ok(growthOpsHtml.includes('/growth-ops.js?v=20260602a'), 'Growth Experiment Console should load the current app-context controller.');
assert.ok(growthOpsJs.includes("growth-ops-handoff-target-contract.js?v=20260602a"), 'Growth Experiment Console should import handoff target ordering from the contract module.');
assert.ok(growthOpsJs.includes('growthPublisherLaunchHandoffTargets()'), 'Growth Publisher launches should delegate target order to the contract module.');
assert.ok(!growthOpsJs.includes("handoff_targets: ['publisher-approval-studio', 'cmo_leader', 'seo_specialist']") && !growthOpsJs.includes("handoff_targets: ['growth-experiment-console', 'growth', 'campaign_operations', 'analytics_console']"), 'Growth Experiment Console controller must not hardcode concrete handoff target arrays.');
assert.ok(growthOpsHandoffTargetContractJs.includes('growthPublisherLaunchHandoffTargets'), 'Growth handoff target policy should live in the target contract module.');
...
assert.ok(pricingOpsHtml.includes('/pricing-ops.js?v=20260602a'), 'Pricing Decision Console should load the current app-context controller.');
assert.ok(pricingOpsJs.includes("pricing-ops-handoff-target-contract.js?v=20260602a"), 'Pricing Decision Console should import handoff target ordering from the contract module.');
assert.ok(pricingOpsJs.includes('pricingOpsHandoffTargets()'), 'Pricing Decision Console should delegate handoff target ordering to the contract module.');
assert.ok(!pricingOpsJs.includes("handoff_targets: ['pricing-decision-console', 'cfo_leader', 'pricing']"), 'Pricing Decision Console controller must not hardcode concrete handoff target arrays.');
assert.ok(pricingOpsHandoffTargetContractJs.includes('pricingOpsHandoffTargets'), 'Pricing Decision Console handoff target policy should live in the target contract module.');
...
assert.ok(chatHtml.includes('/chat.js?v=20260628d'), 'Chat page should load the current compact chat header and composer controller.');
...
assert.ok(chatJs.includes('createChatPlanningProgressController'), 'Chat should delegate planning/progress UI state to a dedicated controller.');
assert.ok(chatPlanningProgressControllerJs.includes('function planningStatusConfig'), 'Planning/progress controller should own pre-order planning status copy.');
assert.ok(chatPlanningProgressControllerJs.includes('function showProgressNarrator'), 'Planning/progress controller should own live order progress narration.');
...
assert.ok(chatJs.includes('APP_WORKSPACE_GROUPS') && chatUsageLibraryControllerJs.includes('groupedAppPanelEntries'), 'Chat Apps panel should show merged app workspaces from the shared app registry instead of every internal lane.');
...
assert.ok(publisherJs.includes("from './publisher-destination-contract.js?v=20260628d'"), 'Publisher Studio should load the extracted destination contract module cache key.');
assert.ok(!publisherJs.includes('const PUBLISH_DESTINATION_PROFILES'), 'Publisher destination profiles should stay outside publisher-approval.js.');
assert.ok(publisherDestinationContractJs.includes('PUBLISH_DESTINATION_PROFILES'), 'Publisher destination contract should own media destination normalization before connector handoff.');
assert.ok(publisherDestinationContractJs.includes("key: 'github_pr'") && publisherDestinationContractJs.includes("capability: 'site_publish_packet'"), 'Publisher destination contract should separate generic owned-site publish packets from explicit GitHub PR handoffs.');
...
assert.ok(publisherDestinationContractJs.includes("key: 'x'"), 'Publisher destination contract should keep X as its own publish destination.');
assert.ok(publisherDestinationContractJs.includes("key: 'reddit'"), 'Publisher destination contract should keep Reddit as its own publish destination.');
assert.ok(publisherDestinationContractJs.includes("key: 'indie_hackers'"), 'Publisher destination contract should keep Indie Hackers as its own publish destination.');
assert.ok(publisherDestinationContractJs.includes("key: 'instagram'"), 'Publisher destination contract should keep Instagram as its own publish destination.');
...
assert.ok(publisherDestinationContractJs.includes('manual_social_copy'), 'Publisher destination contract should keep generic social copy out of GitHub PR fallback.');
assert.ok(publisherDestinationContractJs.includes('Generic publishing packet'), 'Publisher destination contract should not default ambiguous packets to GitHub PR.');
assert.ok(publisherDestinationContractJs.includes('wordpress_application_password'), 'Publisher destination contract should expose WordPress as an external-app style publish connector.');
...
assert.ok(publisherDestinationContractJs.includes('PUBLISHER_CONTRACT_ALIASES') && publisherDestinationContractJs.includes('publisher_packet') && publisherDestinationContractJs.includes('content_package'), 'Publisher destination contract should import AIAGENT publisher packet aliases from server raw_context.');
...
assert.ok(leadOpsJs.includes("lead-ops-handoff-target-contract.js?v=20260602a"), 'Lead Ops should import handoff target options from the contract module.');
assert.ok(leadOpsJs.includes('leadOpsSourcingHandoffTargets()'), 'Lead Ops sourcing requests should delegate target order to the contract module.');
assert.ok(leadOpsJs.includes('leadOpsContextHandoffTargets(target)'), 'Lead Ops context packets should delegate target order to the contract module.');
assert.ok(!leadOpsHtml.includes('<option value="cmo_leader"') && !leadOpsHtml.includes('<option value="list_creator"'), 'Lead Ops HTML must not hardcode concrete agent handoff targets.');
assert.ok(!leadOpsJs.includes("handoff_targets: ['list_creator', 'cmo_leader', 'email_ops']") && !leadOpsJs.includes("handoff_targets: [target, 'list_creator', 'email_ops', 'sms_ops']"), 'Lead Ops controller must not hardcode concrete agent handoff target fallback arrays.');
assert.ok(leadOpsHandoffTargetContractJs.includes('leadOpsHandoffTargetOptions'), 'Lead Ops handoff target option ownership should live in the target contract module.');
assert.ok(leadOpsHandoffTargetContractJs.includes('leadOpsSourcingHandoffTargets'), 'Lead Ops sourcing target policy should live in the target contract module.');
...
assert.ok(leadOpsJs.includes('No lead rows loaded.') || leadOpsJs.includes('まだ営業先がありません。'), 'Lead Ops should render an explicit empty state before server context is loaded.');
...
assert.ok(
  chatJs.includes('function ensureAuthRefreshProgress')
    && [chatJs, chatEventBindingsControllerJs].join('\n').includes('window.setTimeout(ensureAuthRefreshProgress, 9000)')
    && chatJs.includes("els.authStatus.textContent = 'Session status unavailable. Retrying...'"),
  'Chat should replace stale Checking session text and retry if startup auth refresh stalls.'
);
...
assert.ok(
  chatJs.includes('leaderConversationTaskTypeFromText')
    && chatConversationOwnerUtilsJs.includes('export function leaderConversationTaskTypeFromText'),
  'Leader chat consultation requests should be resolved by the conversation-owner utility instead of Open Chat intent intake.'
);
assert.ok(
  chatConversationOwnerControllerJs.includes('function activateLeaderForChat')
    && chatEventBindingsControllerJs.includes('handleLeaderConversationRequest(prompt)'),
  'Leader chat consultation should switch the visible conversation owner before order-intent classification.'
);
...
assert.ok(chatJs.includes("from './chat-conversation-owner-controller.js?v=20260628c'"), 'Chat conversation owner side effects should stay in the conversation owner controller.');
assert.ok(chatConversationOwnerControllerJs.includes('function lockedLeaderOwnerForPrompt'), 'Chat should preserve a confirmed leader unless the user explicitly asks to change it.');
...
assert.ok(chatConversationOwnerControllerJs.includes('data-chat-action="switch-leader"'), 'Chat should offer an explicit switch-leader action instead of automatically changing the leader.');
...
assert.ok(chatConversationOwnerControllerJs.includes('function lockedAgentOwnerForPrompt'), 'Chat should preserve a confirmed agent across follow-up turns.');
assert.ok(chatConversationOwnerControllerJs.includes('Agent:'), 'Chat should show when an individual agent owns the conversation.');
...
assert.ok(chatConversationOwnerControllerJs.includes('function setConversationOwnerFromPrepared'), 'Chat should switch the visible conversation owner from prepare-order responses.');
assert.ok(!chatJs.includes('function setConversationOwnerFromPrepared'), 'Chat entrypoint should not own prepared-response owner switching.');
...
assert.ok(chatJs.includes("from './chat-order-create-recovery.js?v=20260628c'"), 'Chat should load the extracted order-create recovery controller cache key.');
...
assert.ok(chatOrderCreateRecoveryJs.includes('normalizeRecoveryText') && chatJs.includes('normalizeRecoveryText'), 'Chat recovery cleanup should use the recovery module text normalizer instead of an undefined entrypoint helper.');
...
assert.ok(chatEventBindingsControllerJs.includes('!state.pendingIntake && appCommandId') && chatEventBindingsControllerJs.includes('!state.pendingIntake && libraryScope'), 'Pending intake answers should not be stolen by broad app/library command matching.');
...
assert.ok(chatRuntimeStateControllerJs.includes('notifyMilestones: false'), 'Chat startup backfill should not post stale completed-order milestone notices for restored history.');
...
assert.ok(
  authHelpers.includes('session?.user?.email')
    && authHelpers.includes('session?.user?.accountId')
    && authHelpers.includes('session?.user?.id')
    && authHelpers.includes('session?.googleIdentity?.email')
    && authHelpers.includes('session?.githubIdentity?.login'),
  'Auth helper chat gate should accept signed sessions repairable from email/id instead of looping login for sessions without user.login.'
);
assert.ok(
  authContextJs.includes('const identityUser = session?.user || session?.googleIdentity || session?.githubIdentity || null;')
    && authContextJs.includes('const sessionLogin = sessionAccountRepairLogin(session, null);')
    && authContextJs.includes('const sessionLogin = sessionAccountRepairLogin(existingSession, null);'),
  'Auth context should use the same repairable login identity for session validity checks as auth status.'
);
assert.ok(authHelpers.includes('async function handleAdminPageRequest'), 'Auth helper module should serve the admin shell.');
assert.ok(httpCore.includes('function legacyLegalNoticeRedirect') && httpCore.includes("url.pathname !== '/tokushoho'") && httpCore.includes("url.pathname = '/legal-notice.html'"), 'HTTP core should redirect the old tokushoho URL to the legal notice URL before assets extension handling.');
...
assert.ok(workerAssets.includes("'/admin.html'"));
assert.ok(workerAssets.includes("'/admin.css'"));
assert.ok(workerAssets.includes("'/admin.js'"));
assert.ok(workerAssets.includes("'/provider-identity.html'"));
assert.ok(workerAssets.includes("'/provider-identity.js'"));
...
assert.ok(
  [clientJs, clientRunComposerControllerJs].some((source) => /Listing can proceed, but CAIt no longer processes payments, billing, donations, or payouts/.test(source)),
  'Agent registration UI copy should allow listing while warning that money actions are removed.'
);
assert.ok(
  [clientJs, clientRunComposerControllerJs].some((source) => source.includes('A Stripe Payment Link for external donation support is only a future option after review')),
  'Agent registration UI copy should name reviewed external donation support.'
);
...
assert.ok(workerAssets.includes("'/home.css'"));
assert.ok(workerAssets.includes("'/apps.html'"));
assert.ok(workerAssets.includes("'/apps.js'"));
assert.ok(workerAssets.includes("'/analytics-console.html'"));
assert.ok(workerAssets.includes("'/publisher-approval.html'"));
assert.ok(workerAssets.includes("'/lead-ops.html'"));
assert.ok(workerAssets.includes("'/campaign-operations.html'"));
assert.ok(workerAssets.includes("'/ads-ops.html'"));
assert.ok(workerAssets.includes("'/ads-ops.js'"));
assert.ok(workerAssets.includes("'/delivery-manager.html'"));
assert.ok(workerAssets.includes("'/cait-app-bridge.js'"));
