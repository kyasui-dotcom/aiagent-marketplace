...
assert.ok(helpHtml.includes('CAIT CHAT COMMANDS'));
...
assert.ok(newsHtml.includes('START now leads with CAIt Chat and keeps provider earning one step away'));
...
assert.ok(newsHtml.includes('CAIt Chat now turns rough intent into order-ready briefs'));
