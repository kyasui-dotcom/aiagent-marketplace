...
export function createOpenChatIntakeUtils(options = {}) {
  ...
  function openChatLeaderIntakeSignals(prompt = '', inputCounts = {}) {
    ...
    return {
      ...
      automationPreference: /(自動化|自動|自走|ぐるぐる|回す|回したい|定期|毎日|毎週|毎月|毎時|スケジュール|予約|cron|scheduled|schedule|recurring|repeat|loop|automate|automation|autopilot|run automatically|one[-\s]?off|single run|manual|手動|単発|一回だけ|1回だけ|確認してから|止められる)/i.test(text),
  ...
  function openChatMissingLeaderIntakeFields(taskType = '', prompt = '', inputCounts = {}) {
    ...
    if (profile === 'general') {
      ...
      if (String(taskType || '').trim().toLowerCase() === 'cmo_leader' && !signals.automationPreference) missing.push('automation_or_one_off_preference');
