...
import { createClientDeliveryGenericRenderer } from './client-delivery-generic-renderer.js?v=20260602a';
import { createClientDeliveryAuthorityController } from './client-delivery-authority-controller.js?v=20260602a';
...
export function createClientDeliveryActionController(deps = {}) {
  ...
  const deliveryGenericRenderer = createClientDeliveryGenericRenderer({
    connectorActionForChat,
    deliveryControlFieldsForType,
    deliveryGoogleSourceFieldDescriptors,
    deliveryGoogleSourceLoadLabel,
    ...
    deliveryUiText,
    escapeHtml,
    authorityOwnerLabelForRun: (...args) => authorityOwnerLabelForRun(...args),
    genericDeliverableAuthoritySummary: (...args) => genericDeliverableAuthoritySummary(...args),
    genericDeliverableScheduleLabel: (...args) => genericDeliverableScheduleLabel(...args),
    genericDeliverableSectionDescriptors,
    repoOptionsDatalist: (...args) => repoOptionsDatalist(...args),
    xConnectorIdentityForClient
  });
  const {
    renderGenericDeliverableCard: renderGenericDeliverableCardFromContext,
    renderGenericDeliverableControls
  } = deliveryGenericRenderer;
...
const deliveryAuthorityController = createClientDeliveryAuthorityController({
  connectorStatusForClient,
  googleIncludeGroupsFromAuthorityRequest,
  normalizeClientConnector,
  normalizeClientConnectorCapabilityList,
  ...
  normalizeGoogleIncludeGroup,
  normalizeRepoFullName,
  persistGenericDeliverableDraft: (...args) => persistGenericDeliverableDraft(...args),
  state,
  updateGenericDeliverableDraft: (...args) => updateGenericDeliverableDraft(...args)
});
const {
...
async function postExactToInstagram(draft = {}, options = {}) {
  ...
  function renderGenericDeliverableCard(run = null, deliverable = null) {
    ...
    return renderGenericDeliverableCardFromContext(context);
  }
